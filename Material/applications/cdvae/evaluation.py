...
from src.evaluate_utils import get_real_samples, get_generation_res
from train import get_scaler


def save_real_samples(graph_dataset):
    ...
    (gt_frac_coords, gt_num_atoms, gt_atom_types,
     gt_lengths, gt_angles) = get_real_samples(graph_dataset)
...
def task_generation(model, graph_dataset, ld_kwargs, gen_args):
    ...
    save_real_samples(graph_dataset)
...
def main(args):
    ...
    if "gen" in args.tasks:
        task_generation(model, graph_dataset, ld_kwargs, args)
