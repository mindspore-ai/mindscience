...
# ============================================================================
"""
Determined irrep type
"""
from mindchemistry import Irreps, FullyConnectedTensorProduct


def BalancedIrreps(lmax, vec_dim, sh_type=True):
    r"""
    Allocates irreps equally along channel budget, resulting in unequal numbers of irreps in
    ratios of 2l_i + 1 to 2l_j + 1.
    """
    irrep_spec = "0e"
...
def WeightBalancedIrreps(irreps_in1_scalar, irreps_in2, sh=True, lmax=None, dtype=None, ncon_dtype=None):
    r"""
    ...
    if sh:
        irreps_in1 = (Irreps.spherical_harmonics(lmax) * n).sort().irreps.simplify()
    else:
        irreps_in1 = BalancedIrreps(lmax, n)
    weight_numel1 = FullyConnectedTensorProduct(
        irreps_in1, irreps_in2, irreps_in1, ncon_dtype, dtype=dtype
    ).weight_numel
    weight_numel_scalar = FullyConnectedTensorProduct(
        irreps_in1_scalar, Irreps("1x0e"), irreps_in1_scalar, ncon_dtype, dtype=dtype
    ).weight_numel

    while weight_numel1 < weight_numel_scalar:
        n += 1
        if sh:
            irreps_in1 = (Irreps.spherical_harmonics(lmax) * n).sort().irreps.simplify()
        else:
