...
from ..utils.func import norm_keep
...
class SphericalHarmonics(nn.Cell):
    ...
    Args:
        ...
        irreps_out (Union[str, `Irreps`, None]): irreducible representations of input for spherical harmonics. Default: None.
    ...
    def __init__(self, irreps_out, normalize, normalization='integral', irreps_in=None, dtype=float32):
        ...
        if not normalization in ['integral', 'component', 'norm']:
            raise ValueError
        ...
        if self.normalization == 'integral':
            self.norm_factors = [
                ...
                ops.ones(2 * l + 1, dtype)
                for l in self._ls_list
            ]
        elif self.normalization == 'component':
            self.norm_factors = [
                _sqrt(2 * l + 1., dtype) * ops.ones(2 * l + 1, dtype)
                for l in self._ls_list
            ]
    ...
    def construct(self, x):
        ...
        if not self.normalization == 'norm':
...
def spherical_harmonics(l, x, normalize=True, normalization='integral'):
    ...
    Spherical harmonics are polynomials defined on the 3d space :math:`Y^l: \mathbb{R}^3 \longrightarrow \mathbb{R}^{2l+1}`
    ...
    Args:
        ...
        normalization (str): {'integral', 'component', 'norm'}, normalization method of the output tensors. Default: 'intergral'.
...
def _spherical_harmonics(lmax: int, x, y, z):
    ...
    sh_5_8 = -0.141421356237309 * sh_4_0 * x - 0.748331477354788 * sh_4_2 * x + \
             0.748331477354788 * sh_4_6 * z + 0.8 * sh_4_7 * y - 0.141421356237309 * sh_4_8 * z
    ...
    sh_10_4 = 0.774596669241484 * sh_9_14 * x + 0.173205080756887 * sh_9_16 * x - \
              0.173205080756888 * sh_9_2 * z + 0.8 * sh_9_3 * y + 0.774596669241483 * sh_9_4 * z
    ...
    sh_10_18 = -0.0707106781186548 * sh_9_0 * x + 0.874642784226796 * sh_9_16 * z + \
               0.6 * sh_9_17 * y - 0.0707106781186546 * sh_9_18 * z - 0.874642784226796 * sh_9_2 * x
