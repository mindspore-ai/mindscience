...
from .templates import TemplateHitFeaturizer
from .hhsearch import HHSearch
from .msa_query import MmseqQuery
from .parsers import parse_fasta, parse_hhr, parse_a3m
...
class RawFeatureGenerator:
    ...
    def __init__(self, max_hits=20, msa_length=512):
        """Search the a3m info for a given FASTA file."""

        database_search_config = load_config(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                          "database_query_config.yaml"))
    ...
    def monomer_feature_generate(self, fasta_path, save_file_path=None):
        ...
        if save_file_path:
            with open(save_file_path, "wb") as f:
                ...
                logging.info("Feature dict saved at" + save_file_path)
        return feature_dict
