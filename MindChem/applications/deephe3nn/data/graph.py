...
def get_edge_fea(cart_coords, lattice, default_dtype_np, edge_key):
    ...
    cart_coords_j = (
        cart_coords[edge_key[:, 4] - 1]
        + edge_key[:, :3].astype(convert2numpyt(default_dtype_np)) @ lattice
    )
...
def get_graph(
    ...
    if data_folder is not None:
        atom_num_orbital = load_orbital_types(
            os.path.join(data_folder, "orbital_types.dat")
        )

        if os.path.isfile(os.path.join(data_folder, "info.json")):
            with open(os.path.join(data_folder, "info.json"), "r", encoding="utf-8") as info_f:
    ...
    if create_from_dft:
        ...
        edge_idx = np.stack(
            [np.array(edge_idx_first, dtype="int64"), np.array(edge_idx, dtype="int64")]
        )
        ...
        if inference:
            ...
            if edge_aij:
                ...
                for index in range(edge_fea.shape[0]):
                    ...
                    if key in aij_dict:
                        aij_mask[index] = True
                        aij[index, : atom_num_orbital[i], : atom_num_orbital[j]] = (
                            aij_dict[key]
                        )
                    else:
                ...
                data = [
                    ...
                    np.expand_dims(lattice, 0),
                    edge_key,
                    aij,
                    aij_mask,
                    np.array(atom_num_orbital),
                    spinful,
                ]
