...
class AijData:
    ...
    def __init__(
        ...
        if load_graph:
            ...
            logging.info(
                "Finish loading the processed %s structures, spinful: %s",
                len(self.data_numpy),
                self.info["spinful"],
            )
            logging.info(
                "the number of atomic types: %s), cost %s seconds",
                len(self.info["index_to_Z"]),
                time.time() - begin,
            )
    ...
    def mask_padding(data_list_mask):
        ...
        for i in range(length):
            ...
            data_list_mask[i][1] = np.pad(
                data_list_mask[i][1], ((0, 0), (0, padlength)), pad_type
            )
            data_list_mask[i][2] = np.pad(
                data_list_mask[i][2], ((0, padlength), (0, 0)), pad_type
            )
            data_list_mask[i][6] = np.pad(
                data_list_mask[i][6], ((0, padlength), (0, 0)), pad_type
            )
            data_list_mask[i][9] = np.pad(
                data_list_mask[i][9], ((0, padlength), (0, 0)), pad_type
            )
            data_list_mask[i][10] = np.pad(
                data_list_mask[i][10],
                ((0, padlength), (0, 0)),
                pad_type,
            ...
            mask_dim1 = np.append(
                np.ones(data_list_mask[i][11][0], dtype=np.float32),
    ...
    def process(self):
        ...
        if self.multiprocessing:
            with Pool() as pool:
                data_list = list(
                    tqdm.tqdm(
                        pool.imap(self.process_worker, folder_list),
                        total=len(folder_list),
                    )
                )
        else:
            data_list = [
                self.process_worker(folder) for folder in tqdm.tqdm(folder_list)
            ]

        logging.info(
            "Finish processing %d structures, have cost %d seconds",
            len(data_list),
            time.time() - begin,
        )
        ...
        _, orbital_types = load_orbital_types(
            path=os.path.join(folder_list[0], "orbital_types.dat"),
        ...
        for i in range(length):
            orbital_types_new.append(
                orbital_types[np.where(elements == index_to_z[i])[0][0]]
            )
        ...
        zip_data = [
            data_list,
            {
                ...
                "orbital_types": orbital_types_new,
            },
        ]
        ...
        logging.info(
            "Finished saving %d structures to save_graph_dir, have cost %d seconds",
            len(data_list),
            time.time() - begin,
        )
    ...
    def process_worker(self, folder):
        ...
        structure = Structure(
            np.loadtxt(os.path.join(folder, "lat.dat")).T,
            elements,
            site_positions,
    ...
    def set_mask(self, targets, del_aij=True):
        ...
        """
        dtype = np.float32
        equivariant_blocks, out_js_list, out_slices = process_targets(
            self.info["orbital_types"], self.info["index_to_Z"], targets
        )
        index_x = "x"
        ...
        for data in self.data_numpy:
            ...
            atomic_number_edge_i = data[map_dict.get("x", None)][
                data[map_dict.get(index_edge_index, None)][0]
            ]
            atomic_number_edge_j = data[map_dict.get("x", None)][
                data[map_dict.get(index_edge_index, None)][1]
            ]
            ...
            for index_out, equivariant_block in enumerate(equivariant_blocks):
                for n_m_str, block_slice in equivariant_block.items():
                    ...
                    if data[map_dict.get(index_aij, None)] is not None:
                        out_slice = slice(
                            out_slices[index_out], out_slices[index_out + 1]
                        )
                        ...
                        label[condition_index[0], out_slice] += data[
                            map_dict.get(index_aij, None)
                        ][:, condition_slice_i, condition_slice_j].reshape(
                            num_edges, -1
                        )[
                            condition_index
                        ]
