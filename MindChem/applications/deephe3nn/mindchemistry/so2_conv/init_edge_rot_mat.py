...
def init_edge_rot_mat(edge_distance_vec):
    ...
    edge_vec_2 = ops.div(
        edge_vec_2,
        ops.sqrt(ops.maximum(ops.sum(edge_vec_2**2, dim=1), epsilon)).view(-1, 1),
    )
    ...
    edge_vec_2 = ops.where(
        ops.broadcast_to(ops.gt(vec_dot, vec_dot_b), edge_vec_2b.shape),
        edge_vec_2b,
        edge_vec_2,
    )
    vec_dot = ops.abs(ops.sum(edge_vec_2 * norm_x, dim=1)).view(-1, 1)
    edge_vec_2 = ops.where(
        ops.broadcast_to(ops.gt(vec_dot, vec_dot_c), edge_vec_2c.shape),
        edge_vec_2c,
        edge_vec_2,
    )
    ...
    norm_z = ops.div(
        norm_z, ops.sqrt(ops.maximum(ops.sum(norm_z**2, dim=1, keepdim=True), epsilon))
    )
    norm_z = ops.div(
        norm_z, ops.sqrt(ops.maximum(ops.sum(norm_z**2, dim=1), epsilon)).view(-1, 1)
    )

    norm_y = ms_np.cross(norm_x, norm_z, axis=1)
    norm_y = ops.div(
        norm_y, ops.sqrt(ops.maximum(ops.sum(norm_y**2, dim=1, keepdim=True), epsilon))
    )
