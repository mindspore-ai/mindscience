...
class GaussianBasis(nn.Cell):
    ...
    def __init__(
        ...
        widths = ms.Tensor(
            (offset[1] - offset[0]) * ms.numpy.ones_like(offset)
        )  # FloatTensor
        if trainable:
...
class SlipSlopLR:
    ...
    def decay(self):
        ...
        logging.info(
            "Learning rate %s is decayed from %s to %s", num_lr, last_lr, new_lr
        )
...
class RevertDecayLR:
    ...
    def step(self, val_loss):
        ...
        if self.scheduler_type == 1:
            if self.loss_smoothed is None:
                ...
                self.loss_smoothed = (
                    self.alpha * val_loss + (1.0 - self.alpha) * self.loss_smoothed
                )
        ...
        while not save_complete:
            try:
                ...
                logging.info(
                    "Interrupting while saving model might cause the saved model to be deprecated"
                )
    ...
    def save_model(self, epoch, val_loss, is_best=False, **kwargs):
        ...
        state.update(kwargs)
        ms.save_checkpoint(
            self.model, os.path.join(self.save_model_dir, model_ckpt), append_dict=state
        )
        if is_best:
            shutil.copyfile(
                os.path.join(self.save_model_dir, model_ckpt),
                os.path.join(self.save_model_dir, "best_model.ckpt"),
            )
    ...
    orbital_types_cumsum = list(
        map(
            lambda x: np.concatenate(
                [np.zeros(1, dtype=np.int32), np.cumsum(2 * x + 1)]
            ),
            orbital_types,
        )
    )
...
def orbital_analysis(
    ...
    if required_block_type == "s":
        ...
        for target in targets:
            ...
            for n_m_str, block_indices in target.items():
                ...
                if l1 is None and l2 is None:
                    ...
                    net_out_irreps += irreps_from_l1l2(
                        l1, l2, 1, spinful, no_parity=no_parity
                    )[0]
        ...
        il_list = (
            []
        )  # [[1, 1, 2, 0], ...] this means the hopping is from 1st l=1 orbital to 0th l=2 orbital.
        ...
        net_out_irreps, targets = get_net_out_irreps(
            no_parity, spinful, required_block_type, il_list, hoppings_list
        )
