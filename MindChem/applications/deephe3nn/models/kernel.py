...
class DatasetInfo:
    ...
    def __eq__(self, other) -> bool:
        ...
        if other.__class__ != __class__:
            raise ValueError
        a = other.spinful == self.spinful
        b = np.all(other.index_to_z == self.index_to_z)
        c = other.orbital_types == self.orbital_types
        return a * b * c
    ...
    def from_dataset(cls, dataset: AijData):
        ...
        """
        return cls(
            dataset.info["spinful"],
            dataset.info["index_to_Z"],
            dataset.info["orbital_types"],
        )
...
class NetOutInfo:
    ...
    def __init__(self, target_blocks, dataset_info: DatasetInfo):
        ...
        self.blocks, self.js, self.slices = process_targets(
            dataset_info.orbital_types, dataset_info.index_to_z, target_blocks
        )
...
class DeepHE3Kernel:
    ...
    @staticmethod
    def train_process(
        config, scheduler, optimizer, train_loader, val_loader, net, construct_kernel
    ):
        """
        DeepHE3Kernel class train_process
        """
        loss_scaler = DynamicLossScaler(
            scale_value=8192, scale_factor=2, scale_window=50
        )
        ...
        backward = ms.value_and_grad(
            forward_train, None, weights=net.trainable_params(), has_aux=True
        )
        ...
        def train_step(
            ...
            (mse_loss, h_pred), grads = backward(
                data_x,
                data_edge_index,
                data_edge_attr,
                data_label,
        ...
        while epoch < config.num_epoch and learning_rate > config.min_lr:
            ...
            for batch in train_loader:
                ...
                endtime = time.time()
                logging.info(
                    "----------------------train epoch: %s-------step: %s", epoch, step
                )
            ...
            for batch in val_loader:
                ...
                endtime = time.time()
                logging.info(
                    "----------------------eval epoch: %s-------step: %s", epoch, step
                )
            ...
            best_loss = min(best_loss, val_losses.avg)
    ...
    def train(self, train_config):
        ...
        scheduler = RevertDecayLR(
            ...
            optimizer,
            config.save_dir,
            config.revert_decay_patience,
            config.revert_decay_rate,
    ...
    def validate(self, train_config):
        ...
        self.val_process(
            config, scheduler, optimizer, val_loader, net, construct_kernel
        )
    ...
    def register_constructor(self):
        ...
        construct_kernel = E3TensorDecompNet(
            config.net_out_irreps,
    ...
    def config_set_target(self):
        ...
        """
        o, i, s = (
            self.dataset_info.orbital_types,
            self.dataset_info.index_to_z,
            self.dataset_info.spinful,
        )
        self.train_config.set_target(o, i, s)
        self.net_out_info = NetOutInfo(
            self.train_config.target_blocks, self.dataset_info
        )
    ...
    def get_loader(self):
        ...
        generator_dataset_train = ds.GeneratorDataset(
            dataset_tuple,
            column_names=[
                "x",
                "edge_index",
        ...
        generator_dataset_val = ds.GeneratorDataset(
            dataset_tuple,
            column_names=[
                "x",
                "edge_index",
