...
class SkipConnection(nn.Cell):
    ...
    def __init__(self, irreps_in, irreps_out, use_fp16=False):
        ...
        if irreps_in == irreps_out:
            ...
            self.sc = Linear(
                irreps_in=irreps_in, irreps_out=irreps_out, ncon_dtype=self.global_dtype
            )
...
class E3LayerNorm(nn.Cell):
    ...
    def construct(
        ...
        for index, (mul, ir) in enumerate(self.irreps_in):
            field = x[:, ix : ix + mul * ir.dim].reshape(
                -1, mul, ir.dim
            )  # [node, mul, repr]
            if self.subtract_mean or ir.l == 0:
                if mask_degree is not None:
                    mean = self.aggregate(
                        field, batch, dim_size=batch_size, mask=mask_dim1
                    )
                else:
            ...
            if (
                self.divide_norm or ir.l == 0
            ):  # do not divide norm for l>0 irreps if subtract_mean=False
                if mask_degree is not None:
                    var = self.aggregate(
                        ops.square(field), batch, dim_size=batch_size, mask=mask_dim1
                    )
                else:
                ...
                for i_out, (mul_out, ir3) in enumerate(irreps_out):
                    if ir3 in ir1 * ir2:
                        ...
                        weights1.append(
                            Parameter(
                                Tensor(
                                    np.random.randn(mul1, mul_out), dtype=ms.float32
                                ),
                                name=weights1_name,
                            )
                        )
                        weights2.append(
                            Parameter(
                                Tensor(
                                    np.random.randn(mul2, mul_out), dtype=ms.float32
                                ),
                                name=weights2_name,
                            )
                        )
...
class RotateNet(nn.Cell):
    ...
    def __init__(self, default_dtype_ms, spinful=False):
        ...
        self.us_openmx = {
            0: ms.Tensor([1], dtype=ms.complex64),
            1: ms.Tensor(
                [
                    [-1 / 1.4142135623730951, 1j / 1.4142135623730951, 0],
                    [0, 0, 1],
                    [1 / 1.4142135623730951, 1j / 1.4142135623730951, 0],
                ],
                dtype=ms.complex64,
    ...
    def construct(self, h, l_left, l_right):
        ...
        # wiki2openmx_H
        return (
            self.us_openmx2wiki.get(l_left, None).T
            @ h
            @ self.us_openmx2wiki.get(l_right, None)
        )
...
class E3TensorDecompNet(nn.Cell):
    ...
    def __init__(
        ...
        for _, (h_l1, h_l2) in enumerate(out_js_list):
            ...
            mul = 1
            _, required_irreps_out_single, _ = irreps_from_l1l2(
            ...
            for _, ir in required_irreps_out_single:
                for _ in range(mul):
                    ...
                    wm.append(test1)
                    wm_h.append(
                        wigner_3j(ir.l, h_l1, h_l2, dtype=default_dtype_ms)
                        * (2 * ir.l + 1)
                    )
