...
def get_struc_from_np_list(np_list):
    ...
    for cry_array in np_list:
        try:
            struct = Structure(
                lattice=Lattice.from_parameters(
                    *(cry_array["lengths"].tolist() + cry_array["angles"].tolist())
                ),
...
def main(args):
    ...
    for i in trange(len(gt_list)):
        ...
        gt_struc = gt_list[i]
        rms_single, struc_single = get_rms(pred_struc, gt_struc, num_eval, pred_list[i])
    ...
    if Path(output_path).exists():
        ...
        if Path(metrics_out_file).exists():
            with open(metrics_out_file, "r", encoding="utf-8") as f:
