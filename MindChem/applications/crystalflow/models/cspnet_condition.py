...
class SinusoidsEmbedding(nn.Cell):
    ...
    as described in the paper of CrystalFlow.
    ...
    def construct(self, x):
        ...
        emb = ops.ExpandDims()(x, -1) * Tensor(
            self.frequencies, dtype=mindspore.float32
...
class CSPLayer(nn.Cell):
    r"""One layer of the GNN denoiser. For the input node feature
        ...
        :math:`L` in the polar-decomposition form,
        and the fractional coordinates :math:`f_i`, the formula is defined as:

    .. math::

        m_{ij}^{(s)} = mlp_m(h_i^{(s-1)}, h_j^{(s-1)}, L, \text{edge_emb}(f_j - f_i)),
    ...
    def __init__(self, hidden_dim=512, act_fn=nn.SiLU(), dis_emb=None):
        """Initialization
        ...
        self.edge_mlp = nn.SequentialCell(
            [
                nn.Dense(hidden_dim * 2 + 6 + self.dis_dim, hidden_dim),
                act_fn,
    ...
    def edge_model(
        self, node_features, lattice_polar, edge_index, edge2graph, frac_diff, edge_mask
    ):
        ...
        lattice_polar = ops.Gather()(lattice_polar, edge2graph, 0)

        edges_input = ops.Concat(axis=1)((hi, hj, frac_diff, lattice_polar))
    ...
    def node_model(self, node_features, edge_features, edge_index, edge_mask):
        ...
        agg = self.edge_scatter(
            edge_features, edge_index, dim_size=node_features.shape[0], mask=edge_mask
        )
    ...
    def construct(
        ...
        edge_features = self.edge_model(
            node_features, lattice_polar, edge_index, edge2graph, frac_diff, edge_mask
        )
        node_output = self.node_model(
            node_features, edge_features, edge_index, edge_mask
        )
...
class CSPNet(nn.Cell):
    ...
    def __init__(
        ...
        Args:
            ...
            cemb_dim (int): The dimension of condition embedding vector. Defaults to 1.
        """
        super().__init__()
        ...
        self.csp_layers = nn.CellList(
            [CSPLayer(hidden_dim, self.act_fn, self.dis_emb) for _ in range(num_layers)]
        )
        self.cemb_mixin = nn.CellList(
            [
                nn.Dense(hidden_dim, hidden_dim, has_bias=False)
                for _ in range(num_layers)
            ]
        )
        self.cemb_adapter = nn.CellList(
            [
                nn.SequentialCell(
                    [
                        nn.Dense(cemb_dim, hidden_dim),
                        self.act_fn,
                        nn.Dense(hidden_dim, hidden_dim),
                        self.act_fn,
                    ]
        ...
        self.lattice_out = nn.Dense(hidden_dim, 6, has_bias=False)
    ...
    def construct(
        ...
        Args:
            ...
            lattices_polar (Tensor): Lattice mattrix. Shape: (batchsize, 6)
            ...
            node_mask (Tensor): Node mask for padded tensor. Shape: (num_atoms,)
            edge_mask (Tensor): Edge mask for padded tensor. Shape: (num_edges,)
            cemb (Tensor): Embedded condition vector. Shape: (batchsize, cemb_dim)
        ...
        frac_diff = (frac_coords[edge_dst] - frac_coords[edge_src]) % 1
        ...
        cemb = self.lift_node(cemb, node2graph, mask=node_mask)

        for i in range(self.num_layers):
            cemb_bias = self.cemb_mixin[i](self.cemb_adapter[i](cemb))
            node_features = node_features + cemb_bias
            # csp layers
            node_features = self.csp_layers[i](
                node_features,
                lattice_polar,
                edge_index,
                edge2graph,
        ...
        graph_features = self.node_scatter(
            node_features, node2graph, dim_size=lattice_polar.shape[0], mask=node_mask
        )
