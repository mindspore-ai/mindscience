...
import numpy as np
from mindspore import (Tensor, load_checkpoint, load_param_into_net, mint, nn,
                       ops)
...
class SinusoidalTimeEmbeddings(nn.Cell):
    ...
    def construct(self, time):
        ...
        embeddings = ops.Concat(axis=-1)((ops.Sin()(embeddings), ops.Cos()(embeddings)))
        return embeddings
...
def test_cspnet():
    ...
    frac_coords = Tensor(
        [
            ...
            [0.00000000e00, 0.00000000e00, 0.00000000e00],
            [0.00000000e00, 0.00000000e00, 5.00000000e-01],
        ],
        dtype=ms.float32,
    )
    lengths = Tensor(
        [
            [3.86215806e00, 3.86215806e00, 3.86215806e00],
            [4.21191406e00, 4.21191454e00, 5.75016499e00],
        ],
        dtype=ms.float32,
    )
    ...
    edge_index = Tensor(
        [
            ...
            [0, 1, 0, 1, 2, 3, 4, 5, 2, 3, 4, 5, 2, 3, 4, 5, 2, 3, 4, 5],
        ],
        dtype=ms.int32,
    )
    ...
    lattices_out, coords_out = cspnet(
        t,
        atom_types,
        frac_coords,
        lattice_polar,
    ...
    assert mint.isclose(
        lattices_out, tar_lat_polar, rtol=1e-4, atol=1e-4
    ).all(), (
        f"For `cspnet`, the output should be {tar_lat_polar}, but got {lattices_out}."
    )
    assert mint.isclose(
        coords_out, tar_coord, rtol=1e-4, atol=1e-4
    ).all(), f"For `cspnet`, the output should be {tar_coord}, but got {coords_out}."
...
def test_flow():
    ...
    angles = Tensor(
        [
            [9.00000000e01, 9.00000000e01, 9.00000000e01],
            [9.00000000e01, 9.00000000e01, 1.20000000e02],
        ],
        dtype=ms.float32,
    )
    lattice_polar = Tensor(
    ...
    pred_l, tar_l, pred_f, tar_f = cspflow(
        atom_types,
        atom_types,
        lengths,
        angles,
    ...
    out_pred_l = Tensor(
        [
            [-0.54417396, 0.6183988, 0.25345746, 0.41497535, -0.00219233, 0.6622897],
            [-0.5647707, 0.68243337, 0.25912297, 0.45234668, 0.01847154, 0.6095263],
        ]
    )
    out_tar_l = Tensor(
        [
            [-0.02254689, 0.04679973, 0.3856261, -0.08269336, -0.08592724, 0.45530552],
            [-0.53301036, -0.21067567, -0.05119152, 0.04148455, -0.0907657, 0.3682214],
        ]
    )
    out_pred_f = Tensor(
        [
            [-0.7662705, 0.24618103, -0.4741043],
    ...
    out_tar_f = Tensor(
        [
            [0.20181239, -0.07186192, -0.40746307],
    ...
    assert mint.isclose(
        pred_l, out_pred_l, rtol=1e-4, atol=1e-4
    ).all(), f"For `cspnet`, the output should be {pred_l}, but got {out_pred_l}."
    assert mint.isclose(
        pred_f, out_pred_f, rtol=1e-4, atol=1e-4
    ).all(), f"For `cspnet`, the output should be {pred_f}, but got {out_pred_f}."
    assert mint.isclose(
        tar_l, out_tar_l, rtol=1e-4, atol=1e-4
    ).all(), f"For `cspnet`, the output should be {tar_l}, but got {out_tar_l}."
    assert mint.isclose(
        tar_f, out_tar_f, rtol=1e-4, atol=1e-4
    ).all(), f"For `cspnet`, the output should be {tar_f}, but got {out_tar_f}."
...
def test_loss():
    ...
    cspflow = CSPFlow(cspnet)
    download_file(
        "https://download-mindspore.osinfra.cn/mindscience/mindchemistry/crystalflow/ms_flow.ckpt",
        "ms_flow.ckpt",
    )
    ...
    def forward(
        ...
        pred_l, tar_l, pred_x, tar_x = cspflow(
            batch_size_valid,
            atom_types_step,
            lengths_step,
            angles_step,
        ...
        mseloss_x = loss_func_mse(
            pred_x, tar_x, mask=node_mask_step, num=node_num_valid
        )
        mseloss = mseloss_l + 10 * mseloss_x

        return mseloss, mseloss_l, mseloss_x

    train_datatset = fullconnect_dataset(
        name="mp_20",
    ...
    assert (
        mseloss_x <= 0.7
    ), "The denoising of fractional coordinates accuracy is not successful."
