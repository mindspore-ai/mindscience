...
class Crysloader(DataLoaderBase):
    ...
    def __iter__(self):
        ...
        while self.index < self.max_start_sample:
            ...
            frac_coords_step = self.gen_node_attr(
                self.frac_coords, batch_size, node_num
            )
            ...
            num_atoms_step = self.gen_global_attr(self.num_atoms, batch_size).to(
                ms.int32
            )
    ...
    def gen_global_attr(self, global_attr, batch_size):
        ...
        global_attr_step = self.pad_zero_to_end(
            global_attr_step, 0, self.batch_size - batch_size
        )
