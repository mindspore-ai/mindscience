...
- 基于 MindSpore 框架运行，可在配置文件中通过 `device_target` 字段选择后端（见 `configs/config_eval.yaml` 与 `evaluate.py`），默认使用 `Ascend`。

### 版本依赖

- 需要安装 `MindSpore == 2.7.0`（参考 `requirement.txt`）。
- 需要安装 `MindScience`，以提供等变计算相关的基础组件。
- 其它 Python 依赖见 `requirement.txt`。
...
- 在[数据集链接](https://download-mindspore.osinfra.cn/mindscience/mindchemistry/orb/dataset/)下载训练和测试数据集，放置于当前路径的 `dataset` 文件夹下（如果没有需要自己手动创建）。
- 在[模型链接](https://download-mindspore.osinfra.cn/mindscience/mindchemistry/orb/orb_ckpts/)下载 Orb 预训练模型 `orb-mptraj-only-v2.ckpt`，放置于当前路径的 `orb_ckpts` 文件夹下（如果没有需要自己手动创建）。
...
orb
    ...
    │
    ├── orb_ckpts
    │     └── orb-mptraj-only-v2.ckpt     预训练 ORB 模型
    │
    ├── configs                           训练 / 推理配置
...
- 模型主体由 `src` 与 `models` 目录中的模块构成：`OrbLoss` 与训练循环在 `finetune.py` 和 `src/trainer.py` 中实现，`evaluate.py` 提供推理与评估入口。
...
- 修改 `configs/config.yaml` 中的训练参数：
    - 设置微调阶段的训练和测试数据集，见 `train_data_path` 与 `val_data_path` 字段。
    - 设置训练加载的预训练模型权重文件，更改 `checkpoint_path` 字段。
- 安装依赖并运行：
...
- 修改 `configs/config_parallel.yaml` 和 `run_parallel.sh` 文件中的训练参数：
    - 设置微调阶段的训练和测试数据集，见 `train_data_path` 与 `val_data_path` 字段。
    - 设置训练加载的预训练模型权重文件，更改 `checkpoint_path` 字段。
    - 其它训练设置见 Training Configuration 部分。
    - 修改 `run_parallel.sh` 中 `--worker_num` 与 `--local_worker_num` 来设置使用的卡数。
...
- 修改 `configs/config_eval.yaml` 中的推理 / 评估参数：
    - 设置测试数据集，见 `val_data_path` 字段。
    - 设置推理加载的预训练或微调模型权重文件，更改 `checkpoint_path` 字段。
    - 其它推理设置见 Evaluating Configuration 部分。
