...
def diffusion(loader, model, num_evals, step_lr=1e-5):
    ...
    Args:
        loader (TupleIterator): The dataset loader iterator.
    ...
    Returns:
        Tuple(List[Dict], List[List[Dict]]): The ground truth and predicted crystals.
    """
    gt_struc = []
    pred_struc = []
    
    # 【修改】：使用 enumerate 来替代原来 loader 缺失的 .step 属性
    for step_idx, (atom_types_step, frac_coords_step, _, lengths_step, \
        angles_step, edge_index_step, node_batch_step, \
        node_mask_step, edge_mask_step, batch_mask, _, batch_size_step) in enumerate(loader):
        
        num_node_list = count_consecutive_occurrences(
            node_batch_step.asnumpy().tolist())

        # batch_size_step 现在是 Tensor，所以需要用 .asnumpy().item() 转回 int
        b_size = int(batch_size_step.asnumpy().item())
        pred_struc_batch = [[] for _ in range(b_size)]
        epoch_starttime = time.time()
        
        for eval_idx in range(num_evals):
            print(
                # 【修改】：直接使用 enumerate 的 step_idx
                f'Batch {step_idx} / Unknown (Depends on iterator), sample {eval_idx} / {num_evals}'
            )
            starttime = time.time()
            frac_coords_t, lattices_t = model.sample(atom_types_step,
                                                     node_batch_step,
                                                     edge_index_step,
                                                     node_mask_step,
                                                     edge_mask_step,
...
def main(args):
    ...
    test_data_source = Crysloader(config['test']['batch_size'],
                             *test_datatset,
                             shuffle_dataset=True)

    # ---------------- 关键修改：用 GeneratorDataset 包装 ----------------
    # 和 Crysloader yield 的 12 个输出顺序一一对应
    column_names = [
    ...
    # 用纯函数包裹，抹除长度属性，规避长度校验报错
    def test_data_generator():
        for batch in test_data_source:
            yield batch
            
    test_dataset_wrapped = ds.GeneratorDataset(source=test_data_generator, column_names=column_names)
    test_loader = test_dataset_wrapped.create_tuple_iterator()
    # ------------------------------------------------------------------
    ...
    model = CSPDiffusion(decoder)

    model.set_train(False)

    gt_struc, pred_struc = diffusion(test_loader,
                                     model,
                                     config['test']['num_eval'],
                                     step_lr=float(config['test']['step_lr']))
