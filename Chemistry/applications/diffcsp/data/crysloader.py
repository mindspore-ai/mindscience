...
"""dataloader file"""
import numpy as np
# 删除了 from mindspore import Tensor, ops 避免混用底层计算图节点
...
class Crysloader(DataLoaderBase):
    ...
    def __iter__(self):
        ...
        while self.index < self.max_start_sample:
            ...
            # 【修改点 1】：使用 np.reshape 和 np.ndarray.astype 替换 ops 操作
            ...
            # 【修改点 2】：将 Tensor 强转替换为 np.array 保持在 CPU 端的数据纯净性
    ...
    def gen_common_data(self, node_attr, edge_attr):
        ...
        # 【修改点 3】：移除所有 ms.Tensor 包装，改用 np.ndarray.astype()
