...
def parse_args():
    ...
    parser.add_argument('--device_target', type=str, default='CPU', choices=["GPU", "Ascend", "CPU"],
...
if __name__ == '__main__':
    ...
    cost_coord = config['train']['cost_coord']
    cost_lattice = config['train']['cost_lattice']
    ...
    model_parameters = filter(lambda p: p.requires_grad, cspflow.get_parameters())
    params = sum(np.prod(p.shape) for p in model_parameters)
    logging.info("The model you built has %s parameters.", params)

    optimizer = nn.Adam(params=cspflow.trainable_params())
    loss_func_mse = L2LossMask(reduction='mean')

    def forward(atom_types_step, frac_coords_step, _, lengths_step, angles_step, \
                lattice_polar_step, num_atoms_step, edge_index_step, batch_node2graph, \
        ...
        mseloss = cost_lattice * mseloss_l + cost_coord * mseloss_x

        return mseloss, mseloss_l, mseloss_x

    backward = ms.value_and_grad(forward, None, weights=cspflow.trainable_params(), has_aux=True)

    @ms.jit
    def train_step(atom_types_step, frac_coords_step, property_step, lengths_step, angles_step,
                   lattice_polar_step, num_atoms_step,
        ...
        (mseloss, mseloss_l, mseloss_x), grads = backward(atom_types_step, frac_coords_step, property_step,
                                                          lengths_step, angles_step, lattice_polar_step, num_atoms_step,
    ...
    # @ms.jit
    def eval_step(atom_types_step, frac_coords_step, property_step,
                  lengths_step, angles_step, lattice_polar_step, num_atoms_step,
                  edge_index_step, batch_node2graph, node_mask_step, edge_mask_step,
                  batch_mask, node_num_valid, batch_size_valid):
        """eval step - 纯前向计算，不复用包裹过 grad 的 forward"""
        
        # 1. 直接调用网络计算，不经过外部的 forward 函数
        pred_l, tar_l, pred_x, tar_x = cspflow(batch_size_valid, atom_types_step, lengths_step, 
                                               angles_step, lattice_polar_step, num_atoms_step, 
                                               frac_coords_step, batch_node2graph, edge_index_step, 
                                               node_mask_step, edge_mask_step, batch_mask)
    ...
    # 2. 将生成器包装成 MindSpore 的 GeneratorDataset
    # column_names 需要和你 yield 出来的 14 个变量数量一一对应
    column_names = [
        "node_attr_step", "frac_coords_step", "label_step", "lengths_step", 
        "angles_step", "lattice_polar_step", "num_atoms_step", "edge_index_step", 
        "node_batch_step", "node_mask", "edge_mask", "batch_size_mask", 
        "node_num", "batch_size"
    ]
    # 写一个小函数，包住它，彻底抹除类的长度信息
    ...
    # 3. 创建真正的数据迭代器 (output_numpy 默认为 False，会自动转为 ms.Tensor)
    ...
    while epoch < epoch_size:
        ...
        ms.save_checkpoint(cspflow.decoder, config['checkpoint']['last_path'])

        if epoch % 5 == 0:
            #################################################### validation ##########################################
            logging.info("+++++++++++++++ start validation +++++++++++++++++++++")
            cspflow.set_train(False)
            ...
            for atom_types_batch, frac_coords_batch, property_batch, lengths_batch, \
                ...
                eval_mseloss_record.update(mseloss_step.asnumpy())
