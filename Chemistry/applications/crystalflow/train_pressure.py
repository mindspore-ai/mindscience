...
# --------------
# only for reading pressure as condition
# --------------
"""train file"""
import os
import time
...
# =========================================
# use condition model
from models.cspnet_condition import CSPNet
from models.flow_condition import CSPFlow
# =========================================
from models.conditioning import GaussianExpansion
...
if __name__ == '__main__':
    ...
    cond_emb_model = GaussianExpansion(start=config['model']['conditions']['pressure']['start'],
                                       stop=config['model']['conditions']['pressure']['stop'],
                                       n_gaussians=['model']['conditions']['pressure']['n_out'],
                                       )
    cspnet = CSPNet(num_layers=config['model']['num_layers'], hidden_dim=config['model']['hidden_dim'],
                    num_freqs=config['model']['num_freqs'],
                    cemb_dim=['model']['conditions']['pressure']['n_out'])
    ...
    diffcsp = CSPFlow(cspnet, cond_emb_model)
    ...
    def forward(atom_types_step, frac_coords_step, _, lengths_step, angles_step, \
        ...
        """forward"""
        pred_l, tar_l, pred_x, tar_x = diffcsp(batch_size_valid, atom_types_step, lengths_step,
                                               angles_step, lattice_polar_step, num_atoms_step, frac_coords_step,
                                               batch_node2graph, edge_index_step,
    ...
    @ms.jit
    def train_step(atom_types_step, frac_coords_step, property_step,
    ...
    @ms.jit
    def eval_step(atom_types_step, frac_coords_step, property_step, lengths_step,
