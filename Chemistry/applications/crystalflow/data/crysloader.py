...
class Crysloader(DataLoaderBase):
    ...
    def __iter__(self):
        ...
        while self.index < self.max_start_sample:
            ...
            # 【修改】使用 np.reshape 并保持为 np.ndarray，不再强转为 ms.Tensor
            node_attr_step = np.reshape(node_attr_step, (-1,))
            node_attr_step = node_attr_step.astype(np.int32)
            ...
            # 【修改】将 to(ms.int32) 替换为 numpy 的 astype
            num_atoms_step = self.gen_global_attr(self.num_atoms, batch_size).astype(np.int32)

            self.add_step_index(batch_size)

            # 【修改】将原本强制转为 ms.Tensor 的标量转为 NumPy 标量/数组
    ...
    def gen_common_data(self, node_attr, edge_attr):
        ...
        # 【修改】将原本的 ms.Tensor 类型强转修改为 numpy astype 转换
        edge_index_step = edge_index_step.astype(np.int32)
        node_batch_step = node_batch_step.astype(np.int32)
        node_mask = node_mask.astype(np.int32)
        edge_mask = edge_mask.astype(np.int32)
        batch_size_mask = batch_size_mask.astype(np.int32)
    ...
    def gen_node_attr(self, node_attr, batch_size, node_num):
        ...
        # 【修改】统一替换为 np.float32 转换
        node_attr_step = node_attr_step.astype(np.float32)
    ...
    def gen_edge_attr(self, edge_attr, batch_size, edge_num):
        ...
        # 【修改】统一替换为 np.float32 转换
        edge_attr_step = edge_attr_step.astype(np.float32)
    ...
    def gen_global_attr(self, global_attr, batch_size):
        ...
        global_attr_step = self.pad_zero_to_end(global_attr_step, 0,
                                                self.batch_size - batch_size)
        # 【修改】统一替换为 np.float32 转换
        global_attr_step = global_attr_step.astype(np.float32)
