...
def fold_infer(args):
    ...
    model_cfg.evoformer.msa_stack_num = 48
    ...
    for seq_file in seq_files:
        ...
        pdb_path = f'./result/unrelaxed_{seq_name}.pdb'
...
def fold_train(args):
    ...
    step = 0
    np.random.seed(1)
    max_recycles = [int(np.random.uniform(size=1, low=3, high=4)) for _ in range(args.total_steps)]
    np.random.seed()
    names = os.listdir(args.pdb_path)
    names_list = []
    for _ in range(0, args.total_steps):
        names_list.append(names[0].split(".pdb")[0])
    ...
    for d in dataset_iter:
        ...
        if args.mixed_precision and step == 0:
            megafold.to_float(mstype.float16)
        input_list = []
        for recycle in range(max_recycle):
            inputs_feat = [Tensor(feat[recycle]) for feat in inputs_feats]
            if recycle == 0:
                input_list.extend(inputs_feat + [prev_pos, prev_msa_first_row, prev_pair] + ground_truth)
            else:
                input_list.extend(inputs_feat + ground_truth)
            if recycle == max_recycle - 1:
                t1 = time.time()
                prev_pos, prev_msa_first_row, prev_pair = train_net(*input_list, max_recycle)
                t2 = time.time()
        ...
        input_list.extend(inputs_feat + [prev_pos, prev_msa_first_row, prev_pair] + ground_truth)
        t3 = time.time()
        loss = train_net(*input_list)
        t4 = time.time()
        total_time = (t4 - t3) + (t2 - t1)
        loss_info = f"step is: {step}, total_loss: {loss[0]}, fape_sidechain_loss: {loss[1]}," \
                    ...
                    f" masked_loss: {loss[5]}, plddt_loss: {loss[6]}, step_time: {total_time}"
