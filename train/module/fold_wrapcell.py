...
class TrainOneStepCell(nn.Cell):
    ...
    def construct(self, *inputs):
        ...
        for i in range(inputs[-1]):
            if i == 0:
                out = self.network(*inputs[0:46])
            elif i == 1:
                out = self.network(*inputs[46:64], out[0], out[1], out[2], *inputs[64:89])
            else:
                out = self.network(*inputs[89:107], out[0], out[1], out[2], *inputs[107:132])
        return out
