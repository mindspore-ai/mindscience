...
class Interval(HyperCube):
    ...
    def __init__(self, name, coord_min, coord_max, dtype=np.float32, sampling_config=None):
        if not isinstance(coord_min, (int, float)) or not isinstance(coord_max, (int, float)):
            raise ValueError("coord_min: {} and coord_max: {} should be int or float for class Interval"
                             .format(coord_min, coord_max))
