...
class Data:
    ...
    Args:
        ...
        constraint_type (str, optional): constraint type of the specified dataset to get it's corresponding loss
            function. Default: ``None``. The constraint_type can be
            ``"equation"``, ``"bc"``, ``"ic"``, ``"label"`` or ``"function"``.

    Raises:
        TypeError: if `constraint_type` is ``None`` or constraint_type.lower() not in
...
class ExistedDataConfig:
    ...
    Args:
        ...
        data_format (str, optional): the format of existed data files. Default: ``'npy'``. The format of
            ``'npy'`` is supported now.
