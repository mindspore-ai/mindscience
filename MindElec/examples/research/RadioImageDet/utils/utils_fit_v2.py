...
def fit_one_epoch(model_train, model, grad_fn, yolo_loss, loss_history, eval_callback,
    ...
    for iteration, batch in enumerate(
        ...
        y_trues[0] = batch["y_true"]["y_trues"]
        images = ms.Tensor.from_numpy(np.array(images)).type(ms.float32)
        targets = [
            ms.Tensor.from_numpy(ann).type(
                ms.float32) for ann in targets]
        #   Problem: The data y_truths [0] generated by custom data occasionally
        #           has an extra dimension for one piece of data
        #   Solution: Use the following three lines of code to remove the extra dimension
        for i, y_t in enumerate(y_trues[0]):
            if len(y_t.shape) == 5:
                y_trues[0][i] = y_t.squeeze(0)
        y_trues = [
