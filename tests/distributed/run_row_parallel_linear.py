...
def test_row_parallel_linear(args):
    """
    Feature: Row-parallel linear layer correctness.

    Description: Initializes tensor parallelism and compares a standard Linear layer
    with the RowParallelLinear implementation for equivalent outputs under different
    input/sequence parallel settings.

    Expectation: When input_is_parallel is True, each worker's shard matches the
    corresponding slice of the full Linear output; when use_sequence_parallel is True,
    sequence slices are compared. Overall, comparisons pass within numeric tolerances.
    """
    ...
    ms.set_seed(2048)
    row_parallel_linear = RowParallelLinear(args.in_features, args.out_features, bias=args.bias,
    ...
    if args.input_is_parallel:
        ...
        stride = args.in_features // args.num_workers
        parallel_linear_out = row_parallel_linear(x[:, :, tp_rank*stride:(tp_rank+1)*stride])
    else:
        parallel_linear_out = row_parallel_linear(x)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_is_parallel", action="store_true", help="Whether gather output")
    parser.add_argument("--use_sequence_parallel", action="store_true", help="Whether use sequence parallel")
    args_ = parser.parse_args()

    test_row_parallel_linear(args_)
