...
from mindscience import KNO1D, RelativeRMSELoss
...
test_data_path = '/home/workspace/mindspore_dataset/mindscience/kno1d'
...
def test_kno1d_forward_accuracy(mode):
    """
    Feature: KNO1D forward accuracy test
    Description: Test the forward accuracy of the KNO1D model in both GRAPH_MODE and PYNATIVE_MODE.
    Expectation: The output should match the target prediction data within the specified relative and absolute
                tolerance values, ensuring the forward pass of the KNO1D model is accurate.
    """
    ms.set_context(mode=mode)
    ckpt_path = os.path.join(test_data_path, 'kno1d.ckpt')

    model = KNO1D()
    params = load_checkpoint(ckpt_path)
    load_param_into_net(model, params)
    input_data = np.load(os.path.join(test_data_path, 'kno1d_input.npy'))
    test_inputs = Tensor(input_data, mstype.float32)
    output, output_rec = model(test_inputs)
    output = output.asnumpy()
    output_rec = output_rec.asnumpy()
    load_data = np.load(os.path.join(test_data_path, 'kno1d_output.npz'))
    ...
    assert validate_ans, "The verification of KNO1D forward accuracy is not successful."


@pytest.mark.level0
@pytest.mark.platform_arm_ascend910b_training
@pytest.mark.env_onecard
def test_kno1d_grad_accuracy():
    """
    Feature: KNO1D gradient accuracy test
    Description: Test the accuracy of the computed gradients for the KNO1D model. 
    ...
    load_param_into_net(model, params)
    input_data = np.load(os.path.join(test_data_path, 'kno1d_input.npy'))
    input_label = np.load(os.path.join(test_data_path, 'kno1d_label.npy'))
    test_inputs = Tensor(input_data, mstype.float32)
    ...
    with np.load(os.path.join(test_data_path, 'kno1d_grads.npz')) as data:
    ...
    assert validate_ans, "The verification of KNO1D grad accuracy is not successful."
