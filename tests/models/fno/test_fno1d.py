...
from mindscience import FNO1D, RelativeRMSELoss, load_yaml_config
from mindscience.pde import SteadyFlowWithLoss
...
from tools import validate_checkpoint, compare_output, FP16_RTOL, FP16_ATOL
...
test_data_path = '/home/workspace/mindspore_dataset/mindscience/fno1d'
...
def test_fno1d_checkpoint():
    ...
    ckpt_path = os.path.join(test_data_path, 'fno1d.ckpt')
...
def test_fno1d_forward_accuracy(mode):
    ...
    input_data = np.load(os.path.join(test_data_path, 'fno1d_input.npy'))
    test_inputs = Tensor(input_data, mstype.float32)
    output = model(test_inputs)
    output = output.asnumpy()
    output_target = np.load(os.path.join(test_data_path, 'fno1d_pred.npy'))
...
def test_fno1d_grad_accuracy():
    ...
    load_param_into_net(model, params)
    input_data = np.load(os.path.join(test_data_path, 'fno1d_input.npy'))
    input_label = np.load(os.path.join(test_data_path, 'fno1d_input_label.npy'))
    test_inputs = Tensor(input_data, mstype.float32)
    ...
    with np.load(os.path.join(test_data_path, 'fno1d_grads.npz')) as data:
