...
本案例利用 Fourier Neural Operator 学习初始状态到时刻 $t=1$ 状态的映射，实现一维Burgers'方程的求解：
...
| 硬件资源                | Ascend, 显存32G            |
| MindSpore版本           | 2.7.0                 |               |
| 数据集                  | [一维Burgers方程分辨率数据集](https://download-mindspore.osinfra.cn/mindscience/mindflow/dataset/applications/data_driven/burgers/)      |
| 参数量                  | 5.57e5                   |
| 训练参数                | resolution=1024, modes=16, hidden_channels=64, depth=4, batch_size=8, epoch=100 |
| 测试参数                | batch_size=8          |
| 优化器                  | Adam                 |
| 训练损失(MSE)           | 0.004872               |
| 验证损失(RMSE)          | 0.001088                |
