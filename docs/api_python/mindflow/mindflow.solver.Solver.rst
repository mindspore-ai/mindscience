.. py:class:: mindflow.solver.Solver(network, optimizer, loss_fn="l2", metrics=None, eval_network=None, eval_indexes=None, amp_level="O0", **kwargs)
    ...
    参数：
        ...
        - **amp_level** (str) - `mindspore.amp.build_train_network <https://www.mindspore.cn/docs/zh-CN/r2.0.0-alpha/api_python/amp/mindspore.amp.build_train_network.html#mindspore.amp.build_train_network>`_  中参数 `level` 的选项，混合精确训练的级别。支持[“O0”, “O2”, “O3”, “auto”]。默认值：“O0”。
          ...
          - auto：设置为不同设备中的建议级别。在GPU上设置级别为O2，Ascend上设置级别为O3 。建议的级别由导出经验选择，不能总是保持平均数值。用户应指定特殊网络的级别。
            GPU上建议使用O2，Ascend上建议使用O3。有关 `amp_level` 设置的详情可查阅 `mindspore.amp.build_train_network <https://www.mindspore.cn/docs/zh-CN/r2.0.0-alpha/api_python/amp/mindspore.amp.build_train_network.html#mindspore.amp.build_train_network>`_  。
    ...
    .. py:method:: train_with_eval(epoch, train_dataset, test_dataset, eval_interval, callbacks=None, dataset_sink_mode=True, sink_size=-1)
