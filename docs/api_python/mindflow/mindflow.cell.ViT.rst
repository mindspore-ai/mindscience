.. py:class:: mindflow.cell.ViT(img_size=(192, 384), input_dims=7, output_dims=3, patch_size=16, encoder_depth=12, encoder_embed_dim=768, encoder_num_heads=12, decoder_depth=8, decoder_embed_dim=512, decoder_num_heads=16, mlp_ratio=4, dropout_rate=1.0, compute_dtype=mindspore.float16)

    该模块基于ViT，包括encoder层、decoding_embedding层、decoder层和dense层。

    参数：
        - **img_size** (tuple[int]) - 输入的图像尺寸。默认值：（192,384）。
        - **input_dims** (int) - 输入的输入特征维度。默认值：7。
        - **output_dims** (int) - 输出的输出特征维度。默认值：3。
        ...
        - **compute_dtype** (dtype) - encoder层、decoding_embedding层、decoder层和dense层的数据类型。默认值：mindspore.float16。
