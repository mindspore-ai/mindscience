...
.. py:class:: mindearth.cell.DEMNet(in_channels=1, channels=256, kernel_size=3, scale=5, num_blocks=42)
    ...
    输入：
        - **x** (Tensor) - shape为 :math:`(batch\_size, channels, height\_size, width\_size)` 的Tensor。

    输出：
        Tensor，DEM网络的输出。

        - **output** (Tensor) - shape为 :math:`(batch\_size, channels, new_height\_size, new_width\_size)` 的Tensor。