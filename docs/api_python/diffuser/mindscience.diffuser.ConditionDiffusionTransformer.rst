...
.. py:class:: mindscience.diffuser.ConditionDiffusionTransformer(in_channels, out_channels, cond_channels, hidden_channels, layers, heads, time_token_cond=True, cond_as_token=True, compute_dtype=mstype.float32)
    ...
    参数：
        ...
        - **compute_dtype** (mindspore.dtype) - 计算数据类型。支持 ``mstype.float32`` or ``mstype.float16`` 。默认 ``mstype.float32`` ，表示 ``mindspore.float32`` 。
