...
.. py:class:: mindscience.sciops.dft.IDFTn(shape, dim=None, norm='backward', modes=None, compute_dtype=mstype.float32)

    1/2/3D 离散复数傅里叶逆变换。结果应与 `scipy.fft.ifftn() <https://docs.scipy.org/doc/scipy/reference/generated/scipy.fft.ifftn.html#scipy.fft.ifftn>`_ 相同。
    ...
    输出：
        - **br** (Tensor) - 输出张量的实部，尾随维度与 `shape` 对齐。
        - **bi** (Tensor) - 输出张量的虚部，尾随维度与 `shape` 对齐。
