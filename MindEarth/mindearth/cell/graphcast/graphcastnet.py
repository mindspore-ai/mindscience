...
class GraphCastNet(nn.Cell):
    ...
    Examples:
        ...
        >>> per_variable_level_mean = Tensor(np.random.rand(20, ).astype(np.float32), ms.float32)
        >>> per_variable_level_std = Tensor(np.random.rand(20, ).astype(np.float32), ms.float32)
        >>> grid_node_feats = Tensor(np.random.rand(32768, 60).astype(np.float32), ms.float32)
        >>> label = Tensor(np.random.rand(32768, 20).astype(np.float32), ms.float32)
        >>> context.set_context(mode=context.GRAPH_MODE, device_target="Ascend", device_id=7)
        >>> net = GraphCastNet(60, 20, 3, 4, 4, 4, 512, 4,
        >>>                           g2m_src_idx, g2m_dst_idx,
        >>>                           m2m_src_idx, m2m_dst_idx,
        >>>                           m2g_src_idx, m2g_dst_idx,
        >>>                           mesh_node_feats, mesh_edge_feats,
        >>>                           g2m_edge_feats, m2g_edge_feats,
        >>>                           per_variable_level_mean, per_variable_level_std)
        >>> out = net(Tensor(grid_node_feats, ms.float32))
        >>> print(out.shape)
        (32768, 20))
