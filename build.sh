...
build_mindscience()
{
  echo "start build MindSpore Science project."
  ...
  mkdir -pv "${BUILD_PATH}/mindscience"
  cd "${BUILD_PATH}/mindscience"
...
write_checksum() {
    cd "$OUTPUT_PATH" || exit
    PACKAGE_LIST=$(ls mindscience*.whl) || exit
...
echo "---------------- MindSpore Science: build start ----------------"
checkopts "$@"
build_mindscience
mv ${BASEPATH}/build/package/*whl ${OUTPUT_PATH}
write_checksum
echo "---------------- MindSpore Science: build end   ----------------"
