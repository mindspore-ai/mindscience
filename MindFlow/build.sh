...
build_mindflow()
{
  echo "start build mindflow project."
  ...
  mkdir -pv "${BUILD_PATH}/mindflow"
  cd "${BUILD_PATH}/mindflow"
...
write_checksum() {
    cd "$OUTPUT_PATH" || exit
    PACKAGE_LIST=$(ls mindflow*.whl) || exit
...
echo "---------------- MindPhysics: build start ----------------"
checkopts "$@"
build_mindflow
mv ${BASEPATH}/build/package/*whl ${OUTPUT_PATH}
write_checksum
echo "---------------- MindPhysics: build end   ----------------"
