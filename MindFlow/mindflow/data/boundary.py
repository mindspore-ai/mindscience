...
from ..geometry.geometry_base import Geometry, SamplingConfig
...
class BoundaryBC(Boundary):
    ...
    def __init__(self, geometry):
        ...
        if not self.sampling_config:
            raise ValueError("BC info for the current geometry: {} should not be None".format(geometry.name))
...
class BoundaryIC(Boundary):
    ...
    def __init__(self, geometry):
        ...
        if not self.sampling_config:
            raise ValueError("IC info for the current geometry: {} should not be None".format(geometry.name))
