ENGLISH | [CHINESE](https://atomgit.com/mindspore-lab/mindscience/blob/master/MindFlow/applications/cfd/acoustic/README_CN.md)
...
- $u(\mathbf{x},t) \;\; [L]$ Deformation displacement (pressure divided by density), scalar
- $c(\mathbf{x}) \;\; [L/T]$ Wave velocity, scalar
- $f(\mathbf{x},t) \;\; [L/T^2]$ Excitation source (volume distribution force), scalar
...
To ensure convergence, preprocessing and reasonable selection of the value of $\epsilon$ are required. Define the preprocessing operator as $\gamma = i/\epsilon \cdot V$, and take $\epsilon \geq \max\{|k^2 - k_0^2|\}$. Multiply both sides of the iteration equation by $\gamma$ and rearrange, we get
...
2. Clone the MindScience repository or obtain the codes directly from [MindFlow/applications/cfd/acoustic](https://atomgit.com/mindspore-lab/mindscience/tree/master/MindFlow/applications/cfd/acoustic);

### Dataset

Download train and test dataset: [cfd/acoustic/dataset](https://download-mindspore.osinfra.cn/mindscience/mindflow/dataset/applications/cfd/acoustic).
...
You can download the running python scripts from [solve_acoustic.py](https://atomgit.com/mindspore-lab/mindscience/blob/master/MindFlow/applications/cfd/acoustic/solve_acoustic.py).
...
`--dim` represents the dimension of space. Default: `2`;
...
You can use the [Chinese version](https://atomgit.com/mindspore-lab/mindscience/blob/master/MindFlow/applications/cfd/acoustic/acoustic_CN.ipynb) and the [English version](https://atomgit.com/mindspore-lab/mindscience/blob/master/MindFlow/applications/cfd/acoustic/acoustic.ipynb) of Jupyter Notebook to run the training and validation code line by line.
