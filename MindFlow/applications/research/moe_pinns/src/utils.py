...
from .model import Burgers1D, NavierStokes2D, NavierStokesRANS, MoeMlp
...
def plot_burgers(model, epochs=1, resolution=100):
    ...
    for i, t_cs in enumerate(t_cross_sections):
        ...
        plt.ylabel('u(t,x)')
    plt.tight_layout()
    plt.savefig(f'images/burgers-{epochs + 1}-result.jpg')
    plt.close()
...
def calculate_burgers_error(label, prediction):
    ...
    l2_error = np.sqrt(
        np.sum(np.square(error[..., 0]))) / (np.sqrt(np.sum(np.square(label[..., 0])))+1e-8)
...
def cal_l2_error(error, label):
    ...
    l2_error_u = np.sqrt(np.sum(np.square(
        error[:, :, 0]), axis=1)) / (np.sqrt(np.sum(np.square(label[:, :, 0]), axis=1)) + 1e-8)
    l2_error_v = np.sqrt(np.sum(np.square(
        error[:, :, 1]), axis=1)) / (np.sqrt(np.sum(np.square(label[:, :, 1]), axis=1)) + 1e-8)
    l2_error_p = np.sqrt(np.sum(np.square(
        error[:, :, 2]), axis=1)) / (np.sqrt(np.sum(np.square(label[:, :, 2]), axis=1)) + 1e-8)
    return l2_error_u, l2_error_v, l2_error_p


def plot_cylinder_flow(model, epochs, input_data, label, path="./videos"):
    ...
    if sample_t < 100:
        ...
        t_set = np.arange(50, dtype=np.int32) * int(sample_t / 50)
    ...
    for t in t_set:
        ...
        for i, data_2d in enumerate(lpe_2d):
            for j, data in enumerate(data_2d):
                ...
                width = axes_size.AxesY(ax, aspect=1 / (aspect + 1e-8))
    ...
    l2_error_total = np.sqrt(np.sum(np.square(error[:, :, :]), axis=(1, 2))) / \
        (np.sqrt(np.sum(np.square(label[:, :, :]), axis=(1, 2))) + 1e-8)
    ...
    plt.savefig(os.path.join(path, "cylinder_TimeError_" + str(epochs) + ".png"))
...
def calculate_cylinder_flow_error(label, prediction):
    ...
    l2_error_u = np.sqrt(
        np.sum(np.square(error[..., 0]))) / (np.sqrt(np.sum(np.square(label[..., 0]))) + 1e-8)
    l2_error_v = np.sqrt(
        np.sum(np.square(error[..., 1]))) / (np.sqrt(np.sum(np.square(label[..., 1]))) + 1e-8)
    l2_error_p = np.sqrt(
        np.sum(np.square(error[..., 2]))) / (np.sqrt(np.sum(np.square(label[..., 2]))) + 1e-8)
    l2_error = np.sqrt(np.sum(np.square(error))) / \
        (np.sqrt(np.sum(np.square(label))) + 1e-8)
...
def get_cylinder_flow_prediction(model, inputs, label_shape, config):
    ...
    index = 0
    while index < inputs.shape[0]:
        index_end = min(index + config['data']["test_batch_size"], inputs.shape[0])
        test_batch = Tensor(inputs[index: index_end, :], mstype.float32)
        prediction[index: index_end, :] = model(test_batch).asnumpy()
        index = index_end
...
def calculate_periodic_hill_error(label, prediction):
    ...
    l2_error_p = np.sqrt(
        np.sum(np.square(error[..., 2]))) / (np.sqrt(np.sum(np.square(label[..., 2]))) + 1e-8)
    l2_error_uu = np.sqrt(
        np.sum(np.square(error[..., 3]))) / (np.sqrt(np.sum(np.square(label[..., 3]))) + 1e-8)
    l2_error_uv = np.sqrt(
        np.sum(np.square(error[..., 4]))) / (np.sqrt(np.sum(np.square(label[..., 4]))) + 1e-8)
    l2_error_vv = np.sqrt(
        np.sum(np.square(error[..., 5]))) / (np.sqrt(np.sum(np.square(label[..., 5]))) + 1e-8)
...
def calculate_periodic_hill_l2_error(model, inputs, label, config):
    ...
    print_log("    l2_error, uu: ", l2_errors.l2_error_uu, ", uv: ", l2_errors.l2_error_uv,
              ", vv: ", l2_errors.l2_error_vv, ", Total: ", l2_errors.l2_error)
    print_log("==================================================================================================")


def plot_periodic_hill(model, epochs, input_data, label, path="./images"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(path, "periodic_" + str(epochs) + ".png"))
    plt.show()
...
def create_dataset(case_name, config):
    ...
    pre_train_dataset = create_dataset_hooks.get(case_name)(config)
    ...
    pre_loss_dataset = create_dataset_hooks.get(case_name)(config)
    if case_name != "periodic_hill":
    ...
    inputs, label = create_test_dataset_hooks.get(case_name)(
...
def create_model(case_name, config):
    ...
    if case_name == "cylinder_flow":
        ...
        input_scale = list(2.0 / (coord_max - coord_min + 1e-8))
    else:
        input_scale = None
        input_center = None
    model = MoeMlp(in_channels=model_config["in_channels"],
...
def plot(case_name, model, epoch, config, inputs, label):
    r"""visualize the result"""
    if case_name == "burgers":
        plot_burgers(model=model, epochs=epoch,
