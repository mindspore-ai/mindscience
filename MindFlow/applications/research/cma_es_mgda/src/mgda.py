...
def gradient_normalizers(grads, losses, normalization_type):
    ...
    if normalization_type == 'l2':
        for t in range(grads_count):
            ...
            t_gn = np.sqrt(t_gn)
            result_grads.append(grads[t] / t_gn)
    elif normalization_type == 'loss':
        for t in range(grads_count):
            result_grads.append(grads[t] / losses[t])
    elif normalization_type == 'loss+':
        for t in range(grads_count):
            t_gn = np.sum(np.power(grads[t], 2))
...
def mgda_step(losses, origin_grads, config, optimizer):
    ...
    norm_type = config["mgda"]["norm_type"]
    norm_grads = gradient_normalizers(numpy_grads, losses, norm_type)
