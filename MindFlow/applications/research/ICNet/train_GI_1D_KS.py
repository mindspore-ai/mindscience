...
def train(model, NIter, lr):
    ...
    for epoch in range(1, 1 + NIter):
        (loss, loss_u, loss_f_u, loss_lambda_u), grads = grad_fn(
            model.x, model.t, model.x_f, model.t_f, model.u)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file_path", type=str, default="./config/ICNet_KS.yaml")
    ...
    model_pretrain = InvarianceConstrainedNN(
        ...
        use_npu,
        msfloat_type)
    for epoch_train, lr_value in zip(epochs, learning_rate):
        train(model_pretrain, int(float(epoch_train)), lr_value)
