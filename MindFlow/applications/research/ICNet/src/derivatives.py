...
class ThirdOrderGrad_1d(nn.Cell):
    """
    Computes and returns the Third order gradients of the specified column of outputs with respect to the specified
    column of inputs.
    """
    def __init__(self, model, input_idx1, input_idx2, output_idx):
        super(ThirdOrderGrad_1d, self).__init__()
        check_mode("SecondOrderGrad")
        if not isinstance(model, nn.Cell) and not isfunction(model):
            raise TypeError("The type of model should be a function or network, but got {}".format(type(model)))
        if isinstance(input_idx1, bool) or not isinstance(input_idx1, int):
        ...
        self.jac1 = _FirstOrderGrad(model, input_idx=input_idx1, output_idx=output_idx)
        self.jac2 = _FirstOrderGrad(self.jac1, input_idx=input_idx2, output_idx=0)
        self.jac3 = _FirstOrderGrad(self.jac2, input_idx=input_idx2, output_idx=0)

    def construct(self, x):
        hes = self.jac3(x)
        return hes

class ForthOrderGrad_1d(nn.Cell):
    """
    Computes and returns the Third order gradients of the specified column of outputs with respect to the specified
    column of inputs.
    """
    def __init__(self, model, input_idx1, input_idx2, output_idx):
        super(ForthOrderGrad_1d, self).__init__()
        check_mode("SecondOrderGrad")
        if not isinstance(model, nn.Cell) and not isfunction(model):
            raise TypeError("The type of model should be a function or network, but got {}".format(type(model)))
        if isinstance(input_idx1, bool) or not isinstance(input_idx1, int):
        ...
        self.jac2 = _FirstOrderGrad(self.jac1, input_idx=input_idx2, output_idx=0)
        self.jac3 = _FirstOrderGrad(self.jac2, input_idx=input_idx2, output_idx=0)
        self.jac4 = _FirstOrderGrad(self.jac3, input_idx=input_idx2, output_idx=0)
