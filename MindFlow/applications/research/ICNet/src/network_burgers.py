...
class InvarianceConstrainedNN:
    ...
    def __init__(self, X, u, v, X_f, layers, batchno, use_npu, msfloat_type):
        ...
        self.lambda_u = Tensor(np.zeros([11, 1]), self.msfloat_type)
        self.lambda_v = Tensor(np.zeros([11, 1]), self.msfloat_type)
        self.lambda_uux = Tensor([0.0], self.msfloat_type)
        self.lambda_vuy = Tensor([0.0], self.msfloat_type)
        self.lambda_uvx = Tensor([0.0], self.msfloat_type)
        self.lambda_vvy = Tensor([0.0], self.msfloat_type)
        ...
        self.lambda_vuy = Parameter(self.lambda_vuy, name="name_vuy", requires_grad=True)
        self.lambda_uvx = Parameter(self.lambda_uvx, name="name_uvx", requires_grad=True)
        self.lambda_vvy = Parameter(self.lambda_vvy, name="name_vvy", requires_grad=True)
    ...
    def net_f(self, x_f, y_f, t_f, N_f):
        ...
        f_u = u_t + self.lambda_uux * u * u_x + self.lambda_vuy * v * u_y
        f_v = v_t + self.lambda_uvx * u * v_x + self.lambda_vvy * v * v_y
...
class InvarianceConstrainedNN_STRdige:
    ...
    # Initialize the class
    def __init__(self, X, u, v, X_f, layers, batchno, lambda_u, lambda_v, lambda_uux,
                 lambda_vuy, lambda_uvx, lambda_vvy, load_params, second_path, msfloat_type):
        ...
        self.lambda_vuy = Tensor(lambda_vuy, self.msfloat_type)
        self.lambda_uvx = Tensor(lambda_uvx, self.msfloat_type)
        self.lambda_vvy = Tensor(lambda_vvy, self.msfloat_type)
    ...
    def net_f(self, x_f, y_f, t_f, N_f):
        ...
        Phi_u = ops.cat((Tensor(np.ones((int(N_f), 1), dtype=np.float32), self.msfloat_type),
                         u_x, u_xx, u_y, u_yy, u_xy, v_x, v_xx, v_y, v_yy, v_xy), 1)
    ...
    def call_trainstridge(self, lam, d_tol):
        """call_trainstridge"""
        U_t_pred, V_t_pred, Phi_u, Phi_v = self.net_f(
