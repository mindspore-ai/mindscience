...
from src.derivatives import ForthOrderGrad_1d, ThirdOrderGrad_1d, SecondOrderGrad, Grad
...
class InvarianceConstrainedNN:
    ...
    def __init__(self, X, u, X_f, layers, batchno, use_npu, msfloat_type):
        ...
        self.lambda_u = Tensor(np.zeros([5, 1]), self.msfloat_type)
        self.lambda_uux = Tensor([0.0], self.msfloat_type)

        self.lambda_u = Parameter(self.lambda_u, name="name_u", requires_grad=True)
        self.lambda_uux = Parameter(self.lambda_uux, name="name_uux", requires_grad=True)

        # Training data
        self.x = Tensor(np.array(X[:, 0:1], np.float32), self.msfloat_type)
        self.t = Tensor(np.array(X[:, 1:2], np.float32), self.msfloat_type)
        ...
        self.t_f = Tensor(np.array(X_f[:, 1:2], np.float32), self.msfloat_type)
        ...
        params.append(self.lambda_uux)

        # The function of Auto-differentiation
        self.grad = Grad(self.dnn)
        self.hessian_u_xx = SecondOrderGrad(self.dnn, 0, 0, output_idx=0)
        self.hessian_u_xxx = ThirdOrderGrad_1d(self.dnn, 0, 0, output_idx=0)
        self.hessian_u_xxxx = ForthOrderGrad_1d(self.dnn, 0, 0, output_idx=0)
    ...
    def net_f(self, x, t, N_f):
        ...
        u = self.net_u(x, t)
        data = ops.concat([x, t], 1)

        u_t = self.grad(data, 1, 0, u)
        u_x = self.grad(data, 0, 0, u)
        u_xx = self.hessian_u_xx(data)
        u_xxx = self.hessian_u_xxx(data)
        u_xxxx = self.hessian_u_xxxx(data)

        derivatives = [Tensor(np.ones((int(N_f), 1), dtype=np.float32), dtype=self.msfloat_type)]
        derivatives.append(u_x)
        derivatives.append(u_xx)
        derivatives.append(u_xxx)
        derivatives.append(u_xxxx)

        f_u = u_t + self.lambda_uux * u * u_x
    ...
    def loss_fn(self, x, t, x_f, t_f, u):
        """loss_fn"""
        u_pred = self.net_u(x, t)
        f_u_pred = self.net_f(x_f, t_f, self.batchsize_f)
...
class InvarianceConstrainedNN_STRdige:
    ...
    # Initialize the class
    def __init__(self, X, u, X_f, layers, batchno, lambda_u, lambda_uux, load_params, second_path, msfloat_type):
        ...
        self.lambda_uux = Tensor(lambda_uux, self.msfloat_type)
    ...
    def net_f(self, x, t, N_f):
        ...
        f_u = u_t + self.lambda_uux * u * u_x
        Phi = ms.ops.cat((Tensor(np.ones((int(N_f), 1), dtype=np.float32), dtype=self.msfloat_type),
                          u_x, u_xx, u_xxx, u_xxxx), 1)

        lib_fun = derivatives
        for i in range(len(lib_fun)):
            f_u = f_u - lib_fun[i]*self.lambda_u[i:i+1, 0:1]

        return f_u, Phi, u_t, u*u_x

    def loss_fn(self, x, t, x_f, t_f, u):
        """loss_fn"""
        u_pred = self.net_u(x, t)
        f_u_pred, _, _ = self.net_f(x_f, t_f, self.batchsize_f)
    ...
    def call_trainstridge(self, lam, d_tol):
        _, Phi_u, u_t, uu_x = self.net_f(self.x_f, self.t_f, self.batchsize_f)

        U_t_pred = u_t + self.lambda_uux * uu_x

        lambda_u2 = self.train_stridge(Phi_u.numpy(), U_t_pred.numpy(), lam, d_tol, maxit=100, STR_iters=10, l0_penalty=None, normalize=2, split=0.8, print_best_tol=False, uv_flag=True)
    ...
    def stridge(self, X0, y, lam, maxit, tol, normalize=2, uv_flag=True):
        ...
        # Get the standard ridge estimate, inherit w from previous training
        if uv_flag:
        ...
        for j in range(maxit):
            ...
            if lam != 0:
                w[biginds] = np.linalg.lstsq(X[:, biginds].T.dot(X[:, biginds]) + lam*np.eye(len(biginds)),
                                             X[:, biginds].T.dot(y))[0]
            else:
