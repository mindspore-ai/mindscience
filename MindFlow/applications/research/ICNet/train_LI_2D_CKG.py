...
from ChenChao.MindSpore.ICNet.ICNet.ICNet_PR.PR_003.src.network_ckg import InvarianceConstrainedNN, InvarianceConstrainedNN_STRdige
from src.datasets_CKG import read_training_data, print_pde
...
def train(model, NIter, lr):
    ...
    params_train.append(model.lambda_vxx)
    params_train.append(model.lambda_vyy)
    ...
    for epoch in range(1, 1 + NIter):
        ...
        if epoch % 10 == 0:
            ...
            print('It: %d, Loss: %.3e, lambda_u1: %.3f, lambda_u3: %.3f, lambda_uv2: %.3f, lambda_uxx: %.3f, lambda_uyy: %.3f,\
                lambda_v1: %.3f, lambda_v3: %.3f, lambda_vu2: %.3f, lambda_vxx: %.3f, lambda_vyy: %.3f, Time: %.2f' %
                  (epoch, loss.item(),
                   model.lambda_u[1].item(), model.lambda_u[3].item(), model.lambda_u[5].item(),
                   model.lambda_uxx.item(), model.lambda_uyy.item(),
                   model.lambda_v[6].item(), model.lambda_v[8].item(), model.lambda_v[9].item(), model.lambda_vxx.item(), model.lambda_vyy.item(), elapsed))
            ...
            lambda_vxx_history_Adam_Pretrain = np.zeros((1, 1))
            lambda_vyy_history_Adam_Pretrain = np.zeros((1, 1))
            ...
            lambda_vxx_new = np.array([model.lambda_vxx.numpy()])
            lambda_vyy_new = np.array([model.lambda_vyy.numpy()])
            ...
            lambda_vxx_history_Adam_Pretrain = np.append(
                lambda_vxx_history_Adam_Pretrain, lambda_vxx_new, axis=1)
            lambda_vyy_history_Adam_Pretrain = np.append(
                lambda_vyy_history_Adam_Pretrain, lambda_vyy_new, axis=1)
    ...
    np.save(
        f'Loss-Coe/{second_path}/lambda_vxx_history_Adam_Pretrain',
        lambda_vxx_history_Adam_Pretrain)
    np.save(
        f'Loss-Coe/{second_path}/lambda_vyy_history_Adam_Pretrain',
        lambda_vyy_history_Adam_Pretrain)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config_file_path", type=str, default="./config/ICNet_CKG.yaml")
    ...
    description_CKG_u = args.description_CKG_u
    description_CKG_v = args.description_CKG_v
    lam = args.lam
    d_tol = args.d_tol
    ...
    for epoch_train, lr_train in zip(epochs, learning_rate):
        ...
        ms.save_checkpoint(model_pretrain.dnn, f'model/model_{lr}.ckpt')

    lambda_uxx_value = model_pretrain.lambda_uxx.numpy()
    lambda_uyy_value = model_pretrain.lambda_uyy.numpy()
    lambda_vxx_value = model_pretrain.lambda_vxx.numpy()
    lambda_vyy_value = model_pretrain.lambda_vyy.numpy()
    ...
    np.save(f'Loss-Coe/{second_path}/lambda_vxx_value', lambda_vxx_value)
    np.save(f'Loss-Coe/{second_path}/lambda_vyy_value', lambda_vyy_value)
    ...
    if load_params:
        ...
        lambda_vxx_value = np.load(f'Loss-Coe/{second_path}/lambda_vxx_value.npy')
        lambda_vyy_value = np.load(f'Loss-Coe/{second_path}/lambda_vyy_value.npy')
        model_ICCO = InvarianceConstrainedNN_STRdige(X_u_train, u_train, v_train, X_f_train, network_size, BatchNo,
                                                     lambda_u_value, lambda_v_value, lambda_uxx_value, lambda_uyy_value, lambda_vxx_value, lambda_vyy_value,
