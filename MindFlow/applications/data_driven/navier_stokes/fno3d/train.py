...
from mindscience.utils import load_yaml_config, print_log, log_config
from mindscience.models import FNO3D
...
def train(input_args):
    ...
    def calculate_l2_error(model, inputs, labels):
        ...
        for i in range(labels.shape[0]):
            ...
            test_batch = test_batch.reshape(1, grid_size,
                                            grid_size, 1, input_timestep).repeat(1, 1, 1, output_timestep, 1)
    ...
    for step in range(1, 1 + optimizer_params["epochs"]):
        ...
        if step % summary_params["test_interval"] == 0:
            ...
            print_log(f"step: {step}, time elapsed: {(time.time() - local_time_beg) * 1000}ms")
            rms_error = calculate_l2_error(model, test_a, test_u)
