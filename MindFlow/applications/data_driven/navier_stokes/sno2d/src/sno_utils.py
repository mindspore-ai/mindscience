...
def get_analysis_data(n_spatial, gauss, polynomial):
    ...
    x, w = gauss(n_spatial) # parameters of quadrature
    poly = []
    d_inv = []
...
def build_matrices(n_modes, analysis_data):
    ...
    """
    a_n = (analysis_data[1] @ analysis_data[0].T * analysis_data[2])[:n_modes, :]
    s_n = analysis_data[0][:, :n_modes] if len(analysis_data) == 4 else analysis_data[-1][:, :n_modes]
    return np.array(a_n), np.array(s_n)
...
def legendre(n, k):
    ...
    return get_transform_data(n, k, gauss, polynomial)
...
def interpolate_2d_dataset(data, poly_type='Chebyshev_t', kind='cubic', alpha=None):
    ...
    res_x = data.shape[1]
    res_y = data.shape[2]
    x = np.linspace(-1., 1.0, res_x)
    y = np.linspace(-1., 1.0, res_y)
    x_p = poly_data[poly_type](res_x, alpha)
    y_p = poly_data[poly_type](res_y, alpha)

    interp_data = np.zeros(data.shape).astype(np.float32)
    for i in range(data.shape[0]):
        f = interp2d(x, y, data[i].flatten(), kind=kind)
        interp_data[i] = f(x_p, y_p).T
    return interp_data
...
def interpolate_1d_dataset(data, poly_type='Chebyshev_t', kind='cubic', alpha=None):
    ...
    x_p = poly_data[poly_type](res, alpha)

    interp_data = np.zeros(data.shape).astype(np.float32)
    for i in range(data.shape[0]):
        f = interp1d(x, data[i], kind=kind)
        interp_data[i] = f(x_p)
