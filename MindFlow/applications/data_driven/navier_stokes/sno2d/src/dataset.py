...
def load_interp_data(data_config, dataset_type='train', kind='cubic'):
    ...
    if dataset_type == 'test':
        # test data has shape (200, 19, 64, 64, 1, 1)
    ...
    inputs = interpolate_2d_dataset(inputs, poly_type, kind)
    labels = interpolate_2d_dataset(labels, poly_type, kind)

    if dataset_type == 'test':
        batch_size = data_config['batch_size']
        test_size = data_config['test_size']
        test_batches = test_size // batch_size
        test_shape = (test_batches, batch_size, 1, resolution, resolution)
        inputs = inputs.reshape(test_shape)
        labels = labels.reshape(test_shape)
...
def create_training_dataset(data_config,
    ...
    ns_2d_data = ExistedDataConfig(name=data_config["name"],
