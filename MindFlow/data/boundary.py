...
from mindscience.data.flow.geometry.geometry_base import Geometry, SamplingConfig
...
class Boundary(Data):
    ...
    def __init__(self, geometry):
        ...
        check_param_type(geometry.sampling_config, geometry.name, data_type=SamplingConfig)
    ...
    def _get_sampling_data(self, geom_type="BC"):
        ...
        logger.info(f"Get {geom_type} sample_data size: {len(sample_data[0])}")
    ...
    def _initialize(self, batch_size=1, shuffle=False, geom_type="BC"):
        ...
        if batch_size > self.data_size:
            raise ValueError(
                f"If prebatch data, batch_size: {batch_size} "
                f"should not be larger than data size: {self.data_size}."
            )
        ...
        logger.info(
            f"Get {geom_type} dataset: {self.name}, columns: {self.columns_list}, "
    ...
    def _get_item(self, index):
        ...
        for i in range(len(self.columns_list)):
            temp_data = (
                self.data[i][self._bound_index[index]]
                if self.batch_size == 1
                else self.data[i][index * self.batch_size : (index + 1) * self.batch_size]
            )
...
class BoundaryBC(Boundary):
    ...
    def __init__(self, geometry):
        ...
        if not self.sampling_config:
            raise ValueError(
                f"BC info for the current geometry: {geometry.name} should not be None"
            )
...
class BoundaryIC(Boundary):
    ...
    def __init__(self, geometry):
        ...
        if not self.sampling_config:
            raise ValueError(
                f"IC info for the current geometry: {geometry.name} should not be None"
            )
