...
class RusanovNet(RiemannComputer):
    r"""
    Rusanov Riemann computer with network. The network is inspired by Rusanov_Net
    from paper "JAX-FLUIDS: A fully-differentiable high-order computational
    ...
    def compute_riemann_flux(self, con_var_left, con_var_right, axis):
        ...
        flux_left = cal_flux(con_var_left, pri_var_left,
                             axis)
        flux_right = cal_flux(con_var_right, pri_var_right,
                              axis)
        ...
        mean_vel = 0.5 * \
            (mnp.abs(pri_var_right[axis + 1]) +
             mnp.abs(pri_var_left[axis + 1]))
        delta_sound_speed = mnp.abs(sound_speed_left - sound_speed_right)

        var = mnp.stack([delta_vel, mean_vel, mean_sound_speed,
                         delta_sound_speed], axis=0)

        var = self.transpose(var, (3, 1, 2, 0))
        net_out = mnp.exp(self.net(var))
        net_out = self.transpose(net_out, (3, 1, 2, 0))

        flux = 0.5 * (flux_left + flux_right) - net_out * \
            (con_var_right - con_var_left)
