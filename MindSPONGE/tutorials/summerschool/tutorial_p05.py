...
mol.fill_water(edge=4.0, pdb_out=out_pdb, template='water.spce.yaml')
if args.e == 'Ascend':
...
neighbours = NeighbourList(mol, cutoff=None, cast_fp16=True)
...
sim = WithEnergyCell(mol, energy, bias=metad, neighbour_list=neighbours)
vgen = VelocityGenerator(300)
velocity = vgen(mol.shape, mol.atom_mass)
opt = UpdaterMD(
    system=mol,
