...
# 单卡; 按照配置文件配置好yaml文件
config_path = 'mindscience/MindSPONGE/applications/model_configs/ProtT5/t5_pretrain.yaml'  # 根据需要改为本地路径
pipe = PipeLine(name = "ProtT5")
conf = load_config(config_path)
pipe.initialize(conf=conf)
...
# 使用多卡并行; run_downstream.py中代码就是下游任务的代码，使用msrun启动
msrun --worker_num=${worknum} --local_worker_num=${worknum} --master_port=8128 --log_dir=msrun_log --join=True --cluster_time_out=600 ./run_downstream.py
```
