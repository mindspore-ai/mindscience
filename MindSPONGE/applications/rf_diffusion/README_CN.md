...
MindSPONGE
  └── applications
        └── rf_diffusion                       # RFdiffusion 在 MindSpore 上的实现
              ├── README.md                    # 中文使用说明（本文件）
              ├── README_en.md                 # 英文使用说明
              ├── run_inference.py             # 推理入口脚本（Hydra 配置驱动）
              ├── requirements.txt             # Python 依赖列表
              ├── config                       # 推理配置（Hydra YAML）
              │     └── inference
              │           ├── base.yaml        # 通用推理配置
              │           ├── antibody.yaml    # 抗体 / 纳米抗体设计配置
              │           └── symmetry.yaml    # 对称寡聚体设计配置
              ├── rfdiffusion                  # 核心模型与推理逻辑
              │     ├── diffusion.py           # 扩散过程定义
              │     ├── RoseTTAFoldModel.py    # 主网络结构
              │     ├── SE3_network.py         # SE(3) 等变网络
              │     ├── contigs.py             # contigmap 解析与处理
              │     ├── igso3.py               # IGSO3 相关预计算
              │     ├── inference              # 推理封装与工具
              │     │     ├── model_runners.py # 推理入口封装
              │     │     ├── symmetry.py      # 对称扩散工具
              │     │     └── ab_pose.py       # 抗体姿态与结构工具
              │     ├── potentials             # 辅助势函数实现
              │     │     ├── manager.py       # 势函数管理
              │     │     └── potentials.py    # 各类势函数定义
              │     └── ...                    # 其他网络与工具模块
              ├── env                          # 第三方依赖（se3_transformer 等）
              │     └── se3_transformer        # SE3 Transformer 代码
              ├── examples                     # 任务级脚本示例
              │     ├── design_unconditional.sh        # 无条件单体生成
              │     ├── design_motifscaffolding.sh     # 基序支架设计
              │     ├── design_ppi.sh                  # PPI 结合体设计
              │     ├── design_cyclic_oligos.sh        # 循环对称寡聚体
              │     ├── design_tetrahedral_oligos.sh   # 四面体对称寡聚体
              │     ├── antibody_pdbdesign.sh          # 抗体接口设计
              │     ├── nanobody_pdbdesign.sh          # 纳米抗体接口设计
              │     └── ...                            # 其他任务示例脚本
              ├── scripts
              │     └── download_models.sh      # 模型权重一键下载脚本
              └── img                           # 文档中使用的示意图
```
