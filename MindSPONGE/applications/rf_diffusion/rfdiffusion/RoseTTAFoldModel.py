...
from .Embeddings import Extra_emb, MSA_emb, Recycling, Templ_emb
from .Track_module import IterativeSimulator


class RoseTTAFoldModule(nn.Cell):
    def __init__(
        self,
        n_extra_block,
        n_main_block,
    ...
    def construct(
        ...
        # Do recycling
        if msa_prev == None:
            msa_prev = ms.mint.zeros_like(msa_latent[:, 0])
        if pair_prev == None:
            pair_prev = ms.mint.zeros_like(pair)
        if state_prev == None:
            state_prev = ms.mint.zeros_like(state)
        ...
        is_frozen_residue = (
            ...
            else ms.mint.zeros_like(motif_mask).bool()
        )
        ...
        if return_infer:
            ...
            nbin = lddt.shape[1]
            bin_step = 1.0 / nbin
            lddt_bins = ms.mint.linspace(bin_step, 1.0, nbin, dtype=lddt.dtype)
            pred_lddt = nn.Softmax(axis=1)(lddt)
            pred_lddt = ms.mint.sum(lddt_bins[None, :, None] * pred_lddt, dim=1)
