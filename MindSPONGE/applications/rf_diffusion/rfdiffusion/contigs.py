...
class ContigMap:
    ...
    and all fragments of receptor chain on the next one (generally B)
    ...
    def __init__(
        ...
        def parse_inpaint(x):
            return "/".join(x).split("/") if x is not None else None
    ...
    def expand_sampled_mask(self):
        ...
        sampled_mask_length_bound.extend(
            map(lambda x: x + inpaint_last_bound, _receptor_mask_length_bound)
        )
        return (
            receptor,
            receptor_hal,
