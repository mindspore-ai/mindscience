...
def th_ang_v(ab, bc, eps: float = 1e-8):
    ...
    cos_angle = ms.mint.clamp((ab * bc).sum(-1), -1, 1)
    sin_angle = ms.mint.sqrt(1 - cos_angle.square() + eps)
    dih = ms.mint.stack((cos_angle, sin_angle), -1)
    return dih


def broadcast_tensors(*tensors):
    target_shape = []
    for dim in zip(*[t.shape for t in tensors]):
        target_shape.append(max(dim))
    target_shape = tuple(target_shape)

    out = [ops.broadcast_to(t, target_shape) for t in tensors]
    return out
...
def th_dih_v(ab, bc, cd):
    def th_cross(a, b):
        ...
        return ms.mint.cross(a, b, dim=-1)
...
def rigid_from_3_points(N, Ca, C, non_ideal=False, eps=1e-8):
    ...
    v1 = C - Ca
    v2 = N - Ca
    e1 = v1 / (ms.mint.norm(v1, dim=-1, keepdim=True) + eps)
    u2 = v2 - (ms.mint.einsum("bli, bli -> bl", e1, v2)[..., None] * e1)
    e2 = u2 / (ms.mint.norm(u2, dim=-1, keepdim=True) + eps)
    e3 = ms.mint.cross(e1, e2, dim=-1)
    R = ms.mint.cat(
        [e1[..., None], e2[..., None], e3[..., None]], dim=-1
    )  # [B,L,3,3] - rotation matrix

    if non_ideal:
        v2 = v2 / (ms.mint.norm(v2, dim=-1, keepdim=True) + eps)
        # cosine of current N-CA-C bond angle
        cosref = ms.mint.sum(e1 * v2, dim=-1)
        costgt = cos_ideal_NCAC.item()
        cos2del = ms.mint.clamp(
            cosref * costgt
            + ms.mint.sqrt((1 - cosref * cosref) * (1 - costgt * costgt) + eps),
            min=-1.0,
            max=1.0,
        )
        cosdel = ms.mint.sqrt(0.5 * (1 + cos2del) + eps)
        sindel = ms.mint.sign(costgt - cosref) * ms.mint.sqrt(
            1 - 0.5 * (1 + cos2del) + eps
        )
        Rp = ms.mint.eye(3).reshape((1, 1, 3, 3)).repeat(B, L, 1, 1)
        Rp[:, :, 0, 0] = cosdel
        Rp[:, :, 0, 1] = -sindel
        Rp[:, :, 1, 0] = sindel
        Rp[:, :, 1, 1] = cosdel

        R = ms.mint.einsum("blij,bljk->blik", R, Rp)
...
def get_tor_mask(seq, torsion_indices, mask_in=None):
    B, L = seq.shape[:2]
    tors_mask = ms.mint.ones((B, L, 10), dtype=ms.bool_)
    ...
    tors_mask[:, :, 9] = ms.mint.logical_and(seq != aa2num["GLY"], seq != aa2num["ALA"])
    tors_mask[:, :, 9] = ms.mint.logical_and(tors_mask[:, :, 9], seq != aa2num["UNK"])
    tors_mask[:, :, 9] = ms.mint.logical_and(tors_mask[:, :, 9], seq != aa2num["MAS"])

    if mask_in != None:
        # mask for missing atoms
        # chis
        ti0 = ms.mint.gather(mask_in, 2, torsion_indices[seq, :, 0])
        ti1 = ms.mint.gather(mask_in, 2, torsion_indices[seq, :, 1])
        ti2 = ms.mint.gather(mask_in, 2, torsion_indices[seq, :, 2])
        ti3 = ms.mint.gather(mask_in, 2, torsion_indices[seq, :, 3])
        is_valid = ms.mint.stack((ti0, ti1, ti2, ti3), dim=-2).all(dim=-1)
        tors_mask[..., 3:7] = ms.mint.logical_and(tors_mask[..., 3:7], is_valid)
        tors_mask[:, :, 7] = ms.mint.logical_and(
            tors_mask[:, :, 7], mask_in[:, :, 4]
        )  # CB exist?
        tors_mask[:, :, 8] = ms.mint.logical_and(
            tors_mask[:, :, 8], mask_in[:, :, 4]
        )  # CB exist?
        tors_mask[:, :, 9] = ms.mint.logical_and(
...
def get_torsions(
    ...
    tors_planar = ms.mint.zeros((B, L, 10), dtype=ms.bool_)
    ...
    Nideal = ms.tensor([-0.5272, 1.3593, 0.000])
    Cideal = ms.tensor([1.5233, 0.000, 0.000])
    xyz[..., 0, :] = ms.mint.einsum("brij,j->bri", Rs, Nideal) + Ts
    xyz[..., 2, :] = ms.mint.einsum("brij,j->bri", Rs, Cideal) + Ts

    torsions = ms.mint.zeros((B, L, 10, 2))
    ...
    # chis
    ti0 = ms.mint.gather(xyz, 2, torsion_indices[seq, :, 0, None].repeat(1, 1, 1, 3))
    ti1 = ms.mint.gather(xyz, 2, torsion_indices[seq, :, 1, None].repeat(1, 1, 1, 3))
    ti2 = ms.mint.gather(xyz, 2, torsion_indices[seq, :, 2, None].repeat(1, 1, 1, 3))
    ti3 = ms.mint.gather(xyz, 2, torsion_indices[seq, :, 3, None].repeat(1, 1, 1, 3))
    ...
    t0 = ref_angles[seq][..., 0, :]
    torsions[:, :, 7, :] = ms.mint.stack(
        (ms.mint.sum(t * t0, dim=-1), t[..., 0] * t0[..., 1] - t[..., 1] * t0[..., 0]),
        dim=-1,
    )
    ...
    NCpp = (
        NCp
        - ms.mint.sum(NCp * NCCA, dim=-1, keepdim=True)
        / ms.mint.sum(NCCA * NCCA, dim=-1, keepdim=True)
        * NCCA
    )
    t = th_ang_v(CB - CA, NCpp)
    t0 = ref_angles[seq][..., 1, :]
    torsions[:, :, 8, :] = ms.mint.stack(
        (ms.mint.sum(t * t0, dim=-1), t[..., 0] * t0[..., 1] - t[..., 1] * t0[..., 0]),
        dim=-1,
    )
    ...
    t0 = ref_angles[seq][..., 2, :]
    torsions[:, :, 9, :] = ms.mint.stack(
        (ms.mint.sum(t * t0, dim=-1), t[..., 0] * t0[..., 1] - t[..., 1] * t0[..., 0]),
        dim=-1,
    )

    mask0 = ms.mint.isnan(torsions[..., 0]).nonzero()
    mask1 = ms.mint.isnan(torsions[..., 1]).nonzero()
...
def get_tips(xyz, seq):
    ...
    xyz_tips = ms.mint.gather(
        xyz, 2, tip_indices[seq][:, :, None, None].expand((-1, -1, -1, 3))
    ).reshape(B, L, 3)
    mask = ~(ms.mint.isnan(xyz_tips[:, :, 0]))
    if ms.mint.isnan(xyz_tips).any():  # replace NaN tip atom with virtual Cb atom
        # three anchor atoms
        N = xyz[:, :, 0]
        Ca = xyz[:, :, 1]
        C = xyz[:, :, 2]
        ...
        xyz_tips = ms.mint.where(ms.mint.isnan(xyz_tips), Cb, xyz_tips)
...
def make_frame(X, Y):
    Xn = X / ms.mint.norm(X)
    Y = Y - ms.mint.dot(Y, Xn) * Xn
    Yn = Y / ms.mint.norm(Y)
    Z = ms.mint.cross(Xn, Yn)
    Zn = Z / ms.mint.norm(Z)

    return ms.mint.stack((Xn, Yn, Zn), dim=-1)


def cross_product_matrix(u):
    B, L = u.shape[:2]
    matrix = ms.mint.zeros((B, L, 3, 3))
    matrix[:, :, 0, 1] = -u[..., 2]
    matrix[:, :, 0, 2] = u[..., 1]
    matrix[:, :, 1, 0] = u[..., 2]
    matrix[:, :, 1, 2] = -u[..., 0]
...
def writepdb(
    ...
    if bfacts is None:
        bfacts = ms.mint.zeros(atomscpu.shape[0])
    if idx_pdb is None:
        idx_pdb = 1 + ms.mint.arange(atomscpu.shape[0])

    Bfacts = ms.mint.clamp(bfacts, 0, 1)
    for i, s in enumerate(scpu):
        ...
        if len(atomscpu.shape) == 2:
            ...
            atms = aa2long[s]
            # his prot hack
            if s == 8 and ms.mint.norm(atomscpu[i, 9, :] - atomscpu[i, 5, :]) < 1.7:
                atms = (
                    " N  ",
                    " CA ",
                    " C  ",
...
torsion_indices = ms.mint.full((22, 4, 4), 0)
torsion_can_flip = ms.mint.full((22, 10), False, dtype=ms.bool_)
for i in range(22):
...
allatom_mask = ms.mint.zeros((22, 27), dtype=ms.bool_)
long2alt = ms.mint.zeros((22, 27), dtype=ms.int64)
for i in range(22):
...
num_bonds = ms.mint.zeros((22, 27, 27), dtype=ms.int64)
for i in range(22):
    ...
    num_bonds[i, ...] = ms.tensor(num_bonds_i)


# LJ/LK scoring parameters
ljlk_parameters = ms.mint.zeros((22, 27, 5), dtype=ms.float32)
lj_correction_parameters = ms.mint.zeros(
    (22, 27, 4), dtype=ms.bool_
)  # donor/acceptor/hpol/disulf
for i in range(22):
    for j, a in enumerate(aa2type[i]):
        if a is not None:
            ljlk_parameters[i, j, :] = ms.tensor(type2ljlk[a])
...
hbtypes = ms.mint.full(
    (22, 27, 3), -1, dtype=ms.int64
)  # (donortype, acceptortype, acchybtype)
hbbaseatoms = ms.mint.full(
    (22, 27, 2), -1, dtype=ms.int64
)  # (B,B0) for acc; (D,-1) for don
hbpolys = ms.mint.zeros(
...
for i in range(HbDonType.NTYPES):
    for j in range(HbAccType.NTYPES):
        ...
        hbpolys[i, j, 0, 1:3] = ms.tensor(xrange)
        hbpolys[i, j, 0, 3:5] = ms.tensor(yrange)
        hbpolys[i, j, 0, 5:] = ms.tensor(coeffs)
        xrange, yrange, coeffs = hbpolytype2coeffs[pahd]
        hbpolys[i, j, 1, 0] = weight
        hbpolys[i, j, 1, 1:3] = ms.tensor(xrange)
        hbpolys[i, j, 1, 3:5] = ms.tensor(yrange)
        hbpolys[i, j, 1, 5:] = ms.tensor(coeffs)
        xrange, yrange, coeffs = hbpolytype2coeffs[pbah]
        hbpolys[i, j, 2, 0] = weight
        hbpolys[i, j, 2, 1:3] = ms.tensor(xrange)
        hbpolys[i, j, 2, 3:5] = ms.tensor(yrange)
        hbpolys[i, j, 2, 5:] = ms.tensor(coeffs)

# kinematic parameters
base_indices = ms.mint.full((22, 27), 0, dtype=ms.int64)
xyzs_in_base_frame = ms.mint.ones((22, 27, 4))
RTs_by_torsion = ms.mint.eye(4).repeat(22, 7, 1, 1)
reference_angles = ms.mint.ones((22, 3, 2))
...
for i in range(22):
    ...
    for name, base, coords in ideal_coords[i]:
        ...
        xyzs_in_base_frame[i, idx, :3] = ms.tensor(coords)

    # omega frame
    RTs_by_torsion[i, 0, :3, :3] = ms.mint.eye(3)
    RTs_by_torsion[i, 0, :3, 3] = ms.mint.zeros(3)

    # phi frame
    RTs_by_torsion[i, 1, :3, :3] = make_frame(
        xyzs_in_base_frame[i, 0, :3] - xyzs_in_base_frame[i, 1, :3],
        ms.tensor([1.0, 0.0, 0.0]),
    )
    ...
    NCpp = NCp - ms.mint.dot(NCp, NCr) / ms.mint.dot(NCr, NCr) * NCr
    reference_angles[i, 1, :] = th_ang_v(CBr - CAr, NCpp)
    reference_angles[i, 2, :] = th_ang_v(CGr, ms.tensor([-1.0, 0.0, 0.0]))
...
def writepdb_multi(
    ...
    if seq_stack.ndim != 2:
        ...
        seq_stack = ms.mint.tile(seq_stack, (T, 1))
    seq_stack = seq_stack
        ...
        for i, s in enumerate(scpu):
            ...
            for j, atm_j in enumerate(atms):
                ...
                if not use_hydrogens and j >= N_HEAVY:
                    break
                if (atm_j is None) or (ms.mint.all(ms.mint.isnan(atomscpu[i, j]))):
                    continue
                chain_id = "A"
