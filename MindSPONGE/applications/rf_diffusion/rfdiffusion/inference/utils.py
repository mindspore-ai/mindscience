...
def get_next_frames(xt, px0, t, diffuser, so3_type, diffusion_mask, noise_scale=1.0):
    ...
    R_0 = scipy_R.from_matrix(R_0.squeeze().numpy()).as_matrix().astype(np.float32)
    R_t = scipy_R.from_matrix(R_t.squeeze().numpy()).as_matrix().astype(np.float32)
    ...
    next_crds = (
        np.einsum(
            "lrij,laj->lrai",
            all_rot_transitions,
            xt[:, :3, :].numpy() - Ca_t.squeeze()[:, None, ...].numpy(),
        )
...
def get_mu_xt_x0(xt, px0, t, beta_schedule, alphabar_schedule, eps=1e-6):
    ...
    a = (
        (ms.mint.sqrt(alphabar_schedule[t_idx - 1] + eps) * beta_schedule[t_idx])
        / (1 - alphabar_schedule[t_idx])
    ) * px0_ca
    b = (
        (
            ms.mint.sqrt(1 - beta_schedule[t_idx] + eps)
...
def get_next_ca(
    ...
    Parameters:
        ...
        diffusion_mask (mindspore.tensor, required): Tensor of bools, True means NOT diffused at this residue, False means diffused
    ...
    sampled_crds = ms.mint.normal(mu, ms.mint.sqrt(sigma * noise_scale))
...
def get_noise_schedule(T, noiseT, noise1, schedule_type):
    ...
    Parameters:
        ...
        noiseT: The initial (t=T) noise scale
...
class Denoise:
    ...
    def get_potential_gradients(self, xyz, diffusion_mask):
        ...
        Inputs:
            ...
            xyz (mindspore.tensor, required): [L,27,3] Coordinates at which the gradient will be computed

        Outputs:

            Ca_grads (mindspore.tensor): [L,3] The gradient at each Ca atom
        """

        if self.potential_manager == None or self.potential_manager.is_empty():
            return ms.mint.zeros((xyz.shape[0], 3))

        def _forward(x):
            pot = self.potential_manager.compute_all_potentials(x)
            return pot

        # current_potential = self.potential_manager.compute_all_potentials(xyz)
        g_xyz = ops.grad(_forward)(xyz)
    ...
    def get_next_pose(
        ...
        Parameters:
            ...
            xt (mindspore.tensor, required): Current coordinates at timestep t

            px0 (mindspore.tensor, required): Prediction of x0

            t (int, required): timestep t

            diffusion_mask (mindspore.tensor, required): Mask for structure diffusion
        ...
        frames_next = ms.from_numpy(frames_next) + ca_deltas[:, None, :]  # translate

        fullatom_next = ms.mint.full_like(xt, float("nan")).unsqueeze(0)
...
def sampler_selector(conf: DictConfig):
    if conf.scaffoldguided.scaffoldguided:
        ...
        if conf.inference.model_runner == "default":
            ...
            sampler = model_runners.ScaffoldedSampler(conf)
        elif conf.inference.model_runner == "AbSampler":
...
def process_target(pdb_path, parse_hetatom=False, center=True):
    ...
    if not center:
        ca_center = 0
    xyz = ms.from_numpy(target_struct["xyz"] - ca_center)
    seq_orig = ms.from_numpy(target_struct["seq"])
    atom_mask = ms.from_numpy(target_struct["mask"])
    seq_len = len(xyz)

    # Make 27 atom representation
    xyz_27 = ms.mint.full((seq_len, 27, 3), np.nan).float()
    xyz_27[:, :14, :] = xyz[:, :14, :]

    mask_27 = ms.mint.full((seq_len, 27), False)
...
class BlockAdjacency:
    ...
    def __init__(self, conf, num_designs):
        ...
        if self.conf.scaffoldguided.scaffold_list is not None:
            ...
            self.scaffold_list = [
                os.path.split(i)[1][:-6]
                for i in glob.glob(f"{self.conf.scaffoldguided.scaffold_dir}/*_ss.ms")
            ]
    ...
    def get_ss_adj(self, item):
        ...
        """
        with open(
            os.path.join(self.scaffold_dir, f'{item.split(".")[0]}_ss.ms'), "rb"
        ) as f:
            ss = pickle.load(f)
        with open(
            os.path.join(self.scaffold_dir, f'{item.split(".")[0]}_adj.ms'), "rb"
        ) as f:
            adj = pickle.load(f)
    ...
    def expand_mask(self, mask, segments):
        ...
        assert ms.mint.sum(ms.tensor(output)) == ms.mint.sum(~mask)
    ...
    def expand_ss(self, ss, adj, mask, expanded_mask):
        ...
        """
        ss_out = ms.mint.ones(expanded_mask.shape[0]) * 3  # set to mask token
        adj_out = ms.mint.full((expanded_mask.shape[0], expanded_mask.shape[0]), 0.0)
        ss_out[expanded_mask] = ss[~mask]
        expanded_mask_2d = ms.mint.full(adj_out.shape, True)
        # mask out loops/insertions, which is ~expanded_mask
        expanded_mask_2d[~expanded_mask, :] = False
        expanded_mask_2d[:, ~expanded_mask] = False

        mask_2d = ms.mint.full(adj.shape, True)
    ...
    def mask_ss_adj(self, ss, adj, expanded_mask):
        ...
        """
        original_mask = ms.mint.clone(expanded_mask)
        if self.ss_mask > 0:
    ...
    def get_scaffold(self):
        ...
        if self.conf.inference.deterministic:
            ms.manual_seed(self.num_completed)
        ...
        mask = ms.mint.where(ss == 2, 1, 0).bool()
        ...
        return ss.shape[0], ms.mint.nn.functional.one_hot(ss.long(), num_classes=4), adj
