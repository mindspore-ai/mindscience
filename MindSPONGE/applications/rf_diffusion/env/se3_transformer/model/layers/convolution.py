...
class RadialProfile(nn.Cell):
    ...
    def __init__(
        ...
        modules = [
            ...
            nn.LayerNorm((mid_dim,), epsilon=1e-5) if use_layer_norm else None,
            nn.ReLU(),
        ...
        self.net = nn.SequentialCell(*[m for m in modules if m is not None])

    def construct(self, features: Tensor) -> Tensor:
        return self.net(features)


class VersatileConvSE3(nn.Cell):
    """
    ...
    def construct(self, features: Tensor, invariant_edge_feats: Tensor, basis: Tensor):
...
class ConvSE3(nn.Cell):
            ...
            for degree_out, channels_out in fiber_out:
                if fiber_in[degree_out]:
                    p = ms.Parameter(
                        ...
                        name="to_kernel_self_" + str(degree_out),
                    )
                    self.to_kernel_self[str(degree_out)] = p
    ...
    def construct(
        ...
        if self.used_fuse_level == ConvSE3FuseLevel.FULL:
            in_features_fused = ms.mint.cat(in_features, dim=-1)
            out = self.conv(
        ...
        for degree_out in self.fiber_out.degrees:
            ...
            if self.pool:
                if isinstance(out, dict):
                    out[str(degree_out)] = sharker.utils.scatter(
                        out[str(degree_out)], dst, dim=0, reduce="sum"
                    )
                else:
                    out = sharker.utils.scatter(out, dst, dim=0, reduce="sum")
        return out
