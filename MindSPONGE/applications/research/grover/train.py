...
# ============================================================================
"""
The GROVER train(finetune) function
"""
import os
import time
import datetime
...
from src.model.models import GROVEREmbedding, GroverFinetuneTask, GroverFinetuneLossBlock
...
def run_finetune():
    ...
    # logger
    config.outputs_dir = os.path.join(config.save_dir, config.data_file_finetune,
                                      datetime.datetime.now().strftime('%Y-%m-%d_time_%H_%M_%S'))
    config.logger = get_logger(config.outputs_dir, config.rank)
    config.logger.save_args(config)

    # dataset
    smiles_path = os.path.join(config.data_path_finetune, config.data_file_finetune + "_train.csv")
    feature_path = os.path.join(config.data_path_finetune, config.data_file_finetune + "_train.npz")
    config.scaler_path = os.path.join(config.save_dir, config.data_file_finetune + "_scaler")
    if not os.path.exists(config.scaler_path):
        os.makedirs(config.scaler_path)
    ...
    for epoch_idx in range(config.epochs):
        for step_idx, data in enumerate(data_loader):
            ...
            loss = network(input_graph, scope, features_batch, targets)
            config.logger.info('id:{}, step_id:{}, loss: {:.6f}'.format(config.rank, step_idx, loss.asnumpy()))
            loss_meter.update(loss.asnumpy())

        time_used = time.time() - t_end
        per_step_time = time_used / config.steps_per_epoch
        config.logger.info('epoch[{}], loss: {:.6f}'
                           'per step time: {:.4f}s'.format(epoch_idx + 1,
...
if __name__ == '__main__':
    ...
    if config.enable_modelarts:
        ...
        config.data_path_finetune = os.path.join(config.data_dir, config.data_file_finetune)
