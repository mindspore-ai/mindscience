...
class MolGraph:
    ...
    def atom_features(self, atom: Chem.rdchem.Atom):
        ...
        """
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, ATOM_FEATURES.get('atomic_num')) + \
                   onek_encoding_unk(atom.GetTotalDegree(), ATOM_FEATURES.get('degree')) + \
                   onek_encoding_unk(atom.GetFormalCharge(), ATOM_FEATURES.get('formal_charge')) + \
                   onek_encoding_unk(int(atom.GetChiralTag()), ATOM_FEATURES.get('chiral_tag')) + \
                   onek_encoding_unk(int(atom.GetTotalNumHs()), ATOM_FEATURES.get('num_Hs')) + \
                   onek_encoding_unk(int(atom.GetHybridization()), ATOM_FEATURES.get('hybridization')) + \
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs):
        ...
        '''
        self.f_atoms = ms.Tensor(f_atoms, dtype=mstype.float32)
        self.f_bonds = ms.Tensor(f_bonds, dtype=mstype.float32)
        self.a2b = ms.Tensor([a2b[a] + [0] * (self.max_num_bonds - len(a2b[a])) for a in range(self.n_atoms)],
                             dtype=mstype.int32)
        self.b2a = ms.Tensor(b2a, dtype=mstype.int32)
        self.b2revb = ms.Tensor(b2revb, dtype=mstype.int32)
        self.b2b = None  # try to avoid computing b2b b/c O(n_atoms^3)
        self.a2a = self.b2a[self.a2b]  # only needed if using atom messages
        self.a_scope = ms.Tensor(self.a_scope, dtype=mstype.int32)
        self.b_scope = ms.Tensor(self.b_scope, dtype=mstype.int32)
        '''
        self.f_atoms = np.array(f_atoms, dtype=np.float32)
        self.f_bonds = np.array(f_bonds, dtype=np.float32)
        self.a2b = np.array([a2b[a] + [0] * (self.max_num_bonds - len(a2b[a])) for a in range(self.n_atoms)],
                            dtype=np.int32)
        self.b2a = np.array(b2a, dtype=np.int32)
        self.b2revb = np.array(b2revb, dtype=np.int32)
        self.b2b = None  # try to avoid computing b2b b/c O(n_atoms^3)
        self.a2a = self.b2a[self.a2b]  # only needed if using atom messages
        self.a_scope = np.array(self.a_scope, dtype=np.int32)
        self.b_scope = np.array(self.b_scope, dtype=np.int32)
    ...
    def get_components(self):
        ...
        """
        return self.f_atoms, self.f_bonds, self.a2b, self.b2a, self.b2revb, self.a2a, self.a_scope, self.b_scope
    ...
    def get_b2b(self):
        ...
        if self.b2b is None:
            ...
            revmask = (b2b != ms.numpy.tile(self.b2revb.expand_dims(1), (1, b2b.shape[1]))).astype(
                ms.int32)  # num_bonds x max_num_bonds
    ...
    def get_a2a(self):
        ...
        :return: A tensor containing the mapping from each bond index to all the incodming bond indices.
        """
        if self.a2a is None:
            # b = a1 --> a2
