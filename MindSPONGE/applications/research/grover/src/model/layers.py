...
from src.util.nn_utils import get_activation_function, AggregateNeighbor
...
class SelfAttention(nn.Cell):
    ...
    def __init__(self, hidden, in_feature, out_feature):
        ...
        self.w1 = Parameter(Tensor((hidden, in_feature), dtype=ms.float32))
        self.w2 = Parameter(Tensor((out_feature, hidden), dtype=ms.float32))
    ...
    def reset_parameters(self):
        ...
        initializer(XavierUniform(), self.w1)
        initializer(XavierUniform(), self.w2)

    def construct(self, inputs):
        """
        Computing function.
        """
        x = ops.matmul(self.w1, inputs.transpose(1, 0))
        x = self.tanh(x)
...
class MPNEncoder(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.input_layer == "fc":
            ...
            self.W_i = nn.Dense(input_dim, self.hidden_size, has_bias=self.bias)
        ...
        self.W_h = nn.Dense(w_h_input_size, self.hidden_size, has_bias=self.bias)
    ...
    def construct(self,
        ...
        inputs = None
        if self.input_layer == 'fc':
            inputs = self.W_i(init_messages)  # num_bonds x hidden_size # f_bond
...
class MTBlock(nn.Cell):
    ...
    def __init__(self,
        ...
        self.W_i = nn.Dense(self.input_dim, self.hidden_size, has_bias=bias)
        self.attn = MultiHeadedAttention(h=num_attn_head,
                                         d_model=self.hidden_size,
                                         dropout=dropout)
        self.W_o = nn.Dense(self.hidden_size * num_attn_head, self.hidden_size, has_bias=bias)
