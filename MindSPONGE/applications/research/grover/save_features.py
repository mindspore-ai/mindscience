...
from src.data.molfeaturegenerator import get_available_features_generators, get_features_generator
...
def generate_and_save_features():
    ...
    mols, _ = load_smiles_labels(args.data_path)
    ...
    for i, feats in tqdm(enumerate(features_map), total=len(mols)):
        ...
        if (i > 0 and (i + 1) % args.save_frequency == 0) or i == len(mols) - 1:
