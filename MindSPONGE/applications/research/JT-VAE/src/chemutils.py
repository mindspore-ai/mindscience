...
def decode_stereo(smiles_2d):
    ...
    chiral_n = [atom.GetIdx() for atom in dec_isomers[0].GetAtoms() if
...
def sanitize(mol):
    try:
        ...
        print('get mol failed: {}'.format(str(e)))
    return mol
...
def build_edges(n_atoms, nei_list, cliques):
...
# pylint: disable=invalid-name
def init_nei_list(cliques, n_atoms):
    nei_list = [[] for i in range(n_atoms)]
    i_iter = list(range(len(cliques)))
    for i in i_iter:
...
def tree_decomp(mol):
    ...
    nei_list = init_nei_list(cliques, n_atoms)
    ...
    cliques, edges = build_edges(n_atoms, nei_list, cliques)
...
def update_att_confs(ctr_bonds, nei_mol, amap, nei_idx, att_confs):
...
def neighbor_singleton(nei_mol, amap, ctr_atoms, nei_idx):
    att_confs = []
    nei_atom = nei_mol.GetAtomWithIdx(0)
...
def enum_attach(ctr_mol, nei_node, amap, singletons):
    ...
    if nei_mol.GetNumBonds() == 0:  # neighbor singleton
        att_confs = neighbor_singleton(nei_mol, amap, ctr_atoms, nei_idx)
        ...
        if ctr_mol.GetNumBonds() > 1:
            att_confs = update_att_confs(ctr_bonds, nei_mol, amap, nei_idx, att_confs)
    return att_confs
