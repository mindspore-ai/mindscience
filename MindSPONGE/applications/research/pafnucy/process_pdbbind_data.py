        ...
        for dataset_name, data in affinity_data.groupby('set'):
            ...
            with h5py.File('%s/%s.hdf' % (path, dataset_name), 'w') as f:
                for _, row in data.iterrows():
                    name = row['pdbid']
                    affinity = row['Kd_Ki']
                    ligand = next(pybel.readfile(
                        'mol2', '%s/%s/%s/%s_ligand.mol2' % (path, ds_path, name, name)))
                    # do not add the hydrogens! they are in the structure and it would reset the charges
                    try:
                        pocket = next(pybel.readfile(
                            'mol2', '%s/%s/%s/%s_pocket.mol2' % (path, ds_path, name, name)))
                        # do not add the hydrogens! they were already added in chimera and it would reset the charges
                    except ValueError:
                        warnings.warn('no pocket for %s, %s (%s set)' %
        ...
        for die_path, _, pdbfile in os.walk(data_path):
            for pfile in pdbfile:
                if "_pocket.pdb" in pfile:
                    ...
                    command = "obabel -i pdb %s -o mol2 -O %s" % (
                        p_real_file, molfile)
                    os.system(command)
...
def ParseandClean(paths):
    ...
    # Parse Molecules
    dataset_path = {'general': 'general-set-except-refined',
                    'refined': 'refined-set', 'core': 'refined-set'}
    transpdb2mol2(paths, dataset_path)
    extractFeature(path=paths, affinity_data=affinity_data,
