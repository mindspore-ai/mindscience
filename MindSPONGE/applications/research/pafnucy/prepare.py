        ...
        for pocket_file in configs.pocket:
            ...
            try:
                pocket = next(pybel.readfile(
                    configs.pocket_format, pocket_file))
            except:
...
def prepare(configs):
    ...
    if configs.verbose:
        print('%s ligands and %s pockets to prepare:' %
    ...
    with h5py.File(configs.output, configs.mode) as f:
        pocket_generator = get_pocket(
            configs, num_pockets, featurizer=featurizer, num_ligands=num_ligands)
        for ligand_file in configs.ligand:
            ...
            try:
                ligand = next(pybel.readfile(
                    configs.ligand_format, ligand_file))
            except:
    ...
    if configs.verbose:
        print('\n\ncreated %s with %s structures' %
              (configs.output, num_ligands))
