...
The RASP model and FAAST pipeline are developed based on MindSpore + Ascend AI software and hardware platform, and can be easily used on GPU and with Google Colab [FAAST_DEMO](https://colab.research.google.com/drive/1uaki0Ui1Y_gqVW7KSo838aOhXHSM3PTe?usp=sharing). They not only allow for the leveraging of experimental restraints to improve model prediction, but can also facilitate and expedite experimental data analysis with their integrated capabilities.
