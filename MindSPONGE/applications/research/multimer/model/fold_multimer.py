...
class MegaFoldMultimer(nn.Cell):
    """MegaFoldMultimer"""

    def __init__(self, config, mixed_precision):
        super(MegaFoldMultimer, self).__init__()
        ...
        self.num_extra_msa = self.cfg.multimer.embeddings_and_evoformer.num_extra_msa
        ...
        self.pi = np.pi
        self.batch_block = 4
        self.preprocess_1d = nn.Dense(21, self.cfg.msa_channel,
        ...
        self.left_single = nn.Dense(21, self.cfg.pair_channel,
                                    21)
        self.right_single = nn.Dense(21, self.cfg.pair_channel,
                                     weight_init=lecun_init(21))
        ...
        if self.use_chain_relative:
            self.rel_pos_one_hot = nn.OneHot(depth=self.cfg.max_relative_feature * 2 + 2, axis=-1)
            self.rel_chain_one_hot = nn.OneHot(depth=self.max_relative_chain * 2 + 2, axis=-1)
            self.position_activations = nn.Dense(self.cfg.multimer.pair_in_dim, self.cfg.pair_channel,
