...
class PositionalEncodings(nn.Cell):
    ...
    def construct(self, offset, mask):
        d = ops.clip_by_value(offset + self.max_relative_feature, ms.Tensor(0),\
            ms.Tensor(2 * self.max_relative_feature)) * mask + (1 - mask) * (2 * self.max_relative_feature + 1)
...
class ProteinMPNN(nn.Cell):
    ...
    def __init__(self, num_letters, node_features, edge_features,
        ...
        self.w_e = nn.Dense(edge_features, hidden_dim, weight_init=XavierUniform(), has_bias=True)
        ...
        self.w_out = nn.Dense(hidden_dim, num_letters, weight_init=XavierUniform(), has_bias=True)
            ...
            if (chain_mask_gathered == 0).all():
                ...
                if pssm_bias_flag:
                    ...
                    pssm_bias_gathered = ops.GatherD()(pssm_bias, 1, \
                                    ms.numpy.tile(t[:, None, None], (1, 1, pssm_bias.shape[-1])))[:, 0]
