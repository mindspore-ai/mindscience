...
def gather_edges(edges, neighbor_idx):
    ...
    neighbors = ops.broadcast_to(ops.expand_dims(neighbor_idx, -1), (-1, -1, -1, edges.shape[-1]))
...
def gather_nodes(nodes, neighbor_idx):
    ...
    neighbors_flat = ops.broadcast_to(ops.expand_dims(neighbors_flat, -1), (-1, -1, nodes.shape[2]))
    # Gather and re-pack
...
def gather_nodes_t(nodes, neighbor_idx):
    ...
    idx_flat = ops.broadcast_to(ops.expand_dims(neighbor_idx, -1), (-1, -1, nodes.shape[2]))
...
class EncLayer(nn.Cell):
    ...
    def construct(self, h_v, h_e, e_idx, mask_v=None, mask_attend=None):
        ...
        h_v_expand = ops.broadcast_to(ops.expand_dims(h_v, -2), (-1, -1, h_ev.shape[-2], -1))
...
class DecLayer(nn.Cell):
    ...
    def construct(self, h_v, h_e, mask_v=None, mask_attend=None):
        ...
        h_v_expand = ops.broadcast_to(ops.expand_dims(h_v, -2), (-1, -1, h_e.shape[-2], -1))
...
class PositionalEncodings(nn.Cell):
    ...
    def construct(self, offset, mask):
        ...
        d_onehot = ms.Tensor(nn.OneHot(depth=2 * self.max_relative_feature + 1 + 1)(d).asnumpy())
        e = ops.Cast()(self.linear(d_onehot), ms.float32)
        return e
...
class ProteinFeatures(nn.Cell):
    ...
    def construct(self, x, mask, residue_idx, chain_labels):
        ...
        if self.augment_eps > 0:
            x = x + self.augment_eps * ops.UniformInt(x)
        ...
        d_chains = ops.Cast()(((chain_labels[:, :, None] - chain_labels[:, None,\
                                                    :]) == 0), ms.int64)  # find self vs non-self interaction
        e_chains = gather_edges(d_chains[:, :, :, None], e_idx)[:, :, :, 0]
        e_positional = self.embeddings(ops.Cast()(offset, ms.int64), e_chains)
        e = ops.Concat(axis=-1)((ms.Tensor(e_positional.asnumpy()), ms.Tensor(rbf_all.asnumpy())))
    ...
    def _dist(self, x, mask, eps=1E-6):
        ...
        d_neighbors, e_idx = d_neighbors[:, :, 0:int(ms.numpy.minimum(self.top_k, x.shape[1]))], e_idx[:, :, 0:int(
            ms.numpy.minimum(self.top_k, x.shape[1]))]
...
class ProteinMPNN(nn.Cell):
    ...
    def construct(self, x, s, mask, chain_m, residue_idx, chain_encoding_all, randn, use_input_decoding_order=False,
                  decoding_order=None):
    ...
    def sample(self, x, randn, s_true, chain_mask, chain_encoding_all, residue_idx, mask=None, temperature=1.0,
        ...
        s = ops.Zeros()((n_batch, n_nodes), ms.int64)
        ...
        for t_ in range(n_nodes):
            ...
            s_t = ops.Cast()((s_t * chain_mask_gathered + s_true_gathered * (1.0 - chain_mask_gathered)), ms.int64)
            temp1 = self.w_s(s_t)
