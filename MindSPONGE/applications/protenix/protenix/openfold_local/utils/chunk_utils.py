...
"""Chunk utilities for efficient batch processing."""
...
def _fetch_dims(tree):
    return [v for k, v in optree.tree_map(lambda x: x.shape, tree).items()]
...
def _chunk_slice(
    ...
    return mint.cat([s.view((-1,) + t.shape[no_batch_dims:]) for s in sliced_tensors])
...
def chunk_layer(
    ...
    if out is not None:
        def reshape_fn(t):
            return t.view(tuple([-1] + list(t.shape[no_batch_dims:])))
        prepped_outputs = tensor_tree_map(reshape_fn, out)
    ...
    for _ in range(no_chunks):
        ...
        if out_type is dict:
            _assign_dict_chunk(out, output_chunk, i, chunk_size, add_into_out=add_into_out)
            ...
            for x1, x2 in zip(out, output_chunk):
                if add_into_out:
                    ...
                    x1[i: i + chunk_size] = x2
        elif isinstance(output_chunk, ms.Tensor):
            if add_into_out:
...
def _assign_dict_chunk(d1, d2, start_idx, c_size, add_into_out):
    """Assign a dictionary chunk to the output dictionary."""
    for k, v in d1.items():
        if isinstance(v, dict):
            _assign_dict_chunk(v, d2[k], start_idx, c_size, add_into_out)
        else:
            if add_into_out:
                v[start_idx: start_idx + c_size] += d2[k]
            else:
                v[start_idx: start_idx + c_size] = d2[k]

class ChunkSizeTuner:
    """Tunes chunk size for efficient batch processing."""
    ...
    def _determine_favorable_chunk_size(self, fn, args, min_chunk_size):
        """Determine favorable chunk size."""
        ...
        def test_chunk_size(chunk_size):
            try:
                with ms._no_grad():  # pylint: disable=protected-access
    ...
    def _compare_arg_caches(self, ac1, ac2):
        """Compare argument caches."""
        consistent = True
        for a1, a2 in zip(ac1, ac2):
            if not isinstance(ac1, type(ac2)):
                raise TypeError(f"ac1 and ac2 must have the same type, but got {type(ac1)} and {type(ac2)}")
    ...
    def tune_chunk_size(
        ...
        """Tune the chunk size for optimal performance."""
        consistent = True

        def remove_tensors(a):
            return a.shape if isinstance(a, ms.Tensor) else a

        arg_data = tree_map(remove_tensors, args, object)
        if self.cached_arg_data is not None:
            # If args have changed shape/value, we need to re-tune
            if len(self.cached_arg_data) != len(arg_data):
                raise ValueError("len(self.cached_arg_data) must be equal to len(arg_data)")
