...
def convert_stockholm_to_a3m(
    ...
    fasta_chunks = (
        ...
        for k, v in a3m_sequences.items()
    )
...
def _keep_line(line: str, seqnames: Set[str]) -> bool:
    ...
    if line.startswith("#"):  # Other markup - filter out
        return False
...
def truncate_stockholm_msa(stockholm_msa_path: str, max_sequences: int) -> str:
    ...
    with open(stockholm_msa_path, encoding='utf-8') as f:
        for line in f:
            if line.strip() and not line.startswith(("#", "//")):
                # Ignore blank lines, markup and end symbols - remainder are alignment
                # sequence parts.
...
def _parse_hhr_hit(detailed_lines: Sequence[str]) -> TemplateHit:
    ...
    for line in detailed_lines[3:]:
        ...
        if (
            ...
            if length_block != len(delta_query):
                raise ValueError("length_block must be equal to len(delta_query)")
            ...
            if (
                ...
                if length_block != len(delta_hit_sequence):
                    raise ValueError("length_block must be equal to len(delta_hit_sequence)")
