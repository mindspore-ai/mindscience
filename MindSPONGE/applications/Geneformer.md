...
part1: 使用的数据集为Genecorpus-30M数据集，数据集下载路径为 https://huggingface.co/datasets/ctheodoris/Genecorpus-30M/tree/main/example_input_files/gene_classification/dosage_sensitive_tfs
下载gc-30M_sample50k.dataset和dosage_sensitivity_TFs.pickle到Geneformer文件夹下

part2: 下载**gc104M.pkl文件并拷贝到src目录下，下载路径为 https://huggingface.co/ctheodoris/Geneformer/tree/main/geneformer 。

part3: 下载模型原始训练文件pytorch_model.bin到Geneformer文件夹下，获取路径为 https://huggingface.co/ctheodoris/Geneformer/tree/pr146_branch/fine_tuned_models/geneformer-6L-30M_CellClassifier_cardiomyopathies_220224
...
```shell
cd scripts && bash run_8p.sh  #使用多卡需在configs文件中配置rank_table.json

cd scripts && bash run.sh  #单卡使用
```
