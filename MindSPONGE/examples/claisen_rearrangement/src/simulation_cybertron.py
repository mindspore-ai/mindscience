...
class SimulationCybertron(nn.Cell):
    ...
    def main_print(self, *args):
        ...
        steps, temperature, total_potential_energy, _, _, _, _, _, _, _ = list(args)
        if steps == 0:
            print("_steps_ _TEMP_ _TOT_POT_ENE_ _CVariable_ _Bias_Potential_")
        ...
        print("{:>4.0f} {:>8.3f} {:>10.3f} {:>12.3f} {:>13.3f}"
              "".format(steps, float(temperature), float(total_potential_energy), cv.asnumpy(), biasp.asnumpy()))

        if self.file is not None:
            self.file.write("{:>4.0f} {:>8.3f} {:>10.3f} {:>12.3f} {:>13.3f}\n"
                            "".format(steps, float(temperature), float(total_potential_energy), cv.asnumpy(),
                                      biasp.asnumpy()))
