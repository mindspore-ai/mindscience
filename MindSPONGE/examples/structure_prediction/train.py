...
parser.add_argument('--extra_msa_num', type=int, default=1, help='extra msa number')
parser.add_argument('--evo_num', type=int, default=1, help='evo number')
...
parser.add_argument('--train', type=bool, default=True, help='train or eval mode')
parser.add_argument('--run_distribute', type=bool, default=False, help='run distribute')
args_opt = parser.parse_args()

if __name__ == "__main__":
    context.set_context(mode=context.GRAPH_MODE, device_target="Ascend", variable_memory_max_size="29GB",
                        device_id=int(os.getenv('DEVICE_ID')), save_graphs=True, save_graphs_path="./graphs/")
    ...
    fold_net = AlphaFold(model_config, global_config, args_opt.extra_msa_num, args_opt.evo_num, is_training=True)
    net_with_criterion = WithLossCell(fold_net, model_config)
    ...
    train_dataset = create_dataset(args_opt.pdb_data_dir, args_opt.raw_feature_dir, names_list, model_config,
                                   args_opt.resolution_data, num_parallel_worker=1, is_parallel=args_opt.run_distribute,
                                   shuffle=True)
    ...
    for d in dataset_iter:
        ...
        #     final_atom_positions, final_atom_mask, predicted_lddt_logits, prev_pos, prev_msa_first_row, prev_pair = train_net(*inputs_feat, prev_pos, prev_msa_first_row, prev_pair, *ground_truth)
        # step += 1
        ...
        for recycle in range(max_recycle):
            inputs_feat = [Tensor(feat[recycle]) for feat in inputs_feats]
            _, _, _, prev_pos, prev_msa_first_row, prev_pair = train_net(*inputs_feat, prev_pos, prev_msa_first_row,
