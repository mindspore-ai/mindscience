...
def summarize_clusters(protein):
    ...
    msa_sum = csum(mask[:, :, None] * one_hot(23, protein['extra_msa']))
...
def random_crop_to_size(protein, crop_size, max_templates, shape_schema,
                        subsample_templates=False, seed=seed_maker()):
...
def block_delete_msa(protein, config):
  ...
  """
    num_seq = protein['msa'].shape[0]
    block_num_seq = np.floor(num_seq * config.msa_fraction_per_block).astype(np.int32)

    if config.randomize_num_blocks:
        nb = int(np.random.uniform(0, config.num_blocks + 1))
    else:
