...
parser.add_argument('--evo_num', type=int, default=4, help='evo number')
parser.add_argument('--device_id', default=48, type=int, help='Device id to be used.')
...
if __name__ == "__main__":
    ...
    model_config.data.eval.crop_size = args.seq_len
    global_config = global_config.global_config(args.seq_len)
    extra_msa_length = global_config.extra_msa_length
    fold_net = AlphaFold(model_config, global_config, args.extra_msa_num, args.evo_num, is_training=False)
    fold_net.set_train(False)
    fold_net.add_flags_recursive(train_backward=False)
    ...
    for seq_file in seq_files:
        ...
        ground_truth = []
        for i in range(25):
            ground_truth.append(Tensor(np.ones(1, ).astype(np.float32)))
