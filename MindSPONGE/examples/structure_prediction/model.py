...
class AlphaFold(nn.Cell):
    ...
    def __init__(self, config, global_config, extra_num, evo_num, is_training):
        ...
        self.template_embedding = TemplateEmbedding(self.config.template, global_config.template_embedding.slice_num,
                                                    global_config=global_config, is_training=is_training)
        ...
        if is_training:
            ...
            for _ in range(self.extra_msa_stack_num_block):
                msa_stack_block = EvoformerIteration(self.config.evoformer,
                                                     msa_act_dim=64,
                                                     pair_act_dim=128,
                                                     is_extra_msa=True,
                                                     batch_size=None,
    ...
    def _init_tensor(self, global_config):
        ...
        self.evoformer_num_block_eval = Tensor(self.evoformer_num_block, mstype.int32)
    ...
    def construct(self, target_feat, msa_feat, msa_mask, seq_mask, aatype,
        ...
        if self.training:
            ...
            self.idx_extra_msa_stack, idx_extra_msa_stack_int = 0, 0
            while idx_extra_msa_stack_int < self.extra_msa_stack_num_block:
                msa_act, pair_act = \
                    self.extra_msa_stack_iteration(msa_act, pair_act, extra_msa_mask, extra_msa_norm, mask_2d,
        ...
        if self.template_enabled and self.template_embed_torsion_angles:
            ...
            msa_activations1 = mnp.concatenate([msa_activations1, template_activations], axis=0)
        ...
        msa_activations2 = msa_activations1.astype(mstype.float16)
        ...
        if self.training:
            ...
            while self.idx_evoformer_block < self.evoformer_num_block_eval:
                msa_activations2, pair_activations = self.evoformer_iteration(msa_activations2,
                                                                              pair_activations,
                                                                              msa_mask,
                                                                              msa_mask_norm,
                                                                              mask_2d,
        ...
        final_atom_mask = final_atom_mask.astype(mstype.float32)
        if self.training:
