...
.. py:function:: mindsponge.common.get_pdb_info(pdb_path)
    ...
    返回：
        ...
        - **aatype** (numpy.array) 蛋白质一级序列编码，编码方式参考 `mindsponge.common.residue_constants.restype_order`， 取值范围 :math:`[0,20]` ，若为20表示该氨基酸为unkown（`UNK`）。 shape :math:`(N_{res}, )` 。
        ...
        - **atom14_gt_exists** (numpy.array) 按照稠密编码方式编码，蛋白质全原子掩码，默认与 `atom14_atom_exists` 一致。shape :math:`(N_{res}, 14)` 。
