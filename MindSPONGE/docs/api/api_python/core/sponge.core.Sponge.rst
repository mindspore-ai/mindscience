...
.. py:class:: sponge.core.Sponge(network: Union[ :class:`sponge.system.molecule.Molecule`, :class:`sponge.simulation.WithEnergyCell`, :class:`sponge.simulation.RunOneStepCell], potential: :class:`sponge.potential.PotentialCell` = None, optimizer: :class:`mindspore.nn.optim.Optimizer` = None, metrics: dict = None, analysis: :class:`sponge.core.AnalysisCell` = None, **kwargs)

    MindSPONGE 的核心引擎，用于模拟和分析。

    这个 Cell 是分子系统（ :class:`sponge.system.molecule.Molecule` ），势能（ :class:`sponge.potential.PotentialCell` ）和优化器（ :class:`mindspore.nn.optim.Optimizer` ）在 MindSPONGE 中的三个模块的顶级封装。

    有三种方式来封装这些模块：

    1. 直接将 `system`、 `potential` 和 `optimizer` 封装到 :class:`sponge.core.Sponge` 中。

    2. 先将 `system` 和 `potential` 用 :class:`sponge.simulation.WithEnergyCell` 封装，然后将 :class:`sponge.simulation.WithEnergyCell` 和 `optimizer` 一起封装到 :class:`sponge.core.Sponge` 中。在这种情况下，可以通过调整 :class:`sponge.simulation.WithEnergyCell` 来实现对势能的调整，例如设置 `neighbour_list` 和 :class:`sponge.simulation.WithEnergyCell` 中的 `bias`。

    3. 先将 `system` 和 `potential` 用 :class:`sponge.simulation.WithEnergyCell` 封装，然后将 :class:`sponge.simulation.WithEnergyCell` 和 `optimizer` 一起封装到 :class:`sponge.simulation.RunOneStepCell` 中，最后将 :class:`sponge.simulation.RunOneStepCell` 传递到 :class:`sponge.core.Sponge` 中。在这种情况下，可以通过调整 :class:`sponge.simulation.RunOneStepCell` 来实现对力的调整，例如在 :class:`sponge.simulation.RunOneStepCell` 中添加 :class:`sponge.simulation.WithForceCell`。
    ...
    参数:
        - **network** (Union[ :class:`sponge.system.molecule.Molecule`, :class:`sponge.simulation.WithEnergyCell`, :class:`sponge.simulation.RunOneStepCell`]) - 模拟系统的 Cell。
        - **potential** ( :class:`sponge.potential.PotentialCell`, 可选) - 势能。默认值: ``None``。
        - **optimizer** ( :class:`mindspore.nn.optim.Optimizer`, 可选) - 优化器。默认值: ``None``。
        - **metrics** (dict, 可选) - 用于系统分析的metrics字典。字典的键类型应为 `str`，值类型应为 :class:`sponge.metrics.Metric` 或 :class:`sponge.colvar.Colvar`。默认值: ``None``。
        - **analysis** (:class:`sponge.analysis.AnalysisCell`, 可选) - 分析网络。默认值: ``None``。
    ...
    .. py:method:: run(steps: int, callbacks: Union[Callback, List[Callback]] = None, dataset: Dataset = None, show_time: bool = True)
        ...
        参数：
            - **steps** (int) - 步骤数。
            - **callbacks** (Union[Callback, List[Callback]]) - 获取模拟系统信息的回调函数。默认值: ``None``。
    ...
    .. py:method:: analyse(dataset=None, callbacks=None)
        ...
        .. note::
            要使用此API，必须在 :class:`spogne.core.Sponge` 初始化时设置 `metrics`。

        参数：
            - **dataset** (:class:`mindspore.dataset.engine.datasets.Dataset`) -
              要分析的模拟数据集。默认值： ``None``。
            - **callbacks** (Union[:class:`mindspore.train.callback.Callback`, List[:class:`mindspore.train.callback.Callback`]]) -
              训练期间应执行的回调对象列表。默认值： ``None``。
