...
.. py:class:: mindsponge.system.Residue(atom_name, atom_type, atom_mass, atom_charge, atomic_number, bond, head_atom, tail_atom, start_index=0, name='MOL', template)
    ...
    参数：
        ...
        - **bond** (Tensor) - 边序号。
        - **head_atom** (int) - 与前一个残基相连接的头原子的索引。
        - **tail_atom** (int) - 与下一个残基相连的尾原子的索引。
    ...
    .. py:method:: add_atom(atom_name, atom_type, atom_mass, atom_charge, atomic_number)

        设定原子。

        参数：
            - **atom_name** (Union[numpy.ndarray, list(str)]) - 原子名称。
            - **atom_type** (Union[numpy.ndarray, list(str)]) - 原子种类。
            - **atom_mass** (Tensor) - 原子质量。
