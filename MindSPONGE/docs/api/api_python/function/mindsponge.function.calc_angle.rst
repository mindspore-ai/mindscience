...
.. py:function:: mindsponge.function.calc_angle(position_a, position_b: Tensor, position_c: Tensor, pbc_box: Tensor = None)
    ...
    参数：
        - **position_a** (Tensor) - 位置a，shape为 :math:`(B, ..., D)` 。
        - **position_b** (Tensor) - 位置b，shape为 :math:`(B, ..., D)` 。
        - **position_c** (Tensor) - 位置c，shape为 :math:`(B, ..., D)` 。
        - **pbc_box** (Tensor) - PBC box，shape为 :math:`(B, D)` 。默认值：None。

    输出：
        Tensor。计算所得角，取值范围为 :math:`(0, \pi)` 。shape为 :math:`(B, ..., 1)` 。
