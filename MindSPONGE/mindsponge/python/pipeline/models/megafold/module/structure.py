...
from ....cell import InvariantPointAttention
from .....common import residue_constants
from ....cell.initializer import lecun_init
from .....common.utils import torsion_angles_to_frames,\
...
class MultiRigidSidechain(nn.Cell):
    ...
    def __init__(self, config, single_repr_dim):
        ...
        self.input_projection = nn.Dense(single_repr_dim, num_channel,
                                         weight_init=lecun_init(single_repr_dim))
        self.input_projection_1 = nn.Dense(single_repr_dim, num_channel,
                                           weight_init=lecun_init(single_repr_dim))
        self.relu = nn.ReLU()
        self.resblock1 = nn.Dense(num_channel, num_channel,
                                  weight_init=lecun_init(num_channel,
                                                         initializer_name='relu'))
        self.resblock2 = nn.Dense(num_channel, num_channel, weight_init='zeros')
        self.resblock1_1 = nn.Dense(num_channel, num_channel,
                                    weight_init=lecun_init(num_channel, initializer_name='relu'))
        self.resblock2_1 = nn.Dense(num_channel, num_channel, weight_init='zeros')
        self.unnormalized_angles = nn.Dense(num_channel, 14,
                                            weight_init=lecun_init(num_channel))
...
class FoldIteration(nn.Cell):
    ...
    def __init__(self, config, pair_dim, single_repr_dim):
        ...
        self.transition = nn.Dense(num_channel, config.num_channel,
                                   weight_init=lecun_init(num_channel, initializer_name='relu'))
        self.transition_1 = nn.Dense(num_channel, num_channel,
                                     weight_init=lecun_init(num_channel, initializer_name='relu'))
        self.transition_2 = nn.Dense(num_channel, num_channel, weight_init='zeros')
        self.relu = nn.ReLU()
        self.affine_update = nn.Dense(num_channel, 6, weight_init='zeros')
    ...
    def construct(self, act, static_feat_2d, sequence_mask, quaternion, rotation, translation,
                  initial_act, aatype):
        """construct"""
        ...
        # This block corresponds to Jumper et al. (2021) Alg. 23 "Backbone update"
        # Affine update
        affine_update = self.affine_update(act)
        quaternion, rotation, translation = pre_compose(quaternion, rotation, translation,
                                                        affine_update)
...
class StructureModule(nn.Cell):
    ...
    def __init__(self, config, single_repr_dim, pair_dim, seq_length):
    ...
    def construct(self, single, pair, seq_mask, aatype, residx_atom37_to_atom14=None,
        ...
        atom_pos, affine_output_new, angles_sin_cos_new, um_angles_sin_cos_new,\
            sidechain_frames, act_iter = \
            self.iteration_operation(act, act_2d, sequence_mask, quaternion,
                                     rotation, translation, initial_act, aatype)
        ...
        res = (final_atom_positions, final_atom_mask, rp_structure_module, atom14_pred_positions,\
               final_affines, angles_sin_cos_new, um_angles_sin_cos_new, sidechain_frames, \
                sidechain_atom_pos, structure_traj)
        return res

    def iteration_operation(self, act, act_2d, sequence_mask, quaternion, rotation, translation,
                            initial_act, aatype):
        """iteration_operation"""
        affine_init = ()
        angles_sin_cos_init = ()
        ...
        for _ in range(self.num_layer):
            act, quaternion, translation, rotation, affine_output, angles_sin_cos, \
            unnormalized_angles_sin_cos, atom_pos, frames = \
                self.fold_iteration(act, act_2d, sequence_mask, quaternion, rotation,
                                    translation, initial_act, aatype)
            ...
            atom_pos_batch += (mnp.concatenate(vecs_expand_dims(atom_pos, 0),
                                               axis=0)[:, None, ...],)
            frames_batch += (mnp.concatenate(rots_expand_dims(frames[0], 0) +
                                             vecs_expand_dims(frames[1], 0),
                                             axis=0)[:, None, ...],)
        ...
        res = (atom_pos_new, affine_output_new, angles_sin_cos_new, \
               um_angles_sin_cos_new, frames_new, act)
        return res
