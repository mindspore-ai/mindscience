...
# ============================================================================
'''dihedral 14 ljcf force with atom energy with virial'''
...
def dihedral_14_ljcf_force_with_atom_energy_with_virial(atom_numbers, uint_crd, lj_type, charge, boxlength, a_14, b_14,
    ...
    Outputs:
        ...
        atom_virial (Tensor, float32): [N,], the accumulated potential virial for each atom.
    ...
    atom_virial = get_zero_tensor((atom_numbers,))

    frc_mul = np.sum(-frc_abs_3 * crd_scaled, axis=1)

    atom_energy = ops.tensor_scatter_add(atom_energy, a_14_expended, ene_lin_sum)
    atom_virial = ops.tensor_scatter_add(atom_virial, a_14_expended, frc_mul)
