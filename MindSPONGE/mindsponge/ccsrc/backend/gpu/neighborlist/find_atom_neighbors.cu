...
static __global__ void Find_Atom_Neighbors(const int atom_numbers, int max_neighbor_numbers,
                                           int max_atom_in_grid_numbers, int kGridSize,
                                           const float *cutoff_skin_square, const int *atom_in_grid_serial,
                                           const VECTOR *crd, const float *box_length, const int *gpointer,
                                           const int *bucket, const int *atom_numbers_in_grid_bucket,
...
extern "C" int FindAtomNeighbors(int nparam, void **params, int *ndims, int64_t **shapes,
  ...
  Find_Atom_Neighbors<<<ceilf(static_cast<float>(atom_number) / 32), 32, 0, reinterpret_cast<cudaStream_t>(stream)>>>(
    atom_number, max_neighbor_numbers, max_atom_in_grid_numbers, kGridSize, cutoff_square, atom_in_grid_serial,
    reinterpret_cast<VECTOR *>(coordinate), box_length, gpointer, bucket, atom_numbers_in_grid_bucket,
    nl_atom_numbers, nl_atom_serial);
  Copy_Neighbor_List_Atom_Number<<<ceilf(static_cast<float>(atom_number) / 128), 128, 0,
    reinterpret_cast<cudaStream_t>(stream)>>>(atom_number, max_neighbor_numbers,
    nl_atom_numbers, nl_atom_serial, output_nl_atom_numbers, output_nl_atom_serial);
  return 0;
}
