    ...
    VECTOR* frc, float *atom_energy)
{
    if (is_initialized)
    {
        ...
        PME_Excluded_Force_With_Atom_Energy_Correction << <ceilf((float)atom_numbers / 128), 128 >> >
...
static __global__ void PME_Add_Energy_To_Virial(float *d_virial, float *d_direct_ene, float *d_correction_ene, float *d_self_ene, float *d_reciprocal_ene)
{
    d_virial[0] += d_direct_ene[0] + d_correction_ene[0] + d_self_ene[0] + d_reciprocal_ene[0];
}

static __global__ void PME_Add_Energy_To_Potential(float *d_virial, float *d_correction_ene, float *d_self_ene, float *d_reciprocal_ene)
{
    ...
    int need_virial, int need_energy, float *d_virial, float *d_potential)
{
    if (is_initialized)
    {
        ...
        if (need_virial > 0 || need_energy > 0)
        {
            ...
            if (need_energy > 0)
                PME_Add_Energy_To_Potential << <1, 1 >> >(d_potential, d_correction_ene, d_self_ene, d_reciprocal_ene);
            if (need_virial > 0)
                PME_Add_Energy_To_Virial << <1, 1 >> >(d_virial, d_direct_ene, d_correction_ene, d_self_ene, d_reciprocal_ene);
        }
    }
}
