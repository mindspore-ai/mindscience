...
struct GRID_INFORMATION {
  ...
  GRID_POINTER *h_pointer =
      NULL; //指向每个grid的125个邻居grid（包含自身）的指针
...
struct NEIGHBOR_LIST {
  ...
  float skin_permit =
      1.; //用于不严格要求的智能更新近邻表修正，该值越大，更新信号越不容易被触发。
  VECTOR box_length;
  VECTOR quarter_crd_to_uint_crd_cof;
  VECTOR uint_dr_to_dr_cof;

  int refresh_flag; //指示更新近邻表，在CPU上使用
  int *is_need_refresh_neighbor_list =
      NULL; //用于指示是否需要更新近邻表，在GPU上使用
  ...
  //初始化
  void Initial(CONTROLLER *controller, int md_atom_numbers, VECTOR box_length,
               float cut, float skin, const char *module_name = NULL);
  //分配内存
  void Initial_Malloc();
  //清除内存
  void Clear();
  ...
  void Neighbor_List_Update(VECTOR *crd, int *d_excluded_list_start,
                            int *d_excluded_list, int *d_excluded_numbers,
