...
static void Initial_Neighbor_Grid(GRID_POINTER **gpointer, GRID_BUCKET **bucket,
  ...
  Cuda_Malloc_Safely((void **)&atom_numbers_in_grid_bucket[0],
                     sizeof(int) * (grid_info[0].grid_numbers + 1));
  Reset_List<<<ceilf(((float)grid_info[0].grid_numbers + 1) / 32), 32>>>(
      grid_info[0].grid_numbers + 1, atom_numbers_in_grid_bucket[0], 0);

  Malloc_Safely((void **)&grid_info[0].h_bucket,
                sizeof(GRID_BUCKET) * (grid_info[0].grid_numbers + 1));
  for (int i = 0; i < grid_info[0].grid_numbers + 1; i = i + 1) {
    Cuda_Malloc_Safely((void **)&grid_info[0].h_bucket[i].atom_serial,
                       sizeof(int) * in_bucket_atom_numbers_max);
    Reset_List<<<ceilf((float)in_bucket_atom_numbers_max / 32), 32>>>(
        in_bucket_atom_numbers_max, grid_info[0].h_bucket[i].atom_serial, -1);
  }
  Cuda_Malloc_Safely((void **)&bucket[0],
                     sizeof(GRID_BUCKET) * (grid_info[0].grid_numbers + 1));
  cudaMemcpy(bucket[0], grid_info[0].h_bucket,
             sizeof(GRID_BUCKET) * (grid_info[0].grid_numbers + 1),
  ...
  int count;
  int small_out;
  Malloc_Safely((void **)&grid_info[0].h_pointer,
                sizeof(GRID_POINTER) * grid_info[0].grid_numbers);
  for (int i = 0; i < grid_info[0].grid_numbers; i = i + 1) {
    ...
    for (int l = -2; l <= 2; l = l + 1) {
      for (int m = -2; m <= 2; m = m + 1) {
        for (int n = -2; n <= 2; n = n + 1) {
          ...
          if (!small_out) {
            lin_pointer.grid_serial[count] =
                zz * grid_info[0].Nxy + yy * grid_info[0].Nx + xx;
    ...
    Cuda_Malloc_Safely((void **)&grid_info[0].h_pointer[i].grid_serial,
                       sizeof(int) * 125); // 5*5*5
    cudaMemcpy(grid_info[0].h_pointer[i].grid_serial, lin_pointer.grid_serial,
               sizeof(int) * 125, cudaMemcpyHostToDevice);
  }
  Cuda_Malloc_Safely((void **)&gpointer[0],
                     sizeof(GRID_POINTER) * grid_info[0].grid_numbers);

  cudaMemcpy(gpointer[0], grid_info[0].h_pointer,
             sizeof(GRID_POINTER) * grid_info[0].grid_numbers,
             cudaMemcpyHostToDevice);
}

static __global__ void Clear_Grid_Bucket(const int grid_numbers,
                                         int *atom_numbers_in_grid_bucket,
                                         GRID_BUCKET *bucket) {
...
static __global__ void Find_Atom_In_Grid_Serial(
    const int atom_numbers, const VECTOR grid_length_inverse, const VECTOR *crd,
    const INT_VECTOR grid_N, const int gridxy, int *atom_in_grid_serial) {
  int atom_i = blockDim.x * blockIdx.x + threadIdx.x;
  if (atom_i < atom_numbers) {
    int Nx = (float)crd[atom_i].x *
...
Put_Atom_In_Grid_Bucket(const int atom_numbers, const int *atom_in_grid_serial,
...
static __global__ void Find_atom_neighbors_gridly(
    ...
    const VECTOR uint_dr_to_dr_cof, const int *atom_in_grid_serial,
    const GRID_POINTER *gpointer, const GRID_BUCKET *bucket,
    const int *atom_numbers_in_grid_bucket, ATOM_GROUP *nl,
    const float cutoff_skin_square, const int max_atom_numbers_in_gird) {
  if (threadIdx.y < 125) {
    ...
    int *sm_bucket_i = (int *)(shared_memory + sizeof(UNSIGNED_INT_VECTOR) *
                                                   max_atom_numbers_in_gird);
    ATOM_GROUP *sm_nl =
        (ATOM_GROUP *)(shared_memory +
                       (sizeof(UNSIGNED_INT_VECTOR) + sizeof(int)) *
                           max_atom_numbers_in_gird);
    ...
    for (int j = threadIdx.x; j < atom_numbers_in_grid_j; j += blockDim.x) {
      ...
      for (int i = 0; i < atom_numbers_in_grid_i; i++) {
        ...
        if (atom_j > atom_i) {
          dr = Get_Periodic_Displacement(uint_crd_j, uints[i],
                                         uint_dr_to_dr_cof);
...
static __global__ void Find_atom_neighbors(
        const int atom_numbers, const UNSIGNED_INT_VECTOR *uint_crd, const
VECTOR uint_dr_to_dr_cof, const int *atom_in_grid_serial, const GRID_POINTER
*gpointer, const GRID_BUCKET *bucket, const int *atom_numbers_in_grid_bucket,
        ATOM_GROUP *nl, const float cutoff_skin_square)
{
        ...
        if (atom_i < atom_numbers)
        {
                ...
                *atom_numbers_address = 0;
                ...
                VECTOR dr;
                float dr2;
                for (int grid_cycle = threadIdx.x; grid_cycle < 125; grid_cycle
= grid_cycle + blockDim.x)
                {
                        grid_serial2 = gpointer_i.grid_serial[grid_cycle];
                        GRID_BUCKET bucket_i = bucket[grid_serial2];
                        for (int j = 0; j <
atom_numbers_in_grid_bucket[grid_serial2]; j = j + 1)
                        {
                                atom_j = bucket_i.atom_serial[j];
                                uint_crd_j = uint_crd[atom_j];
                                if (atom_j > atom_i)
                                {
                                        int_x = uint_crd_j.uint_x -
uint_crd_i.uint_x; int_y = uint_crd_j.uint_y - uint_crd_i.uint_y; int_z =
uint_crd_j.uint_z - uint_crd_i.uint_z; dr.x = uint_dr_to_dr_cof.x*int_x; dr.y =
uint_dr_to_dr_cof.y*int_y; dr.z = uint_dr_to_dr_cof.z*int_z; dr2 = dr.x*dr.x +
dr.y*dr.y + dr.z*dr.z; if (dr2 < cutoff_skin_square)
                                        {
                                                nl_i.atom_numbers =
atomicAdd(atom_numbers_address, 1); nl_i.atom_serial[nl_i.atom_numbers] =
atom_j;
                                        }
                                }
                        }
...
*/
static __global__ void Is_need_refresh_neighbor_list_cuda(
...
static __global__ void Delete_Excluded_Atoms_Serial_In_Neighbor_List(
    const int atom_numbers, ATOM_GROUP *nl, const int *excluded_list_start,
...
static __global__ void Refresh_Neighbor_List(
    int *refresh_sign, const int thread, const int atom_numbers, VECTOR *crd,
    VECTOR *old_crd, UNSIGNED_INT_VECTOR *uint_crd,
    const VECTOR quarter_crd_to_uint_crd_cof, const VECTOR uint_dr_to_dr_cof,
    int *atom_in_grid_serial, const float skin, const VECTOR box_length,
    const GRID_INFORMATION grid_info, const GRID_POINTER *gpointer,
    GRID_BUCKET *bucket, int *atom_numbers_in_grid_bucket, ATOM_GROUP *d_nl,
  ...
  if (refresh_sign[0] == 1) {
    ...
    Crd_Periodic_Map<<<ceilf((float)atom_numbers / thread), thread>>>(
        atom_numbers, crd, box_length);

    Find_Atom_In_Grid_Serial<<<ceilf((float)atom_numbers / thread), thread>>>(
        atom_numbers, grid_info.grid_length_inverse, crd, grid_info.grid_N,
        grid_info.Nxy, atom_in_grid_serial);
    ...
    Find_atom_neighbors_gridly<<<
        {(unsigned int)ceilf((float)grid_info.grid_numbers)}, {8, 128},
        (sizeof(int) + sizeof(UNSIGNED_INT_VECTOR) + sizeof(ATOM_GROUP)) *
            max_atom_in_grid_numbers>>>(
        atom_numbers, uint_crd, uint_dr_to_dr_cof, atom_in_grid_serial,
        gpointer, bucket, atom_numbers_in_grid_bucket, d_nl, cutoff_skin_square,
        max_atom_in_grid_numbers);

    /*Find_atom_neighbors << < {1, (unsigned int)ceilf((float)atom_numbers /
       125)}, { 8, 125 } >> > (atom_numbers, uint_crd, uint_dr_to_dr_cof,
...
*/
    ...
    Delete_Excluded_Atoms_Serial_In_Neighbor_List<<<
        ceilf((float)atom_numbers / thread), thread>>>(
        atom_numbers, d_nl, excluded_list_start, excluded_list,
        excluded_numbers);
    refresh_sign[0] = 0;
  }
}
...
static void Refresh_Neighbor_List_No_Check(
    ...
    UNSIGNED_INT_VECTOR *uint_crd, const VECTOR quarter_crd_to_uint_crd_cof,
    const VECTOR uint_dr_to_dr_cof, int *atom_in_grid_serial, const float skin,
    const VECTOR box_length, const GRID_INFORMATION grid_info,
    const GRID_POINTER *gpointer, GRID_BUCKET *bucket,
    int *atom_numbers_in_grid_bucket, ATOM_GROUP *d_nl,
    int *excluded_list_start, int *excluded_list, int *excluded_numbers,
    float cutoff_skin_square, const int max_atom_in_grid_numbers) {
  Clear_Grid_Bucket<<<ceilf((float)grid_info.grid_numbers / 32), 32>>>(
      grid_info.grid_numbers, grid_info.atom_numbers_in_grid_bucket,
      grid_info.bucket);

  Crd_Periodic_Map<<<ceilf((float)atom_numbers / 32), 32>>>(atom_numbers, crd,
                                                            box_length);

  Find_Atom_In_Grid_Serial<<<ceilf((float)atom_numbers / 32), 32>>>(
      atom_numbers, grid_info.grid_length_inverse, crd, grid_info.grid_N,
      grid_info.Nxy, grid_info.atom_in_grid_serial);

  cudaMemcpy(old_crd, crd, sizeof(VECTOR) * atom_numbers,
             cudaMemcpyDeviceToDevice);

  Put_Atom_In_Grid_Bucket<<<ceilf((float)atom_numbers / 32), 32>>>(
      atom_numbers, grid_info.atom_in_grid_serial, grid_info.bucket,
  ...
  Find_atom_neighbors_gridly<<<
      ...
      (sizeof(int) + sizeof(UNSIGNED_INT_VECTOR) + sizeof(ATOM_GROUP)) *
          max_atom_in_grid_numbers>>>(
      atom_numbers, uint_crd, uint_dr_to_dr_cof, atom_in_grid_serial, gpointer,
...
void NEIGHBOR_LIST::Neighbor_List_Update(VECTOR *crd,
                                         int *d_excluded_list_start,
                                         int *d_excluded_list,
  ...
  if (is_initialized) {
    if (forced_update) { //如果强制要求更新就强制更新
      Refresh_Neighbor_List_No_Check(
          ...
          grid_info, grid_info.gpointer, grid_info.bucket,
          grid_info.atom_numbers_in_grid_bucket, d_nl, d_excluded_list_start,
      ...
      Refresh_Neighbor_List<<<1, 1>>>(
          is_need_refresh_neighbor_list, 32, atom_numbers, crd, old_crd,
          uint_crd, quarter_crd_to_uint_crd_cof, uint_dr_to_dr_cof,
          grid_info.atom_in_grid_serial, skin, box_length, grid_info,
          grid_info.gpointer, grid_info.bucket,
          grid_info.atom_numbers_in_grid_bucket, d_nl, d_excluded_list_start,
          d_excluded_list, d_excluded_numbers, cutoff_with_skin_square,
          max_atom_in_grid_numbers);
    }
...
void NEIGHBOR_LIST::Initial_Malloc() {
  ...
  for (int i = 0; i < atom_numbers; i = i + 1) {
    ...
    Cuda_Malloc_Safely((void **)&h_nl[i].atom_serial,
                       sizeof(int) * max_neighbor_numbers);
  }
  cudaMemcpy(d_nl, h_nl, sizeof(ATOM_GROUP) * atom_numbers,
             cudaMemcpyHostToDevice);
  for (int i = 0; i < atom_numbers; i = i + 1) {
    Malloc_Safely((void **)&h_nl[i].atom_serial,
                  sizeof(int) * max_neighbor_numbers);
  }
  Cuda_Malloc_Safely((void **)&is_need_refresh_neighbor_list, sizeof(int));
  Reset_List<<<1, 1>>>(1, is_need_refresh_neighbor_list, 0);
  Cuda_Malloc_Safely((void **)&grid_info.atom_in_grid_serial,
...
void NEIGHBOR_LIST::Initial(CONTROLLER *controller, int md_atom_numbers,
  ...
  if (controller[0].Command_Exist(this->module_name, "refresh_interval")) {
    refresh_interval =
        atoi(controller[0].Command(this->module_name, "refresh_interval"));
  }
  if (controller[0].Command_Exist(this->module_name,
                                  "max_atom_in_grid_numbers")) {
    max_atom_in_grid_numbers = atoi(
        controller[0].Command(this->module_name, "max_atom_in_grid_numbers"));
  }
  if (controller[0].Command_Exist(this->module_name, "max_neighbor_numbers")) {
    max_neighbor_numbers =
        atoi(controller[0].Command(this->module_name, "max_neighbor_numbers"));
  }
  if (controller[0].Command_Exist(this->module_name, "skin_permit")) {
    skin_permit =
        2. * atof(controller[0].Command(
                 this->module_name,
                 "skin_permit")); //以外界的0.5等于不变（即程序内的1.）
  }
  this->skin = skin;
  this->cutoff = cut;
  ...
  this->quarter_crd_to_uint_crd_cof =
      0.25f * CONSTANT_UINT_MAX_FLOAT / box_length;
...
void NEIGHBOR_LIST::Clear() {
  if (is_initialized == 1) {
    ...
    cudaMemcpy(h_nl, d_nl, sizeof(ATOM_GROUP) * atom_numbers,
    ...
    cudaMemcpy(grid_info.h_bucket, grid_info.bucket,
               sizeof(GRID_BUCKET) * grid_info.grid_numbers,
               cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(grid_info.h_pointer, grid_info.gpointer,
               sizeof(GRID_BUCKET) * grid_info.grid_numbers,
               cudaMemcpyDeviceToHost);
    for (int i = 0; i < grid_info.grid_numbers; i = i + 1) {
