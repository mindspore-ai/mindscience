...
void MD_INFORMATION::Read_Mass(CONTROLLER *controller)
{
    ...
    {
        controller[0].printf("    Error: failed to initialize mass, because no atom_numbers found\n");
        getchar();
        exit(1);
    }
    if (atom_numbers > 0)
...
void MD_INFORMATION::Read_Charge(CONTROLLER *controller)
{
    ...
    {
        controller[0].printf("    Error: failed to initialize charge, because no atom_numbers found\n");
        getchar();
        exit(1);
    }
    if (atom_numbers > 0)
...
void MD_INFORMATION::Read_Rst7(const char* file_name, int irest, CONTROLLER controller)
{
    ...
    //in some bad rst7, the coordinates will be extremel bad, so need a full box map
    for (int i = 0; i < 10; i = i + 1)
    {
