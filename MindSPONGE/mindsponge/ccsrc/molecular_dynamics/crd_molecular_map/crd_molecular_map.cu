    ...
    const int exclude_numbers, const int *exclude_length, const int *exclude_start, const int *exclude_list)
{
    ...
    {
        atom_i = i;
        for (int j = exclude_start[i] + exclude_length[i] - 1; j >= exclude_start[i]; j--) //这里使用倒序是因为链表构建是用的头插法
        {
    ...
    for (int i = 0; i < atom_numbers; i++)
    {
        if (!visited[i])
        {
            ...
            while (!queue.empty())
            {
                atom = queue[0];
                queue.pop_front();
                box_map_times[atom].int_x = floorf((crd[atom_front].x - crd[atom].x) / box_length.x + 0.5);
                box_map_times[atom].int_y = floorf((crd[atom_front].y - crd[atom].y) / box_length.y + 0.5);
                box_map_times[atom].int_z = floorf((crd[atom_front].z - crd[atom].z) / box_length.z + 0.5);
