...
static void Import_Information_From_Rst7(const char *file_name,
  ...
  for (int i = 0; i < atom_numbers[0]; i = i + 1) {
    scanf_ret = fscanf(fin, "%f %f %f", &h_crd[i].x, &h_crd[i].y, &h_crd[i].z);
  }
  if (has_vel == 1) {
    for (int i = 0; i < atom_numbers[0]; i = i + 1) {
      scanf_ret =
  ...
  scanf_ret = fscanf(fin, "%f %f %f", &box_length[0].x, &box_length[0].y,
                     &box_length[0].z);
  controller.printf("        system size is %f %f %f\n", box_length[0].x,
                    box_length[0].y, box_length[0].z);
  cudaMemcpy(crd[0], h_crd, sizeof(VECTOR) * atom_numbers[0],
...
static __global__ void restrain_energy(const int restrain_numbers,
                                       const int *restrain_list,
                                       const VECTOR *crd, const VECTOR *crd_ref,
                                       const float weight,
                                       const VECTOR boxlength, float *ene) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i < restrain_numbers) {
    int atom_i = restrain_list[i];
    VECTOR dr =
        Get_Periodic_Displacement(crd_ref[atom_i], crd[atom_i], boxlength);
...
static __global__ void restrain_force_with_atom_energy_and_virial(
    ...
    const VECTOR *crd_ref, const float weight, const VECTOR boxlength,
...
void RESTRAIN_INFORMATION::Initial(CONTROLLER *controller,
  ...
  if (controller[0].Command_Exist(this->module_name, "atom_id")) {
    ...
    FILE *fr = NULL;
    Open_File_Safely(&fr, controller[0].Command(this->module_name, "atom_id"),
                     "r");
    ...
    cudaMemcpy(this->h_lists, temp_atom_lists,
               sizeof(int) * this->restrain_numbers, cudaMemcpyHostToHost);
    Cuda_Malloc_Safely((void **)&this->d_lists,
                       sizeof(int) * this->restrain_numbers);
    cudaMemcpy(this->d_lists, temp_atom_lists,
               sizeof(int) * this->restrain_numbers, cudaMemcpyHostToDevice);
    Cuda_Malloc_Safely((void **)&this->d_restrain_ene,
    ...
    if (controller[0].Command_Exist(this->module_name, "coordinate_in_file")) {
      ...
      FILE *fp = NULL;
      Open_File_Safely(
          &fp, controller[0].Command(this->module_name, "coordinate_in_file"),
          "r");
      ...
      cudaMemcpy(crd_ref, h_crd, sizeof(VECTOR) * atom_numbers,
                 cudaMemcpyHostToDevice);
      free(h_crd);
      fclose(fp);
      ...
      Import_Information_From_Rst7(
          controller[0].Command(this->module_name, "amber_rst7"), &temp_atom,
          &ref_time, &crd_ref, &d_vel, &h_boxlength, controller[0]);
    } else {
      controller[0].printf(
          "    restrain reference coordinate copy from input coordinate\n");
      cudaMemcpy(crd_ref, crd, sizeof(VECTOR) * atom_numbers,
...
void RESTRAIN_INFORMATION::Restraint(const VECTOR *crd, const VECTOR box_length,
                                     float *atom_energy, float *atom_virial,
                                     VECTOR *frc) {
  if (is_initialized) {
    restrain_force_with_atom_energy_and_virial<<<
        (unsigned int)ceilf((float)this->restrain_numbers / threads_per_block),
        threads_per_block>>>(this->restrain_numbers, this->d_lists, crd,
                             this->crd_ref, this->weight, box_length,
                             atom_energy, atom_virial, frc);
  }
}

float RESTRAIN_INFORMATION::Get_Energy(const VECTOR *crd,
                                       const VECTOR box_length,
                                       int is_download) {
  if (is_initialized) {
    restrain_energy<<<(unsigned int)ceilf((float)this->restrain_numbers /
                                          threads_per_block),
                      threads_per_block>>>(
        this->restrain_numbers, this->d_lists, crd, this->crd_ref, this->weight,
        box_length, d_restrain_ene);
    Sum_Of_List(d_restrain_ene, d_sum_of_restrain_ene, restrain_numbers);
    if (is_download) {
      cudaMemcpy(&h_sum_of_restrain_ene, d_sum_of_restrain_ene, sizeof(float),
