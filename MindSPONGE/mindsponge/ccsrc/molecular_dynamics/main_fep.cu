...
int main(int argc, char *argv[]) {
  ...
  for (fep_core.input.current_frame = 1;
       fep_core.input.current_frame <= fep_core.input.frame_numbers;
       ++fep_core.input.current_frame) {
    Main_Calculation();
    Main_Print();
...
void Main_Initial(int argc, char *argv[]) {
  ...
  if (fep_core.charge_pertubated)
    pme.Initial(&controller, fep_core.atom_numbers, fep_core.box_length,
                fep_core.nb.cutoff);

  if (fep_core.charge_pertubated || lj_soft.is_initialized) {
    // when we need to calculate direct energy
    neighbor_list.Initial(&controller, fep_core.atom_numbers,
                          fep_core.box_length, fep_core.nb.cutoff,
                          fep_core.nb.skin);
    neighbor_list.Neighbor_List_Update(
        fep_core.crd, fep_core.nb.d_excluded_list_start,
        fep_core.nb.d_excluded_list, fep_core.nb.d_excluded_numbers, 1);
  }
...
void Main_Calculation() {
  ...
  if (bond.is_initialized)
    fep_core.data.partition.bond_ene =
        bond.Get_Energy(fep_core.uint_crd, fep_core.pbc.uint_dr_to_dr_cof);
  else
    fep_core.data.partition.bond_ene = 0.0;

  if (angle.is_initialized)
    fep_core.data.partition.angle_ene =
        angle.Get_Energy(fep_core.uint_crd, fep_core.pbc.uint_dr_to_dr_cof);
  else
    fep_core.data.partition.angle_ene = 0.0;

  if (dihedral.is_initialized)
    fep_core.data.partition.dihedral_ene =
        dihedral.Get_Energy(fep_core.uint_crd, fep_core.pbc.uint_dr_to_dr_cof);
  else
    fep_core.data.partition.dihedral_ene = 0.0;

  if (nb14.is_initialized) {
    fep_core.data.partition.nb14_EE_ene = nb14.Get_14_CF_Energy(
        fep_core.uint_crd, fep_core.d_charge, fep_core.pbc.uint_dr_to_dr_cof);
    fep_core.data.partition.nb14_LJ_ene = nb14.Get_14_LJ_Energy(
        fep_core.uint_crd, fep_core.pbc.uint_dr_to_dr_cof);
  } else {
    fep_core.data.partition.nb14_EE_ene = fep_core.data.partition.nb14_LJ_ene =
        0.0;
  }

  if (bond_soft.is_initialized)
    fep_core.data.partition.bond_soft_ene =
        bond_soft.Get_Energy(fep_core.uint_crd, fep_core.pbc.uint_dr_to_dr_cof);
  else
    fep_core.data.partition.bond_soft_ene = 0.0;

  if (lj_soft.is_initialized) {
    fep_core.data.lj_soft_ene = lj_soft.Get_Energy_With_Coulomb_Direct(
        fep_core.uint_crd, neighbor_list.d_nl, fep_core.d_charge);
    ...
    fep_core.data.partition.coul_direct_intersys_ene =
        lj_soft.h_direct_ene_sum_intersys;
    fep_core.data.partition.coul_direct_intrasys_ene =
        lj_soft.h_direct_ene_sum_intrasys;
    fep_core.data.partition.vdw_long_range_correction =
  ...
  if (fep_core.charge_pertubated) {
    fep_core.data.partition.coul_long_range = pme.Get_Energy(
        fep_core.uint_crd, fep_core.d_charge, neighbor_list.d_nl,
        fep_core.pbc.uint_dr_to_dr_cof, fep_core.nb.d_excluded_list_start,
        fep_core.nb.d_excluded_list, fep_core.nb.d_excluded_numbers,
        pme.CORRECTION);

    fep_core.data.partition.coul_long_range += pme.Get_Energy(
        fep_core.uint_crd, fep_core.d_charge, neighbor_list.d_nl,
        fep_core.pbc.uint_dr_to_dr_cof, fep_core.nb.d_excluded_list_start,
        fep_core.nb.d_excluded_list, fep_core.nb.d_excluded_numbers, pme.SELF);
  ...
  fep_core.data.partition.pV = fep_core.data.pressure * fep_core.box_length.x *
                               fep_core.box_length.y * fep_core.box_length.z;
...
void Main_Print() {
  ...
  controller.Step_Print("Coul(direct.)",
                        fep_core.data.partition.coul_direct_intrasys_ene +
                            fep_core.data.partition.coul_direct_intersys_ene);
  controller.Step_Print("LR_corr(sc.)",
                        fep_core.data.partition.vdw_long_range_correction);
  ...
  controller.Step_Print("Coul(all.)",
                        fep_core.data.partition.coul_long_range +
                            fep_core.data.partition.coul_direct_intersys_ene +
                            fep_core.data.partition.coul_direct_intrasys_ene);
