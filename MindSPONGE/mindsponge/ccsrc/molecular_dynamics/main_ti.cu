...
int main(int argc, char *argv[]) {
  ...
  for (TI_core.input.current_frame = 1;
       TI_core.input.current_frame <= TI_core.input.frame_numbers;
       ++TI_core.input.current_frame) {
    Main_Calculation();
    Main_Print();
...
void Main_Initial(int argc, char *argv[]) {
  ...
  if (TI_core.charge_pertubated) {
    pme.Initial(&controller, TI_core.atom_numbers, TI_core.box_length,
                TI_core.nb.cutoff);
    TI_core.cross_pme.Initial(TI_core.atom_numbers, pme.PME_Nall);
  }

  if (TI_core.charge_pertubated || lj_soft.is_initialized) {
    neighbor_list.Initial(&controller, TI_core.atom_numbers, TI_core.box_length,
                          TI_core.nb.cutoff, TI_core.nb.skin);
    neighbor_list.Neighbor_List_Update(
        TI_core.crd, TI_core.nb.d_excluded_list_start,
        TI_core.nb.d_excluded_list, TI_core.nb.d_excluded_numbers, 1);
  }
...
void Main_Calculation() {
  ...
  if (bondA.is_initialized)
    TI_core.data.bondA_ene =
        bondA.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.bondA_ene = 0.0;

  if (bondB.is_initialized)
    TI_core.data.bondB_ene =
        bondB.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.bondB_ene = 0.0;

  if (angleA.is_initialized)
    TI_core.data.angleA_ene =
        angleA.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.angleA_ene = 0.0;

  if (angleB.is_initialized)
    TI_core.data.angleB_ene =
        angleB.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.angleB_ene = 0.0;

  if (dihedralA.is_initialized)
    TI_core.data.dihedralA_ene =
        dihedralA.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.dihedralA_ene = 0.0;

  if (dihedralB.is_initialized)
    TI_core.data.dihedralB_ene =
        dihedralB.Get_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.dihedralB_ene = 0.0;

  if (nb14A.is_initialized) {
    TI_core.data.nb14A_EE_ene = nb14A.Get_14_CF_Energy(
        TI_core.uint_crd, TI_core.d_charge, TI_core.pbc.uint_dr_to_dr_cof);
    TI_core.data.nb14A_LJ_ene =
        nb14A.Get_14_LJ_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  } else {
  ...
  if (nb14B.is_initialized) {
    TI_core.data.nb14B_EE_ene = nb14B.Get_14_CF_Energy(
        TI_core.uint_crd, TI_core.d_charge, TI_core.pbc.uint_dr_to_dr_cof);
    TI_core.data.nb14B_LJ_ene =
        nb14B.Get_14_LJ_Energy(TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  } else {
  ...
  if (bond_soft.is_initialized)
    TI_core.data.bond_soft_dH_dlambda = bond_soft.Get_Partial_H_Partial_Lambda(
        TI_core.uint_crd, TI_core.pbc.uint_dr_to_dr_cof);
  else
    TI_core.data.bond_soft_dH_dlambda = 0.0;

  if (lj_soft.is_initialized) {
    TI_core.data.lj_soft_dH_dlambda =
        lj_soft.Get_Partial_H_Partial_Lambda_With_Columb_Direct(
            TI_core.uint_crd, TI_core.d_charge, neighbor_list.d_nl,
            TI_core.d_charge_B_A, TI_core.charge_pertubated);
    TI_core.data.coul_direct_dH_dlambda = *lj_soft.h_sigma_of_dH_dlambda_direct;
    TI_core.data.lj_soft_long_range_correction =
        lj_soft.Partial_H_Partial_Lambda_Long_Range_Correction();
  } else {
  ...
  if (TI_core.charge_pertubated) {
    TI_core.data.pme_dH_dlambda =
        TI_core.Get_Cross_PME_Partial_H_Partial_Lambda(&pme, neighbor_list.d_nl,
...
void Main_Iteration() {
  ...
  Main_Volume_Update();
  neighbor_list.Neighbor_List_Update(
      TI_core.crd, TI_core.nb.d_excluded_list_start, TI_core.nb.d_excluded_list,
      TI_core.nb.d_excluded_numbers, 1);
}
