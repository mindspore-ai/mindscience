...
void SETTLE::Initial(CONTROLLER* controller, CONSTRAIN* constrain, float *h_mass, const char* module_name)
{
    ...
    if (constrain->constrain_pair_numbers > 0)
    {
        ...
        float *link_r = NULL;
        Malloc_Safely((void**)&linker_numbers, sizeof(int)*constrain->atom_numbers);
        Malloc_Safely((void**)&linker_atoms, 2 * sizeof(int)*constrain->atom_numbers);
        Malloc_Safely((void**)&link_r, 3 * sizeof(float)*constrain->atom_numbers);
        ...
        if (triangle_numbers > 0 || pair_numbers > 0)
        {
            ...
            Cuda_Malloc_Safely((void**)&constrain->constrain_pair, sizeof(CONSTRAIN_PAIR)* new_constrain_pair_numbers);
            cudaMemcpy(constrain->constrain_pair, constrain->h_constrain_pair, sizeof(CONSTRAIN_PAIR)* new_constrain_pair_numbers, cudaMemcpyHostToDevice);
...
void SETTLE::Remember_Last_Coordinates(UNSIGNED_INT_VECTOR *uint_crd, VECTOR scaler)
{
    if (!is_initialized)
    {
        return;
    ...
    remember_pair_AB << <64, 1024 >> >(pair_numbers, d_pairs, uint_crd, scaler,
        last_pair_AB);
    remember_triangle_BA_CA << <64, 1024 >> >(triangle_numbers, d_triangles, uint_crd, scaler,
...
void SETTLE::Do_SETTLE(const float *d_mass, VECTOR *crd, VECTOR box_length, VECTOR *vel,
    int need_pressure, float* d_pressure)
{
    ...
    settle_pair << <64, 64 >> > (pair_numbers, d_pairs, d_mass, crd, box_length, last_pair_AB,
        constrain->dt, constrain->v_factor, constrain->x_factor, vel, virial_vector + triangle_numbers);
    
    settle_triangle << <64, 64 >> > (triangle_numbers, d_triangles, d_mass, crd, box_length, last_triangle_BA, last_triangle_CA,
        constrain->dt, constrain->v_factor, constrain->x_factor, vel, virial_vector);
