...
struct CONSTRAIN {
  ...
  VECTOR
      quarter_crd_to_uint_crd_cof; //该系数可将实际坐标变为对应的一半长度的无符号整型坐标
  float volume; //体积

  float v_factor =
      1.0f; //一个积分步中,一个微小的力F对速度的影响，即dv = v_factor * F * dt/m
  ...
  // 20201125
  // 由于MD_INFORMATION里面暂时没有加入原子序数，所以不用原子序数判断H，而是直接比较质量
  //当质量较小时，就认为是H
  //传入的指针指向HOST内存
  void Add_HBond_To_Constrain_Pair(
      ...
      const char *module_name = NULL); //要求均是指向host上内存的指针
  ...
  void Add_HAngle_To_Constrain_Pair(
      ...
      const float *atom_mass); //要求均是指向host上内存的指针
  ...
  void Initial_Constrain(CONTROLLER *controller, const int atom_numbers,
                         ...
                         const float exp_gamma, const int is_Minimization,
