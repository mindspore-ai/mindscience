...
void CONSTRAIN::Initial_Constrain(CONTROLLER *controller,
                                  ...
                                  const float exp_gamma,
                                  const int is_Minimization, float *atom_mass,
                                  int *system_freedom) {
  //从传入的参数复制基本信息
  ...
  this->quarter_crd_to_uint_crd_cof =
      0.25 * CONSTANT_UINT_MAX_FLOAT / box_length;
  ...
  constrain_pair_numbers = bond_constrain_pair_numbers +
                           angle_constrain_pair_numbers + extra_numbers;
  ...
  Cuda_Malloc_Safely((void **)&constrain_pair,
                     sizeof(CONSTRAIN_PAIR) * constrain_pair_numbers);
  ...
  //上传GPU
  cudaMemcpy(constrain_pair, h_constrain_pair,
             sizeof(CONSTRAIN_PAIR) * constrain_pair_numbers,
...
void CONSTRAIN::Add_HBond_To_Constrain_Pair(
    CONTROLLER *controller, const int bond_numbers, const int *atom_a,
    const int *atom_b, const float *bond_r, const float *atom_mass,
...
void CONSTRAIN::Add_HAngle_To_Constrain_Pair(
    CONTROLLER *controller, const int angle_numbers, const int *atom_a,
    const int *atom_b, const int *atom_c, const float *angle_theta,
    const float *atom_mass) {
  int temp = 0;
  if (controller->Command_Exist(this->module_name, "angle") &&
      atoi(controller->Command(this->module_name, "angle"))) {
    temp = angle_numbers;
  }
  ...
  //预先分配一个足够大的CONSTRAIN_PAIR用于临时存储
  Malloc_Safely((void **)&h_angle_pair,
                sizeof(CONSTRAIN_PAIR) * angle_numbers * 2);
  int s = 0;
  float mass_a, mass_c;
  for (int i = 0; i < temp; i = i + 1) {
    ...
    if ((mass_a < constrain_mass && mass_a > 0) ||
      ...
      for (int j = 0; j < bond_info.bond_numbers; j = j + 1) {
        //找到a，b原子的平衡距离
        if ((bond_info.atom_a[j] == atom_a[i] &&
             bond_info.atom_b[j] == atom_b[i]) ||
            (bond_info.atom_a[j] == atom_b[i] &&
             bond_info.atom_b[j] == atom_a[i])) {
          rab = bond_info.bond_r[j];
        }

        //找到b，c原子的平衡距离
        if ((bond_info.atom_a[j] == atom_c[i] &&
             bond_info.atom_b[j] == atom_b[i]) ||
            (bond_info.atom_a[j] == atom_b[i] &&
             bond_info.atom_b[j] == atom_c[i])) {
      ...
      h_angle_pair[s].constant_r =
          sqrtf(rab * rab + rbc * rbc - 2. * rab * rbc * cosf(angle_theta[i]));
      h_angle_pair[s].constrain_k =
