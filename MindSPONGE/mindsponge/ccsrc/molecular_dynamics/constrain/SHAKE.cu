...
Constrain_Force_Cycle(const int constrain_pair_numbers,
  ...
  if (pair_i < constrain_pair_numbers) {
    ...
    dr.x = ((int)(uint_crd[cp.atom_i_serial].uint_x -
                  uint_crd[cp.atom_j_serial].uint_x)) *
           scaler.x;
    dr.y = ((int)(uint_crd[cp.atom_i_serial].uint_y -
                  uint_crd[cp.atom_j_serial].uint_y)) *
           scaler.y;
    dr.z = ((int)(uint_crd[cp.atom_i_serial].uint_z -
                  uint_crd[cp.atom_j_serial].uint_z)) *
           scaler.z;
    float r_1 = rnorm3df(dr.x, dr.y, dr.z);
    frc_abs = 0.5 * (dr * dr - cp.constant_r * cp.constant_r) / (dr * dr0) *
              cp.constrain_k;
...
static __global__ void
Refresh_Uint_Crd(const int atom_numbers, const VECTOR *crd,
                 const VECTOR quarter_crd_to_uint_crd_cof,
                 UNSIGNED_INT_VECTOR *uint_crd, const VECTOR *test_frc,
  ...
  if (atom_i < atom_numbers) {
    ...
    crd_lin.x =
        crd_lin.x + half_exp_gamma_plus_half * frc_lin.x *
                        mass_lin; // mass实际为mass的倒数，frc_lin已经乘以dt^2
...
static __global__ void Last_Crd_To_dr(const int constarin_pair_numbers,
                                      const VECTOR *atom_crd,
                                      const VECTOR quarter_crd_to_uint_crd_cof,
                                      const VECTOR uint_dr_to_dr,
                                      const CONSTRAIN_PAIR *constrain_pair,
                                      VECTOR *pair_dr) {
  int pair_i = blockDim.x * blockIdx.x + threadIdx.x;
...
static __global__ void
Refresh_Crd_Vel(const int atom_numbers, const float dt_inverse, const float dt,
                VECTOR *crd, VECTOR *vel, const VECTOR *test_frc,
                const float *mass_inverse, const float exp_gamma,
                const float half_exp_gamma_plus_half) {
  int atom_i = blockDim.x * blockIdx.x + threadIdx.x;
  if (atom_i < atom_numbers) {
    VECTOR crd_lin = crd[atom_i];
...
void SHAKE::Initial_Simple_Constrain(CONTROLLER *controller,
  ...
  if (constrain->constrain_pair_numbers > 0) {
    ...
    if (controller[0].Command_Exist(this->module_name, "iteration_numbers")) {
      int scanf_ret =
          sscanf(controller[0].Command(this->module_name, "iteration_numbers"),
                 "%d", &iteration_numbers);
    }
    ...
    if (controller[0].Command_Exist(this->module_name, "step_length")) {
      int scanf_ret =
          sscanf(controller[0].Command(this->module_name, "step_length"), "%f",
                 &step_length);
    }
    controller[0].printf("    constrain step length is %.2f\n", step_length);

    Cuda_Malloc_Safely((void **)&constrain_frc,
                       sizeof(VECTOR) * constrain->atom_numbers);
    Cuda_Malloc_Safely((void **)&test_uint_crd,
                       sizeof(UNSIGNED_INT_VECTOR) * constrain->atom_numbers);
    Cuda_Malloc_Safely((void **)&last_pair_dr,
                       sizeof(VECTOR) * constrain->constrain_pair_numbers);
    Cuda_Malloc_Safely((void **)&d_pair_virial,
                       sizeof(float) * constrain->constrain_pair_numbers);
...
void SHAKE::Remember_Last_Coordinates(VECTOR *crd,
                                      UNSIGNED_INT_VECTOR *uint_crd,
                                      VECTOR scaler) {
  if (is_initialized) {
    //获得分子模拟迭代中上一步的距离信息
    Last_Crd_To_dr<<<ceilf((float)constrain->constrain_pair_numbers / 128),
                     128>>>(constrain->constrain_pair_numbers, crd,
                            constrain->quarter_crd_to_uint_crd_cof,
...
static __global__ void Constrain_Force_Cycle_With_Virial(
    const int constrain_pair_numbers, const UNSIGNED_INT_VECTOR *uint_crd,
  ...
  if (pair_i < constrain_pair_numbers) {
    CONSTRAIN_PAIR cp = constrain_pair[pair_i];
    VECTOR dr0 = pair_dr[pair_i];
    VECTOR dr = Get_Periodic_Displacement(uint_crd[cp.atom_i_serial],
                                          uint_crd[cp.atom_j_serial], scaler);
    float r_1 = rnorm3df(dr.x, dr.y, dr.z);
...
static __global__ void pressure_fix(float *pressure, float *virial,
                                    float factor) {
  pressure[0] += factor * virial[0];
}

void SHAKE::Constrain(VECTOR *crd, VECTOR *vel, const float *mass_inverse,
                      const float *d_mass, VECTOR box_length, int need_pressure,
                      float *d_pressure) {
  if (is_initialized) {
    //清空约束力和维里
    ...
    if (need_pressure > 0) {
      Reset_List<<<ceilf((float)constrain->constrain_pair_numbers / 1024.0f),
                   1024>>>(constrain->constrain_pair_numbers, d_pair_virial,
                           0.0f);
    ...
    for (int i = 0; i < iteration_numbers; i = i + 1) {
      ...
      if (need_pressure > 0) {
        Constrain_Force_Cycle_With_Virial<<<
            ceilf((float)constrain->constrain_pair_numbers / 128), 128>>>(
            constrain->constrain_pair_numbers, test_uint_crd,
            constrain->uint_dr_to_dr_cof, constrain->constrain_pair,
            last_pair_dr, constrain_frc, d_pair_virial);
      } else {
        Constrain_Force_Cycle<<<
            ceilf((float)constrain->constrain_pair_numbers / 128), 128>>>(
            constrain->constrain_pair_numbers, test_uint_crd,
            constrain->uint_dr_to_dr_cof, constrain->constrain_pair,
    ...
    if (need_pressure > 0) {
      Sum_Of_List<<<1, 1024>>>(constrain->constrain_pair_numbers, d_pair_virial,
                               d_virial);
      pressure_fix<<<1, 1>>>(d_pressure, d_virial,
                             1 / constrain->dt / constrain->dt / 3.0 /
                                 constrain->volume);
    }

    Refresh_Crd_Vel<<<ceilf((float)constrain->atom_numbers / 128), 128>>>(
        constrain->atom_numbers, constrain->dt_inverse, constrain->dt, crd, vel,
        constrain_frc, mass_inverse, constrain->v_factor, constrain->x_factor);
  }
}
