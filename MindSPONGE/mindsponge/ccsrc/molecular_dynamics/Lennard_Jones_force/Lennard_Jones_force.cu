...
static __global__ void LJ_Direct_CF_Force_With_Atom_Energy_And_LJ_Virial_Direct_CF_Energy_CUDA(
    const int atom_numbers, const ATOM_GROUP *nl,
    const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR boxlength,
    const float *LJ_type_A, const float *LJ_type_B, const float cutoff,
...
static __global__ void LJ_Force_CUDA(
    const int atom_numbers, const ATOM_GROUP *nl,
    const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR boxlength,
    const float *LJ_type_A, const float *LJ_type_B, const float cutoff,
    VECTOR *frc)
...
static __global__ void LJ_Force_With_Atom_Virial_CUDA(
    const int atom_numbers, const ATOM_GROUP *nl,
    const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR boxlength,
    const float *LJ_type_A, const float *LJ_type_B, const float cutoff,
...
static __global__ void LJ_Force_With_Atom_Energy_CUDA(
    const int atom_numbers, const ATOM_GROUP *nl,
    const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR boxlength,
    const float *LJ_type_A, const float *LJ_type_B, const float cutoff,
...
__global__ void LJ_Energy_CUDA(
    const int atom_numbers, const ATOM_GROUP *nl,
    const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR boxlength,
    const float *LJ_type_A, const float *LJ_type_B, const float cutoff_square,
    float *lj_ene)
{
    int atom_i = blockDim.x*blockIdx.x + threadIdx.x;
    if (atom_i < atom_numbers)
        ...
        for (int j = threadIdx.y; j < N; j = j + blockDim.y)
        {
            ...
            if (dr2 < cutoff_square)
            {
                ...
                dr_2 = (0.083333333*LJ_type_A[atom_pair_LJ_type] * dr_6
                    - 0.166666666*LJ_type_B[atom_pair_LJ_type]) * dr_6;//LJ的A,B系数已经乘以12和6因此要反乘
...
void LENNARD_JONES_INFORMATION::LJ_Force(const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler, VECTOR *frc,
    const ATOM_GROUP *nl, const float cutoff)
{
    if (is_initialized)
        LJ_Force_CUDA << <(unsigned int)ceilf((float)atom_numbers / thread_LJ.x), thread_LJ >> >
            (atom_numbers, nl,
            uint_crd, scaler,
...
void LENNARD_JONES_INFORMATION::LJ_PME_Direct_Force_With_Atom_Energy(const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler, VECTOR *frc,
    ...
    const int need_virial, float *atom_lj_virial, float *atom_direct_pme_energy)
{
    if (is_initialized)
    {
        ...
        {
            LJ_PME_Direct_Force_With_Atom_Energy(atom_numbers, uint_crd_with_LJ, uint_dr_to_dr_cof, frc, nl, cutoff, pme_beta, atom_energy);
        }
        else if (!need_atom_energy > 0 && need_virial> 0)
        {
            ...
            LJ_Direct_CF_Force_With_LJ_Virial_Direct_CF_Energy_CUDA << <{1, (unsigned int)ceilf((float)atom_numbers / thread_LJ.y)}, thread_LJ >> >
                ...
                d_LJ_A, d_LJ_B, cutoff,
                frc, pme_beta, TWO_DIVIDED_BY_SQRT_PI, atom_lj_virial, atom_direct_pme_energy);
        }
        else
        {
            Reset_List(atom_direct_pme_energy, 0.0f, atom_numbers, 1024);
            LJ_Direct_CF_Force_With_Atom_Energy_And_LJ_Virial_Direct_CF_Energy_CUDA << <{1, (unsigned int)ceilf((float)atom_numbers / thread_LJ.y)}, thread_LJ >> >
                ...
                uint_crd_with_LJ, uint_dr_to_dr_cof,
                d_LJ_A, d_LJ_B, cutoff,
                frc, pme_beta, TWO_DIVIDED_BY_SQRT_PI, atom_energy, atom_lj_virial, atom_direct_pme_energy);
        }
    }
}

void LENNARD_JONES_INFORMATION::LJ_Force_With_Atom_Energy_And_Virial(const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler, VECTOR *frc,
    ...
    int need_virial, float *atom_lj_virial, float *virial)
{
    if (is_initialized)
    {
        ...
        {
            LJ_Force_With_Atom_Energy_CUDA << <(unsigned int)ceilf((float)atom_numbers / thread_LJ.x), thread_LJ >> >
            (atom_numbers, nl,
                uint_crd, scaler,
                d_LJ_A, d_LJ_B, cutoff,
                frc,  atom_energy);
        ...
        {
            LJ_Force_With_Atom_Virial_CUDA << <(unsigned int)ceilf((float)atom_numbers / thread_LJ.x), thread_LJ >> >
                (atom_numbers, nl,
                uint_crd, scaler,
                d_LJ_A, d_LJ_B, cutoff,
                frc, atom_lj_virial);
        ...
        {
            LJ_Force_With_Atom_Energy_And_Virial_CUDA << <(unsigned int)ceilf((float)atom_numbers / thread_LJ.x), thread_LJ >> >
                (atom_numbers, nl,
                uint_crd, scaler,
                d_LJ_A, d_LJ_B, cutoff,
...
void LENNARD_JONES_INFORMATION::LJ_Energy(const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler,
    const ATOM_GROUP *nl, const float cutoff_square, float *d_LJ_energy_atom)
{
    if (is_initialized)
        LJ_Energy_CUDA << <(unsigned int)ceilf((float)atom_numbers / thread_LJ.x), thread_LJ >> >
            (atom_numbers, nl,
            uint_crd, scaler,
            d_LJ_A, d_LJ_B, cutoff_square,
            d_LJ_energy_atom);
...
//void LENNARD_JONES_INFORMATION::LJ_Energy(const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler,
//    const ATOM_GROUP *nl, const float cutoff_square, int is_reset, int is_sum)
//{
//    if (is_initialized)
//    {
//        if (is_reset)
