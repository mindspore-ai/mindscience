...
struct LENNARD_JONES_INFORMATION {
  ...
  void LJ_Force(const UINT_VECTOR_LJ_TYPE *uint_crd, const VECTOR scaler,
                VECTOR *frc, const ATOM_GROUP *nl, const float cutoff_square);
  //用于计算LJ的作用能量参数基本仿照LJ_Force函数
  void LJ_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const ATOM_GROUP *nl);
  //给SITS使用计算总LJ能的函数
  void LJ_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const ATOM_GROUP *nl,
                 const int is_reset, const int is_sum);
  ...
  void LJ_Force_With_PME_Direct_Force(
      const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd,
      const VECTOR scaler, VECTOR *frc, const ATOM_GROUP *nl,
      const float cutoff,
      const float
          pme_beta); //实际是LJ_Force_With_PME_Direct_Force的一个重载，将atom_numbers暴露出来

  void LJ_Force_With_PME_Direct_Force(const UNSIGNED_INT_VECTOR *uint_crd,
  ...
  void LJ_Energy(
      const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd,
      const VECTOR scaler, const ATOM_GROUP *nl, const float cutoff_square,
      float *
          d_LJ_energy_atom); //直接是LJ_Energy的一个重载，将d_LJ_energy_atom暴露出来
  void LJ_PME_Direct_Force_With_Atom_Energy(
      const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd,
      const VECTOR scaler, VECTOR *frc, const ATOM_GROUP *nl,
      const float cutoff, const float pme_beta, float *atom_energy);

  //上面的函数去掉PME直接部分的版本
  void LJ_Force_With_Atom_Energy_And_Virial(
      const int atom_numbers, const UINT_VECTOR_LJ_TYPE *uint_crd,
      const VECTOR scaler, VECTOR *frc, const ATOM_GROUP *nl,
      const float cutoff, int need_atom_energy, float *atom_energy,
