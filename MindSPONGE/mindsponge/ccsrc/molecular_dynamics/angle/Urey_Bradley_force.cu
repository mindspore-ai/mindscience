...
(const UNSIGNED_INT_VECTOR* uint_crd, const VECTOR scaler, VECTOR* frc, float* atom_energy, float* atom_virial)
{
    if (is_initialized)
    {
        ...
        (uint_crd, scaler, frc, atom_energy, atom_virial);
        angle.Angle_Force_With_Atom_Energy
        (uint_crd, scaler, frc, atom_energy);
    }
}

float UREY_BRADLEY::Get_Energy(const UNSIGNED_INT_VECTOR* uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        ...
        if (is_download)
        {
            cudaMemcpy(angle.h_sigma_of_angle_ene, angle.d_sigma_of_angle_ene, sizeof(float), cudaMemcpyDeviceToHost);
            cudaMemcpy(bond.h_sigma_of_bond_ene, bond.d_sigma_of_bond_ene, sizeof(float), cudaMemcpyDeviceToHost);
