...
__device__ __host__ VECTOR Get_Periodic_Displacement(const VECTOR vec_a, const VECTOR vec_b, const VECTOR box_length, const VECTOR box_length_inverse)
{
    VECTOR dr;
    dr = vec_a - vec_b;
...
{
    Vector_Translation << <(unsigned int)ceilf((float)vector_numbers / threads_per_block), threads_per_block >> >
        (vector_numbers, vec_list, translation_vec);
}
