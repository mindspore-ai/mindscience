...
static __global__ void MD_Iteration_Leap_Frog_With_Langevin(
    ...
    const float gamma_ln, const float *sigma_mass, VECTOR *vel, VECTOR *crd,
...
static __global__ void MD_Iteration_Leap_Frog_With_Langevin_With_Max_Velocity(
    const int atom_numbers, const float dt, const float *inverse_mass,
    const float gamma_ln, const float *sigma_mass, VECTOR *vel, VECTOR *crd,
    VECTOR *frc, VECTOR *acc, VECTOR *random_frc, const float max_velocity) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i < atom_numbers) {
    VECTOR acc_i = inverse_mass[i] * frc[i] - gamma_ln * vel[i] +
                   sigma_mass[i] * random_frc[i];
...
void Langevin_MD_INFORMATION::Initial(CONTROLLER *controller,
  ...
  cudaMemcpy(d_sigma_mass, h_sigma_mass, sizeof(float) * atom_numbers,
...
void Langevin_MD_INFORMATION::MD_Iteration_Leap_Frog(VECTOR *frc, VECTOR *crd,
                                                     VECTOR *vel, VECTOR *acc) {
  if (is_initialized) {
    Rand_Normal<<<(unsigned int)ceilf((float)float4_numbers /
                                      threads_per_block),
                  threads_per_block>>>(float4_numbers, rand_state,
                                       (float4 *)random_force);

    if (max_velocity <= 0) {
      MD_Iteration_Leap_Frog_With_Langevin<<<
          (unsigned int)ceilf((float)atom_numbers / threads_per_block),
          threads_per_block>>>(atom_numbers, dt, d_mass_inverse, gamma_ln,
                               d_sigma_mass, vel, crd, frc, acc, random_force);
    } else {
      MD_Iteration_Leap_Frog_With_Langevin_With_Max_Velocity<<<
          (unsigned int)ceilf((float)atom_numbers / threads_per_block),
          threads_per_block>>>(atom_numbers, dt, d_mass_inverse, gamma_ln,
                               d_sigma_mass, vel, crd, frc, acc, random_force,
                               max_velocity);
