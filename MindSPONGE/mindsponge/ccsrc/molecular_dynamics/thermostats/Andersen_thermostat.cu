...
void ANDERSEN_THERMOSTAT_INFORMATION::Initial(CONTROLLER *controller,
  ...
  Setup_Rand_Normal_Kernel<<<(unsigned int)ceilf((float)float4_numbers / 1024),
                             1024>>>(float4_numbers, rand_state, random_seed);
  ...
  cudaMemcpy(d_factor, h_factor, sizeof(float) * atom_numbers,
  ...
  if (is_initialized) {
    ...
    if (max_velocity <= 0) {
      MD_Iteration_Leap_Frog_With_Andersen<<<
          ...
          atom_numbers, dt / 2, dt, inverse_mass, d_factor, vel, crd, frc, acc,
          random_vel);
    } else {
