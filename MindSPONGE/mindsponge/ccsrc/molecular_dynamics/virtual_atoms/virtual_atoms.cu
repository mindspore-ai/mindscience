...
__global__ void v0_Coordinate_Refresh(const int virtual_numbers,
                                      const VIRTUAL_TYPE_0 *v_info,
                                      const UNSIGNED_INT_VECTOR *uint_crd,
                                      const VECTOR scaler, VECTOR *coordinate) {
...
__global__ void v1_Coordinate_Refresh(const int virtual_numbers,
  ...
  if (i < virtual_numbers) {
    ...
    float a = v_temp.a;
    VECTOR rv1 = a * Get_Periodic_Displacement(uint_crd[atom_2],
                                               uint_crd[atom_1], scaler);
    coordinate[atom_v] = coordinate[atom_1] + rv1;
  }
}

__global__ void v2_Coordinate_Refresh(const int virtual_numbers,
...
__global__ void v3_Coordinate_Refresh(const int virtual_numbers,
...
__global__ void v0_Force_Redistribute(const int virtual_numbers,
                                      const VIRTUAL_TYPE_0 *v_info,
                                      const UNSIGNED_INT_VECTOR *uint_crd,
                                      const VECTOR scaler, VECTOR *force) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i < virtual_numbers) {
    VIRTUAL_TYPE_0 v_temp = v_info[i];
    int atom_v = v_temp.virtual_atom;
...
__global__ void v1_Force_Redistribute(const int virtual_numbers,
                                      const VIRTUAL_TYPE_1 *v_info,
                                      const UNSIGNED_INT_VECTOR *uint_crd,
                                      const VECTOR scaler, VECTOR *force) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void v2_Force_Redistribute(const int virtual_numbers,
                                      const VIRTUAL_TYPE_2 *v_info,
                                      const UNSIGNED_INT_VECTOR *uint_crd,
                                      const VECTOR scaler, VECTOR *force) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void v3_Force_Redistribute(const int virtual_numbers,
                                      const VIRTUAL_TYPE_3 *v_info,
                                      const UNSIGNED_INT_VECTOR *uint_crd,
                                      const VECTOR scaler, VECTOR *force) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
void VIRTUAL_INFORMATION::Initial(CONTROLLER *controller, int atom_numbers,
                                  int *system_freedom,
  ...
  if (controller[0].Command_Exist("virtual_atom_in_file")) {
    ...
    while (fgets(line, 1024, fp) != NULL) {
      ...
      case 1:
        ...
        virtual_level[virtual_atom] =
            std::max(virtual_level[temp[0]], virtual_level[temp[1]]) + 1;
        break;

      case 2:
        scanf_ret = sscanf(line, "%*d %*d %d %d %d %f %f", temp, temp + 1,
                           temp + 2, tempf, tempf + 1);
        if (scanf_ret != 5) {
          controller[0].printf("        Error: can not parse line #%d.\n",
                               line_numbers);
          getchar();
        ...
        virtual_level[virtual_atom] =
            std::max(virtual_level[temp[0]], virtual_level[temp[1]]);
        virtual_level[virtual_atom] =
            std::max(virtual_level[virtual_atom], virtual_level[temp[2]]) + 1;
        break;
      ...
      default:
        controller[0].printf("        Error: can not parse line #%d because %d "
                             "is not a proper type for virtual atoms.\n",
    ...
    for (int layer = 0; layer < max_level; layer++) {
      ...
      if (temp_vl[0].v0_info.virtual_numbers > 0) {
        ...
        Malloc_Safely((void **)&temp_vl[0].v0_info.h_virtual_type_0,
                      sizeof(VIRTUAL_TYPE_0) *
                          temp_vl[0].v0_info.virtual_numbers);
        Cuda_Malloc_Safely((void **)&temp_vl[0].v0_info.d_virtual_type_0,
                           sizeof(VIRTUAL_TYPE_0) *
                               temp_vl[0].v0_info.virtual_numbers);
      }
      if (temp_vl[0].v1_info.virtual_numbers > 0) {
        ...
        Malloc_Safely((void **)&temp_vl[0].v1_info.h_virtual_type_1,
                      sizeof(VIRTUAL_TYPE_1) *
                          temp_vl[0].v1_info.virtual_numbers);
        Cuda_Malloc_Safely((void **)&temp_vl[0].v1_info.d_virtual_type_1,
                           sizeof(VIRTUAL_TYPE_1) *
                               temp_vl[0].v1_info.virtual_numbers);
      }
      if (temp_vl[0].v2_info.virtual_numbers > 0) {
        ...
        Malloc_Safely((void **)&temp_vl[0].v2_info.h_virtual_type_2,
                      sizeof(VIRTUAL_TYPE_2) *
                          temp_vl[0].v2_info.virtual_numbers);
        Cuda_Malloc_Safely((void **)&temp_vl[0].v2_info.d_virtual_type_2,
                           sizeof(VIRTUAL_TYPE_2) *
                               temp_vl[0].v2_info.virtual_numbers);
      }
      if (temp_vl[0].v3_info.virtual_numbers > 0) {
        ...
        Malloc_Safely((void **)&temp_vl[0].v3_info.h_virtual_type_3,
                      sizeof(VIRTUAL_TYPE_3) *
                          temp_vl[0].v3_info.virtual_numbers);
        Cuda_Malloc_Safely((void **)&temp_vl[0].v3_info.d_virtual_type_3,
                           sizeof(VIRTUAL_TYPE_3) *
                               temp_vl[0].v3_info.virtual_numbers);
      }
    }
    ...
    while (fgets(line, 1024, fp) != NULL) {
      ...
      case 0:
        scanf_ret =
            sscanf(line, "%*d %d %d %f",
                   &temp_vl[0].v0_info.h_virtual_type_0[count0].virtual_atom,
                   &temp_vl[0].v0_info.h_virtual_type_0[count0].from_1,
                   &temp_vl[0].v0_info.h_virtual_type_0[count0].h_double);
      ...
      case 1:
        scanf_ret =
            sscanf(line, "%*d %d %d %d %f",
                   &temp_vl[0].v1_info.h_virtual_type_1[count0].virtual_atom,
                   &temp_vl[0].v1_info.h_virtual_type_1[count1].from_1,
                   &temp_vl[0].v1_info.h_virtual_type_1[count1].from_2,
      ...
      case 2:
        scanf_ret =
            sscanf(line, "%*d %d %d %d %d %f %f",
                   &temp_vl[0].v2_info.h_virtual_type_2[count2].virtual_atom,
                   &temp_vl[0].v2_info.h_virtual_type_2[count2].from_1,
                   &temp_vl[0].v2_info.h_virtual_type_2[count2].from_2,
                   &temp_vl[0].v2_info.h_virtual_type_2[count2].from_3,
                   &temp_vl[0].v2_info.h_virtual_type_2[count2].a,
      ...
      case 3:
        scanf_ret =
            sscanf(line, "%*d %d %d %d %d %f %f",
                   &temp_vl[0].v3_info.h_virtual_type_3[count3].virtual_atom,
                   &temp_vl[0].v3_info.h_virtual_type_3[count3].from_1,
                   &temp_vl[0].v3_info.h_virtual_type_3[count3].from_2,
                   &temp_vl[0].v3_info.h_virtual_type_3[count3].from_3,
                   &temp_vl[0].v3_info.h_virtual_type_3[count3].d,
    ...
    for (int layer = 0; layer < max_level; layer++) {
      ...
      if (temp_vl.v0_info.virtual_numbers > 0)
        cudaMemcpy(temp_vl.v0_info.d_virtual_type_0,
      ...
      if (temp_vl.v1_info.virtual_numbers > 0)
        cudaMemcpy(temp_vl.v1_info.d_virtual_type_1,
      ...
      if (temp_vl.v2_info.virtual_numbers > 0)
        cudaMemcpy(temp_vl.v2_info.d_virtual_type_2,
      ...
      if (temp_vl.v3_info.virtual_numbers > 0)
        cudaMemcpy(temp_vl.v3_info.d_virtual_type_3,
...
void VIRTUAL_INFORMATION::Coordinate_Refresh(
    const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *crd) {
  if (is_initialized) {
    //每层之间需要串行计算，层内并行计算
    for (int layer = 0; layer < max_level; layer++) {
      VIRTUAL_LAYER_INFORMATION temp_vl = virtual_layer_info[layer];
      if (temp_vl.v0_info.virtual_numbers > 0)
        v0_Coordinate_Refresh<<<(unsigned int)ceilf(
                                    (float)temp_vl.v0_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v0_info.virtual_numbers, temp_vl.v0_info.d_virtual_type_0,
            uint_crd, scaler, crd);
      if (temp_vl.v1_info.virtual_numbers > 0)
        v1_Coordinate_Refresh<<<(unsigned int)ceilf(
                                    (float)temp_vl.v1_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v1_info.virtual_numbers, temp_vl.v1_info.d_virtual_type_1,
            uint_crd, scaler, crd);
      if (temp_vl.v2_info.virtual_numbers > 0)
        v2_Coordinate_Refresh<<<(unsigned int)ceilf(
                                    (float)temp_vl.v2_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v2_info.virtual_numbers, temp_vl.v2_info.d_virtual_type_2,
            uint_crd, scaler, crd);
      if (temp_vl.v3_info.virtual_numbers > 0)
        v3_Coordinate_Refresh<<<(unsigned int)ceilf(
                                    (float)temp_vl.v3_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v3_info.virtual_numbers, temp_vl.v3_info.d_virtual_type_3,
            uint_crd, scaler, crd);
    }
  }
}

void VIRTUAL_INFORMATION::Force_Redistribute(
  ...
  if (is_initialized) {
    ...
    for (int layer = max_level - 1; layer >= 0; layer--) {
      VIRTUAL_LAYER_INFORMATION temp_vl = virtual_layer_info[layer];
      if (temp_vl.v0_info.virtual_numbers > 0)
        v0_Force_Redistribute<<<(unsigned int)ceilf(
                                    (float)temp_vl.v0_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v0_info.virtual_numbers, temp_vl.v0_info.d_virtual_type_0,
            uint_crd, scaler, frc);
      if (temp_vl.v1_info.virtual_numbers > 0)
        v1_Force_Redistribute<<<(unsigned int)ceilf(
                                    (float)temp_vl.v1_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v1_info.virtual_numbers, temp_vl.v1_info.d_virtual_type_1,
            uint_crd, scaler, frc);
      if (temp_vl.v2_info.virtual_numbers > 0)
        v2_Force_Redistribute<<<(unsigned int)ceilf(
                                    (float)temp_vl.v2_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v2_info.virtual_numbers, temp_vl.v2_info.d_virtual_type_2,
            uint_crd, scaler, frc);
      if (temp_vl.v3_info.virtual_numbers > 0)
        v3_Force_Redistribute<<<(unsigned int)ceilf(
                                    (float)temp_vl.v3_info.virtual_numbers /
                                    threads_per_block),
                                threads_per_block>>>(
            temp_vl.v3_info.virtual_numbers, temp_vl.v3_info.d_virtual_type_3,
