...
struct VIRTUAL_INFORMATION {
  ...
  void Initial(CONTROLLER *controller, int atom_numbers, int *system_freedom,
               const char *module_name = NULL); //初始化
  void Clear();                                 //清除内存
  void Force_Redistribute(const UNSIGNED_INT_VECTOR *uint_crd,
                          const VECTOR scaler, VECTOR *force); //进行力重分配

  void Coordinate_Refresh(const UNSIGNED_INT_VECTOR *uint_crd,
                          const VECTOR scaler,
                          VECTOR *crd); //更新虚拟位点的坐标
};
#endif
