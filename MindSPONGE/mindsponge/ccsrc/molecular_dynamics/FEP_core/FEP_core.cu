...
static __global__ void Seperate_Direct_Atom_Energy_CUDA(
    ...
    const float *charge, const int *subsys_mask, const float beta,
    const float cutoff_square, float *direct_ene_intersys,
    float *direct_ene_intrasys) {
...
void FEP_CORE::non_bond_information::Initial(CONTROLLER *controller,
                                             FEP_CORE *FEP_core) {
  ...
  if (controller[0].Command_Exist("exclude_in_file")) {
    ...
    cudaMemcpy(d_excluded_list_start, h_excluded_list_start,
               sizeof(int) * FEP_core->atom_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(d_excluded_numbers, h_excluded_numbers,
               sizeof(int) * FEP_core->atom_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(d_excluded_list, h_excluded_list,
               sizeof(int) * excluded_atom_numbers, cudaMemcpyHostToDevice);
...
void FEP_CORE::periodic_box_condition_information::Update_Volume(
    VECTOR box_length) {
...
void FEP_CORE::trajectory_input::Initial(CONTROLLER *controller,
                                         FEP_CORE *FEP_core) {
  ...
  Malloc_Safely((void **)&(FEP_core->data.frame_ene),
                sizeof(float) * frame_numbers);
  Malloc_Safely((void **)&(FEP_core->data.frame_partition_ene),
                sizeof(partition_energy_data) * frame_numbers);
}
...
void FEP_CORE::Initial(CONTROLLER *controller) {
  ...
  Cuda_Malloc_Safely((void **)&d_direct_atom_energy_intersys,
                     sizeof(float) * atom_numbers);
  Cuda_Malloc_Safely((void **)&d_direct_atom_energy_intrasys,
                     sizeof(float) * atom_numbers);
  ...
  if (controller->Command_Exist(FLOAT32ENE_COMMAND)) {
    Open_File_Safely(&float32ene_file, controller->Command(FLOAT32ENE_COMMAND),
                     "ab");
  } else {
...
void FEP_CORE::energy_data::Sum_One_Frame(int current_frame) {
  current_frame_ene =
      partition.bond_ene + partition.angle_ene + partition.dihedral_ene +
      partition.nb14_LJ_ene + partition.nb14_EE_ene + partition.bond_soft_ene +
      partition.vdw_intersys_ene + partition.vdw_intrasys_ene +
      partition.coul_direct_intersys_ene + partition.coul_direct_intrasys_ene +
      partition.vdw_long_range_correction + partition.coul_long_range +
      partition.pV;

  frame_partition_ene[current_frame - 1] = partition;
  frame_ene[current_frame - 1] =
      (current_frame_ene) / temperature / CONSTANT_kB;
}
...
void FEP_CORE::Print_Pure_Ene_To_Result_File() {
  ...
  fwrite(data.frame_partition_ene, sizeof(partition_energy_data),
         input.frame_numbers, float32ene_file);
}

void FEP_CORE::Seperate_Direct_Atom_Energy(ATOM_GROUP *nl,
                                           const float pme_beta) {
  Reset_List<<<ceilf((float)atom_numbers / 1024.0f), 1024>>>(
      atom_numbers, d_direct_atom_energy_intersys, 0.0f);
  Reset_List<<<ceilf((float)atom_numbers / 1024.0f), 1024>>>(
      atom_numbers, d_direct_atom_energy_intrasys, 0.0f);
  Seperate_Direct_Atom_Energy_CUDA<<<atom_numbers / 32 + 1, 32>>>(
      atom_numbers, nl, uint_crd, pbc.uint_dr_to_dr_cof, d_charge,
      d_subsys_division, pme_beta, nb.cutoff * nb.cutoff,
      d_direct_atom_energy_intersys, d_direct_atom_energy_intrasys);
  Sum_Of_List<<<1, 1024>>>(atom_numbers, d_direct_atom_energy_intersys,
                           d_direct_ene_intersys);
  Sum_Of_List<<<1, 1024>>>(atom_numbers, d_direct_atom_energy_intrasys,
                           d_direct_ene_intrasys);

  cudaMemcpy(&(data.partition.coul_direct_intersys_ene), d_direct_ene_intersys,
             sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(&(data.partition.coul_direct_intrasys_ene), d_direct_ene_intrasys,
             sizeof(float), cudaMemcpyDeviceToHost);
}
