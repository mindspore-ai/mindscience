...
static __global__ void Dihedral_Energy_CUDA(
    ...
    const float *gamc, const float *gams, const float *pn, float *ene) {
...
static __global__ void Dihedral_Force_With_Atom_Energy_CUDA(
    const int dihedral_numbers, const UNSIGNED_INT_VECTOR *uint_crd,
    const VECTOR scaler, const int *atom_a, const int *atom_b,
    const int *atom_c, const int *atom_d, const int *ipn, const float *pk,
...
void DIHEDRAL::Memory_Allocate() {
  if (!Malloc_Safely((void **)&this->h_atom_a,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_atom_a in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_b,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_atom_b in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_c,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_atom_c in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_d,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_atom_d in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_ipn,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_ipn in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_pk,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_pk in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_gamc,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_gamc in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_gams,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_gams in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_pn,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_pn in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_dihedral_ene,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc DIHEDARL::h_dihedral_energy in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_sigma_of_dihedral_ene, sizeof(float)))
    printf("Error occurs when malloc DIHEDARL::h_sigma_energy in "
           "DIHEDRAL::Dihedral_Initialize");

  // CUDA
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_a,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_atom_a in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_b,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_atom_b in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_c,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_atom_c in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_d,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_atom_d in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_ipn,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_ipn in "
           "DIHEDRAL::Dihedral_Initialize");

  if (!Cuda_Malloc_Safely((void **)&this->d_pk,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_pk in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_gamc,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_gamc in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_gams,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_gams in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_pn,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_pn in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_dihedral_ene,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc DIHEDARL::d_dihedral_energy in "
           "DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_sigma_of_dihedral_ene,
                          sizeof(float)))
    printf("Error occurs when CUDA malloc DIHEDARL::d_sigma_energy in "
           "DIHEDRAL::Dihedral_Initialize");
}

void DIHEDRAL::Parameter_Host_To_Device() {
  cudaMemcpy(this->d_atom_a, this->h_atom_a,
             sizeof(int) * this->dihedral_numbers, cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_atom_b, this->h_atom_b,
             sizeof(int) * this->dihedral_numbers, cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_atom_c, this->h_atom_c,
             sizeof(int) * this->dihedral_numbers, cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_atom_d, this->h_atom_d,
             sizeof(int) * this->dihedral_numbers, cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_ipn, this->h_ipn, sizeof(int) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);

  cudaMemcpy(this->d_pk, this->h_pk, sizeof(float) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_pn, this->h_pn, sizeof(float) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_gamc, this->h_gamc, sizeof(float) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_gams, this->h_gams, sizeof(float) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);
  cudaMemcpy(this->d_dihedral_ene, this->h_dihedral_ene,
             sizeof(float) * this->dihedral_numbers, cudaMemcpyHostToDevice);

  cudaMemcpy(this->d_sigma_of_dihedral_ene, this->h_sigma_of_dihedral_ene,
...
float DIHEDRAL::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd,
                           const VECTOR scaler, int is_download) {
  if (is_initialized) {
    Dihedral_Energy_CUDA<<<(unsigned int)ceilf((float)this->dihedral_numbers /
                                               this->threads_per_block),
                           this->threads_per_block>>>(
        this->dihedral_numbers, uint_crd, scaler, this->d_atom_a,
        this->d_atom_b, this->d_atom_c, this->d_atom_d, this->d_ipn, this->d_pk,
        this->d_gamc, this->d_gams, this->d_pn, this->d_dihedral_ene);
    Sum_Of_List<<<1, 1024>>>(this->dihedral_numbers, this->d_dihedral_ene,
                             this->d_sigma_of_dihedral_ene);

    if (is_download) {
      cudaMemcpy(this->h_sigma_of_dihedral_ene, this->d_sigma_of_dihedral_ene,
...
void DIHEDRAL::Dihedral_Force_With_Atom_Energy(
    const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc,
    float *atom_energy) {
  if (is_initialized) {
    Dihedral_Force_With_Atom_Energy_CUDA<<<(unsigned int)ceilf(
                                               (float)this->dihedral_numbers /
                                               this->threads_per_block),
                                           this->threads_per_block>>>(
        this->dihedral_numbers, uint_crd, scaler, this->d_atom_a,
        this->d_atom_b, this->d_atom_c, this->d_atom_d, this->d_ipn, this->d_pk,
        this->d_gamc, this->d_gams, this->d_pn, frc, atom_energy);
  }
}
