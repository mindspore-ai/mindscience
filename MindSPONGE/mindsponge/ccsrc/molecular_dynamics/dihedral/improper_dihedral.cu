...
Dihedral_Energy_CUDA(const int dihedral_numbers,
                     ...
                     const int *atom_d, const float *pk, const float *phi0,
                     float *ene) {
...
static __global__ void Dihedral_Force_With_Atom_Energy_CUDA(
    ...
    const int *atom_c, const int *atom_d, const float *pk, const float *phi0,
...
void IMPROPER_DIHEDRAL::Initial(CONTROLLER *controller,
...
void IMPROPER_DIHEDRAL::Memory_Allocate() {
  if (!Malloc_Safely((void **)&this->h_atom_a,
    ...
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_atom_a in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_b,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_atom_b in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_c,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_atom_c in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_atom_d,
                     sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_atom_d in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_pk,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_pk in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_phi0,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_phi0 in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_dihedral_ene,
                     sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_dihedral_energy in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Malloc_Safely((void **)&this->h_sigma_of_dihedral_ene, sizeof(float)))
    printf("Error occurs when malloc IMPROPER_DIHEDRAL::h_sigma_energy in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");

  // CUDA
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_a,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_atom_a in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_b,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_atom_b in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_c,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_atom_c in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_atom_d,
                          sizeof(int) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_atom_d in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");

  if (!Cuda_Malloc_Safely((void **)&this->d_pk,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_pk in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_phi0,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_phi0 in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_dihedral_ene,
                          sizeof(float) * this->dihedral_numbers))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_dihedral_energy "
           "in IMPROPER_DIHEDRAL::Dihedral_Initialize");
  if (!Cuda_Malloc_Safely((void **)&this->d_sigma_of_dihedral_ene,
                          sizeof(float)))
    printf("Error occurs when CUDA malloc IMPROPER_DIHEDRAL::d_sigma_energy in "
           "IMPROPER_DIHEDRAL::Dihedral_Initialize");
}
...
void IMPROPER_DIHEDRAL::Parameter_Host_To_Device() {
  ...
  cudaMemcpy(this->d_phi0, this->h_phi0, sizeof(float) * this->dihedral_numbers,
             cudaMemcpyHostToDevice);
...
float IMPROPER_DIHEDRAL::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd,
  ...
  if (is_initialized) {
    Dihedral_Energy_CUDA<<<(unsigned int)ceilf((float)this->dihedral_numbers /
        ...
        this->d_atom_b, this->d_atom_c, this->d_atom_d, this->d_pk,
        this->d_phi0, this->d_dihedral_ene);
...
void IMPROPER_DIHEDRAL::Dihedral_Force_With_Atom_Energy(
