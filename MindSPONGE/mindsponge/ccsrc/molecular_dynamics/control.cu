...
void CONTROLLER::Default_Set() { srand((unsigned)time(NULL)); }
...
void CONTROLLER::Init_Cuda() {
  ...
  if (count <= target) {
    printf("        Error: The available device count %d is less than the "
           "setting target %d.",
           count, target);
    exit(0);
  }
...
void CONTROLLER::Input_Check() {
  if (!(Command_Exist("dont_check_input") &&
    ...
    for (CheckMap::iterator iter = choice_check.begin();
      ...
      if (iter->second == 2) {
        printf("Warning: the value '%s' of command '%s' matches none of the "
               "choices.\n",
    ...
    if (no_warning) {
      printf("\nWarning: inputs raised %d warning(s). If You know WHAT YOU ARE "
             "DOING, press any key to continue. Set dont_check_input = 1 to "
...
void CONTROLLER::Print_To_Screen_And_Mdout(FILE *mdout) {
  ...
  for (int i = 0; i < outputs_key.size(); i++) {
    line_numbers++;
    fprintf(stdout, "%12s = %12s, ", outputs_key[i].c_str(),
...
void TIME_RECORDER::Start() { start_timestamp = clock(); }
