    ...
    const int *atom_a, const int *atom_b, const float *bond_k, const float *bond_r0, const int * AB_mask, VECTOR *frc, float *atom_energy, float *atom_virial, const float lambda, const float alpha)
{
    ...
    if (bond_i < bond_numbers)
    {
        ...
        float temp_denominator = 1 + alpha * tmp_lambda_ * (abs_r - r0)*(abs_r - r0);
        float tempf = 2 * k * tmp_lambda * (abs_r - r0) / temp_denominator / temp_denominator;
        ...
        atomicAdd(&atom_energy[atom_i], tmp_lambda * k * (abs_r - r0) * (abs_r - r0) / temp_denominator);
    }
}

static __global__ void Soft_Bond_Energy_CUDA(const int bond_numbers, const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler,
    const int *atom_a, const int *atom_b, const float *bond_k, const float *bond_r0, const int * AB_mask, float *bond_ene, const float lambda, const float alpha)
{
    int bond_i = blockDim.x*blockIdx.x + threadIdx.x;
    if (bond_i < bond_numbers)
    {
        ...
        bond_ene[bond_i] = tmp_lambda * k * tempf2/(1 + alpha * (1 - tmp_lambda) * tempf2);
    }
}
...
void BOND_SOFT::Initial(CONTROLLER *controller, const char *module_name)
{
    ...
    if (controller[0].Command_Exist(this->module_name, "in_file"))
    {
        ...
        {
            printf("Error: FEP lambda of bond must be given for the calculation of SOFT BOND.\n");
            getchar();
        }
        ...
        {
            printf("Warning: FEP alpha of soft bond missing for the calculation of SOFT BOND, set to default value 0.0.\n");
            this->alpha = 0.0;
        }
        FILE *fp = NULL;
        ...
        {
            toscan = fscanf(fp, "%d %d %f %f %d", h_atom_a + i, h_atom_b + i, h_k + i, h_r0 + i, h_ABmask+i);
        }
        fclose(fp);
...
{
    if (!Malloc_Safely((void**)&(this->h_atom_a), sizeof(int)*this->soft_bond_numbers))
        printf("        Error occurs when malloc BOND_SOFT::h_atom_a in BOND_SOFT::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_atom_b), sizeof(int)*this->soft_bond_numbers))
        printf("        Error occurs when malloc BOND_SOFT::h_atom_b in BOND_SOFT::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_k), sizeof(float)*this->soft_bond_numbers))
        printf("        Error occurs when malloc BOND_SOFT::h_k in BOND_SOFT::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_r0), sizeof(float)*this->soft_bond_numbers))
        printf("        Error occurs when malloc BOND_SOFT::h_r0 in BOND_SOFT::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_ABmask), sizeof(int)*this->soft_bond_numbers))
        printf("        Error occurs when malloc BOND_SOFT::h_ABmask in BOND_SOFT::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_soft_bond_ene), sizeof(float)*this->soft_bond_numbers))
    ...
    if (!Cuda_Malloc_Safely((void**)&this->d_atom_b, sizeof(int)*this->soft_bond_numbers))
        printf("        Error occurs when CUDA malloc BOND_SOFT::d_atom_b in BOND_SOFT::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_k, sizeof(float)*this->soft_bond_numbers))
        printf("        Error occurs when CUDA malloc BOND_SOFT::d_k in BOND_SOFT::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_r0, sizeof(float)*this->soft_bond_numbers))
        printf("        Error occurs when CUDA malloc BOND_SOFT::d_r0 in BOND_SOFT::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_ABmask, sizeof(int)*this->soft_bond_numbers))
        printf("         Error occurs when CUDA malloc BOND_SOFT::d_ABmask in BOND_SOFT::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_soft_bond_ene, sizeof(float)*this->soft_bond_numbers))
...
{
    cudaMemcpy(this->d_atom_a, this->h_atom_a, sizeof(int)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_atom_b, this->h_atom_b, sizeof(int)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_k, this->h_k, sizeof(float)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_r0, this->h_r0, sizeof(float)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_ABmask, this->h_ABmask, sizeof(float)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_soft_bond_ene, this->h_soft_bond_ene, sizeof(float)*this->soft_bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_sigma_of_soft_bond_ene, this->h_sigma_of_soft_bond_ene, sizeof(float), cudaMemcpyHostToDevice);
}
...
void BOND_SOFT::Soft_Bond_Force_With_Atom_Energy_And_Virial(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc, float * atom_energy, float *atom_virial)
{
    if (is_initialized)
    {
        Soft_Bond_Force_With_Atom_Energy_And_Virial_CUDA << <(unsigned int)ceilf((float)this->soft_bond_numbers / this->threads_per_block), this->threads_per_block >> >
            (this->soft_bond_numbers, uint_crd, scaler,
            this->d_atom_a, this->d_atom_b, this->d_k, this->d_r0, this->d_ABmask, frc, atom_energy, atom_virial, this->lambda, this->alpha);
    }
}

float BOND_SOFT::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        Soft_Bond_Energy_CUDA << <(unsigned int)ceilf((float)this->soft_bond_numbers / this->threads_per_block), this->threads_per_block >> >
            (this->soft_bond_numbers, uint_crd, scaler,
            this->d_atom_a, this->d_atom_b, this->d_k, this->d_r0, this->d_ABmask, this->d_soft_bond_ene, this->lambda, this->alpha);

        Sum_Of_List << <1, 1024 >> >(this->soft_bond_numbers, this->d_soft_bond_ene, this->d_sigma_of_soft_bond_ene);
        if (is_download)
        {
            cudaMemcpy(this->h_sigma_of_soft_bond_ene, this->d_sigma_of_soft_bond_ene, sizeof(float), cudaMemcpyDeviceToHost);
