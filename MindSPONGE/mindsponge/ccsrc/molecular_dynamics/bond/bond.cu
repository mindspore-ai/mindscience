...
static __global__ void Bond_Energy_CUDA(const int bond_numbers, const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler,
...
//将bond 对原子上的力（frc）、原子上的能量（atom_energy）、原子上的维力值（atom_virial）
//一并计算。
//对于简易和轻度修改，可以不用考虑能量与维力值的计算。
//  在不使用涉及维力系数的模拟中，可以不用计算正确的维力值
//  在能量数值不影响模拟的过程中，可以不用计算正确的能量值
//  只有力是最基本的计算要求
static __global__ void Bond_Force_With_Atom_Energy_And_Virial_CUDA(const int bond_numbers, const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler,
    const int *atom_a, const int *atom_b, const float *bond_k, const float *bond_r0, VECTOR *frc, float *atom_energy, float *atom_virial)
{
    int bond_i = blockDim.x*blockIdx.x + threadIdx.x;
    if (bond_i < bond_numbers)
    {
...
void BOND::Initial(CONTROLLER *controller, const char *module_name)
{
    ...
    if (controller[0].Command_Exist(this->module_name, file_name_suffix))
    {
        ...
        {
             read_ret = fscanf(fp, "%d %d %f %f", h_atom_a + i, h_atom_b + i, h_k + i, h_r0 + i);
        }
        fclose(fp);
...
void BOND::Read_Information_From_AMBERFILE(const char *file_name, CONTROLLER controller)
{
    ...
    while (true)
    {
            ...
            && strcmp(temp_second_str, "POINTERS") == 0)
        {
            ...
            if (!Malloc_Safely((void**)&bond_type_k, sizeof(float)* bond_type_numbers))
            {
                controller.printf("        Error occurs when malloc bond_type_k in BOND::Read_Information_From_AMBERFILE");
            }
            
            if (!Malloc_Safely((void**)&bond_type_r, sizeof(float)* bond_type_numbers))
            {
                controller.printf("        Error occurs when malloc bond_type_r in BOND::Read_Information_From_AMBERFILE");
            }
        }
...
{
    if (!Malloc_Safely((void**)&(this->h_atom_a), sizeof(int)*this->bond_numbers))
        printf("        Error occurs when malloc BOND::h_atom_a in BOND::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_atom_b), sizeof(int)*this->bond_numbers))
        printf("        Error occurs when malloc BOND::h_atom_b in BOND::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_k), sizeof(float)*this->bond_numbers))
        printf("        Error occurs when malloc BOND::h_k in BOND::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_r0), sizeof(float)*this->bond_numbers))
        printf("        Error occurs when malloc BOND::h_r0 in BOND::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_bond_ene), sizeof(float)*this->bond_numbers))
        printf("        Error occurs when malloc BOND::h_bond_ene in BOND::Memory_Allocate");
    if (!Malloc_Safely((void**)&(this->h_sigma_of_bond_ene), sizeof(float)))
        printf("        Error occurs when malloc BOND::h_sigma_of_bond_ene in BOND::Memory_Allocate");

    if (!Cuda_Malloc_Safely((void**)&this->d_atom_a, sizeof(int)*this->bond_numbers))
        printf("        Error occurs when CUDA malloc BOND::d_atom_a in BOND::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_atom_b, sizeof(int)*this->bond_numbers))
        printf("        Error occurs when CUDA malloc BOND::d_atom_b in BOND::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_k, sizeof(float)*this->bond_numbers))
        printf("        Error occurs when CUDA malloc BOND::d_k in BOND::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_r0, sizeof(float)*this->bond_numbers))
        printf("        Error occurs when CUDA malloc BOND::d_r0 in BOND::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_bond_ene, sizeof(float)*this->bond_numbers))
        printf("        Error occurs when CUDA malloc BOND::d_bond_ene in BOND::Memory_Allocate");
    if (!Cuda_Malloc_Safely((void**)&this->d_sigma_of_bond_ene, sizeof(float)))
        printf("        Error occurs when CUDA malloc BOND::d_sigma_of_bond_ene in BOND::Memory_Allocate");
}

void BOND::Parameter_Host_To_Device()
{
    cudaMemcpy(this->d_atom_a, this->h_atom_a, sizeof(int)*this->bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_atom_b, this->h_atom_b, sizeof(int)*this->bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_k, this->h_k, sizeof(float)*this->bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_r0, this->h_r0, sizeof(float)*this->bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_bond_ene, this->h_bond_ene, sizeof(float)*this->bond_numbers, cudaMemcpyHostToDevice);
    cudaMemcpy(this->d_sigma_of_bond_ene, this->h_sigma_of_bond_ene, sizeof(float), cudaMemcpyHostToDevice);
}
...
float BOND::Get_Energy(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, int is_download)
{
    if (is_initialized)
    {
        Bond_Energy_CUDA << <(unsigned int)ceilf((float)this->bond_numbers / this->threads_per_block), this->threads_per_block >> >
            (this->bond_numbers, uint_crd, scaler,
            this->d_atom_a, this->d_atom_b, this->d_k, this->d_r0, this->d_bond_ene);

        Sum_Of_List << <1, 1024 >> >(this->bond_numbers, this->d_bond_ene, this->d_sigma_of_bond_ene);
        if (is_download)
        {
            cudaMemcpy(this->h_sigma_of_bond_ene, this->d_sigma_of_bond_ene, sizeof(float), cudaMemcpyDeviceToHost);
...
void BOND::Bond_Force_With_Atom_Energy_And_Virial(const UNSIGNED_INT_VECTOR *uint_crd, const VECTOR scaler, VECTOR *frc, float *atom_energy, float *atom_virial)
{
    if (is_initialized)
    {
        Bond_Force_With_Atom_Energy_And_Virial_CUDA << <(unsigned int)ceilf((float)this->bond_numbers / this->threads_per_block), this->threads_per_block >> >
            (this->bond_numbers, uint_crd, scaler,
            this->d_atom_a, this->d_atom_b, this->d_k, this->d_r0, frc, atom_energy, atom_virial);
    }
}
