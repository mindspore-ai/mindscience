...
from mindsponge.pipeline.data import get_chi_atom_pos_indices
    ...
    def construct(self, target_feat, msa_feat, msa_mask, seq_mask, aatype,
        ...
        if self.template_enabled and self.template_embed_torsion_angles:
            ...
            torsion_angles_sin_cos, alt_torsion_angles_sin_cos, torsion_angles_mask\
                = atom37_to_torsion_angles(template_aatype, template_all_atom_positions,
                                           template_all_atom_masks, self.chi_atom_indices,
                                           self.chi_angles_mask, self.mirror_psi_mask,
                                           self.chi_pi_periodic, self.indices0, self.indices1)
            template_features = mnp.concatenate([aatype_one_hot,
                                                 mnp.reshape(torsion_angles_sin_cos,
                                                             [num_templ, num_res, 14]),
                                                 mnp.reshape(alt_torsion_angles_sin_cos,
                                                             [num_templ, num_res, 14]),
