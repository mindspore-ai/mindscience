...
from .. import data
from .. import util
from ..util import scatter_mean, scatter_add, scatter_max
...
class GraphAttentionConv(MessagePassingBase):
    ...
    def __init__(self, input_dim, output_dim, edge_input_dim=None, n_head=1, negative_slope=0.2,
        ...
        self.linear = nn.Dense(input_dim, output_dim)
        if edge_input_dim:
            self.edge_linear = nn.Dense(edge_input_dim, output_dim)
        else:
            self.edge_linear = None
    ...
    def message(self, graph, inputs):
        ...
        message = util.flatten(attention * value, start=1)
        return message
...
class RelationalGraphConv(MessagePassingBase):
    ...
    def __init__(self, input_dim, output_dim, n_relation, edge_input_dim=None, batch_norm=False, activation="relu"):
        ...
        self.self_loop = nn.Dense(input_dim, output_dim)
        self.linear = nn.Dense(n_relation * input_dim, output_dim)
        if edge_input_dim:
            self.edge_linear = nn.Dense(edge_input_dim, input_dim)
        else:
            self.edge_linear = None
    ...
    def aggregate(self, graph, message):
        ...
        node_out = graph.edges[1] * self.n_relation + graph.node_type
