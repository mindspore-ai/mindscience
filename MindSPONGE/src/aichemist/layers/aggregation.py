...
class MeanAggregation(nn.Cell):
    ...
    def construct(self, node2graph, inputs, axis=0):
        ...
        output = util.scatter_mean(inputs, node2graph, axis=axis, n_axis=batch_size)
        return output
...
class SumAggregation(nn.Cell):
    ...
    def construct(self, node2graph, inputs, axis=0):
        ...
        """
        batch_size = node2graph.max() + 1
        output = util.scatter_add(inputs, node2graph, axis=axis, n_axis=batch_size)
        return output
...
class MaxAggregation(nn.Cell):
    ...
    def construct(self, node2graph, inputs, axis=0):
        ...
        """
        batch_size = node2graph.max() + 1
        output = util.scatter_max(inputs, node2graph, axis=axis, n_axis=batch_size)[0]
        return output
...
class Softmax(nn.Cell):
    ...
    def construct(self, node2graph, inputs, axis=0):
        ...
        """
        batch_size = node2graph.max() + 1
        x = inputs - util.scatter_max(inputs, node2graph, axis=axis, n_axis=batch_size)[0][node2graph]
        x = x.exp()
        normalizer = util.scatter_add(x, node2graph, axis=axis, n_axis=batch_size)[node2graph]
...
class Set2Set(nn.Cell):
    ...
    def construct(self, node2graph, inputs, axis=0):
        ...
        for _ in range(self.num_step):
            ...
            output = util.scatter_add(attention.expand_dims(-1) * inputs,
