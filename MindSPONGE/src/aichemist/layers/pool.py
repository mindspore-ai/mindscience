...
from ..util import scatter_add, scatter_mean

from ..data import Graph, GraphBatch
from .. import core


class DiffPool(core.Cell):
    """
    ...
    def __init__(self, input_dim, output_node, feature_layer=None, pool_layer=None, loss_weight=1, zero_diagonal=False,
        ...
        if pool_layer is not None:
            self.linear = nn.Dense(pool_layer.output_dim, output_node)
        else:
            self.linear = nn.Dense(input_dim, output_node)
    ...
    def dense_pool(self, graph, inputs, assignment):
        ...
        if isinstance(graph, GraphBatch):
            n_nodes = ops.ones(len(graph), dtype=ms.int64) * self.output_node
            n_edges = ops.ones(len(graph), dtype=ms.int64) * self.output_node ** 2
