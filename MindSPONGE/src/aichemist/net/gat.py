...
class GraphAttentionNet(core.Cell):
    """
    ...
    def __init__(self, input_dim, hidden_dims, edge_input_dim=None, n_head=1, negative_slope=0.2, short_cut=False,
        ...
        if readout == "sum":
            self.readout = layers.SumAggregation()
        elif readout == "mean":
            self.readout = layers.MeanAggregation()
        else:
    ...
    def construct(self, graph, inputs):
        ...
        Returns:
            dict with ``node_feat`` and ``graph_feat`` fields:
...
class KnowledgeBaseAttnNet(GraphAttentionNet):
    ...
    def __init__(self,
        ...
        self.linear = nn.Dense(self.output_dim, embedding_dim)
        self.output_dim = embedding_dim

        norm = Normal(-max_score / embedding_dim, max_score / embedding_dim)
        entity = initializer(norm, (n_entity, embedding_dim))
        relation = initializer(norm, (n_relation, embedding_dim))
    ...
    def construct(self, graph, inputs):
        ...
        score = util.transe_score(entity, self.relation, h_index, t_index, r_index)
