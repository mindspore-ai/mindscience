...
def multinomial(inputs, num_sample, replacement=False):
    """
    Fast multinomial sampling. This is the default implementation in PyTorch v1.6.0+.

    Parameters:
        inputs (Tensor): unnormalized distribution
        num_sample (int): number of samples
        replacement (bool, optional): sample with replacement or not
    """
    if replacement:
        return ops.multinomial(inputs, num_sample, replacement)

    rand = ops.rand_like(inputs).log() / inputs
...
def mean_with_nan(inputs, axis=None, keepdim=False):
    """
    ...
    return masked_mean(inputs, mask, axis, keepdim)
...
def extend(data, size, inputs, input_size):
    ...
    new_data = ops.zeros(new_cum_size[-1], *data.shape[1:], dtype=data.dtype)
...
def one_hot(index, size):
    ...
    result = ops.zeros(shape)
    if index.numel():
        assert index.min() >= 0
        assert index.max() < size
        result.scatter_(-1, index.expand_dims(-1), 1)
    return result
...
def clipped_policy_gradient_objective(policy, agent, reward, eps=0.2):
    ...
    objective = ops.min(ratio * reward, ratio.clip(1 - eps, 1 + eps) * reward)
    return objective
