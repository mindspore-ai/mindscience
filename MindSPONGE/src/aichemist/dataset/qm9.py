...
class QM9(data.MolSet):
    ...
    def __init__(self, path, batch_size=128, verbose=1, shuffle=True, info='graph', **kwargs):
        info = ['atom_coord', 'atom_type']
        super().__init__(batch_size=batch_size, verbose=verbose,
                         info=info, shuffle=shuffle, **kwargs)
        path = os.path.expanduser(path)
        if not os.path.exists(path):
            os.makedirs(path)
        self.path = path

    def load(self, max_len=None):
        """load"""
        fname = self.path + '/' + self.fname
        if not os.path.exists(self.fname):
            fname = util.download(self.url, self.path, md5=self.md5)
        sdf_file = util.extract(fname, "gdb9.sdf")
        csv_file = util.extract(fname, "gdb9.sdf.csv")
